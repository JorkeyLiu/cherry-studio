...
- **Owned isolated root**: each scenario owns a **realpath-canonicalized `mkdtemp` root** containing: synthetic data DB, synthetic `Data`/`Files` fixtures, IndexedDB/Local Storage fixtures, destination archive, and restore staging. **Fixture construction, manager setup, archive inspection, parity checking, and cleanup are outside timing** (see §7.2-5). **M8 is not read-only in synthetic space — this owned temporary backup/restore I/O is permitted; read-only qualification applies only to production/user state.**
...
- **Exclude**: fixture generation, DB migration/seeding, manager construction, archive metadata inspection before the call, and final cleanup. `restore_ms_<suffix>` includes callback parity because parity is required inside `preExitCleanup` (LOCK-004). Pre-staging validation failures require zero `relaunch`/`exit` (no timing artifact); callback parity failure blocks cleanup/relaunch and emits no artifact.
- **Warmup=1, samples=5 per scenario**; **no threshold gate**. Do **not** claim isolated substage timings unless separately designed. Correctness/parity gates must pass before timing is considered (bounded declared scale, no path/content/user data).
...
- **Activation & status**: M8 remains **Planned/Open and unexecuted/unactivated** after this documentation. M8 warrants **directional L3 diagnostic evidence only** (if ever activated). **No package command is registered, no bench/test file is added, no run is claimed** in this phase.
- **Privacy-safe output**: no message text/entry names/absolute paths/raw user data/credentials/per-user fingerprint in `metrics`/`gates`/`scale`; metadata version is numeric/`detail`, not free-form; parity uses digests/counts only. Artifact is **privacy-safe schema-v1, gitignored**; large raw DB/profile artifacts are never retained (`performance-measurement.md` §4).
