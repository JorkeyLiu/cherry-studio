...
| **6** | DB-Health Implementation | **S6.1–S6.3 Authorized & Implemented; S6.4 Candidate Defined — Evidence Authorized (decision evidence only; production Not Authorized); S6.5 Candidate — Not Authorized** | Phase 5 contract |
...
- **Status**: **S6.1–S6.3 Authorized & Implemented; S6.4 Candidate Defined — Evidence Authorized (decision evidence only, SQ-01 synthetic comparison); S6.4 production implementation and S6.5 remain Candidate — Not Authorized**. Per-slice explicit approval required; Phase 5 closed 2026-08-29, Phase 6 partially Open; no capacity-threshold adoption.
...
| **S6.4** Short-query candidate SQ-01 (≤2-codepoint gram projection) | Derived auxiliary 1–2 Unicode-codepoint gram projection — distinct contiguous 1- and 2-codepoint substrings (Unicode codepoints, not UTF-16 code units) of normalized main_text, keyed by (gram, block_id) for exact gram lookup; terms <3 use indexed gram lookup, terms ≥3 retain trigram FTS; per-term sets intersected; exact regex owns whole-word/CJK matching only; deterministic ordering remains existing fetch order (`sort_order`→`id`); independent from M1 dense sort-order and S6.5 | M2/M3 short-query isolation; ADR if schema/index | §6.7.1 evidence-task contract satisfies parity + plan determinism + directional storage/write/rollback review | Remove auxiliary projection/index; restore LIKE path | **Candidate Defined — Evidence Authorized (decision evidence only); Production — Not Authorized** |
...
#### 6.7.1 S6.4 Short-Query Candidate SQ-01 — Candidate Defined — Evidence Authorized (2026-08-29)

Prior review 2026-08-29 — outcome Need Specific Evidence; S6.4 remained Candidate — Not Authorized (documentation-only; ARCH-001..012 unchanged; no production code/tests/schema/index/migration/IPC/StoreSync/S6.5/sync/startup/retention/threshold/baseline/SLA/`architecture.md` change). M1 confirmed dense-order O(N) sibling shifts; M2 confirmed <3-codepoint `LIKE` full scan with trigram FTS unable to serve <3 but no alternative evaluated; M3 diagnosed current search fetch plan on synthetic 50k corpus — all directional only, harness-fidelity and current-plan attribution without alternative or index benefit. M1 and M2/M3 are independent semantic-risk boundaries; evidence not pooled; any schema/index remains ADR-gated.

SQ-01 candidate shape (finite, production not authorized): derived auxiliary 1–2 Unicode-codepoint gram projection keyed for exact gram lookup — distinct contiguous 1-codepoint and 2-codepoint substrings (Unicode codepoints, not UTF-16 code units) of normalized main_text, keyed by (gram, block_id). Terms length ≥3 retain current trigram FTS candidate path; terms length <3 use indexed exact gram lookup; per-term block_id sets are intersected (AND); existing exact regex remains authority for whole-word/CJK matching only; deterministic ordering remains existing fetch order (`sort_order`→`id`). Main SQLite authority and existing normalization, sort order (`sort_order`→`id`), Unicode/CJK, whole-word, mixed-term AND, cursor, pagination, error propagation, and exact-regex-authority semantics remain unchanged; independent from M1 dense sort-order mutation and S6.5; no pooling.

Authorization boundary: SQ-01 is Candidate Defined — Evidence Authorized only for an isolated disposable production-shaped synthetic comparison with no real user data (neither required nor authorized). Production implementation, migration, index/schema adoption, and ADR review remain Candidate — Not Authorized and remain SQLite migration governance/ADR-gated.

Evidence-task contract (all four fields required for progress):

1) Decision/outcome: decide whether SQ-01 should advance to production design + SQLite migration ADR review, remain candidate, or be rejected.
2) Claim: compared with current short-term LIKE scan, SQ-01 preserves exact ordered search behavior — including Unicode codepoint boundaries (codepoint, not UTF-16), normalization (Markdown stripping, CRLF/CR, lowercasing), whole-word/CJK matching, deterministic fetch order, cursor/pagination, and error behavior/ propagation — while replacing candidate collection with indexed gram lookup; storage/write cost and rollback implications are made explicit, with no threshold claim.
3) Minimum sufficient method: isolated disposable production-shaped synthetic corpus (no real data) — current path vs SQ-01; exact full ordered parity across 1- and 2-codepoint cases (Unicode codepoints, not UTF-16 units) including ASCII, CJK, astral-plane (supplementary-plane/emoji), and mixed Unicode one/two-codepoint sequences; substring and whole-word (regex owns whole-word/CJK matching only; deterministic ordering remains existing fetch order `sort_order`→`id`); mixed short+long terms; escaping/special characters; Markdown stripping, CRLF/CR normalization, and lowercasing parity; empty/not-found; newest/oldest; multi-page cursor with duplicate-free pagination plus malformed-cursor error parity (non-silent, no silent reset/fallback) and database/candidate-source failure parity (non-silent error propagation, same failure semantics as current path); two-run deterministic EXPLAIN classification proving current SCAN and SQ-01 indexed SEARCH for short candidate collection; directional read-stage comparison; backfill plus insert/update/delete write-path cost; logical and physical storage delta; migration/trigger/rollback design review. Existing ≥3 trigram path remains unchanged; whole-word/CJK matching remains exact regex authority only and deterministic ordering remains existing fetch order.
4) Stopping condition: stop after one complete reproducible comparison package satisfies parity and plan determinism and records directional read/write/storage/rollback evidence sufficient for a three-way decision; do not iterate toward a target or implement production.

Next evidence batch may create only isolated harness/prototype artifacts exercising the synthetic comparison; production code, schema, index, migration, and ADR remain unauthorized.
...
|---|---|---|
| **Full-topic/windowed fetch/cache joins** | Data-access contract implementation | Phase 5 contract; S6.1–S6.3 implemented; S6.4 Candidate Defined — Evidence Authorized (production Not Authorized); S6.5 Candidate — Not Authorized |
...
| Data-access contract (R-02..R-06, completeness, ordering, context) | Phase 5 | **Closed 2026-08-29 (outcome/residual-risk)** — R-02..R-06 satisfied by S6.1–S6.3; calibration optional/non-blocking; S6.4 Candidate Defined — Evidence Authorized (production Not Authorized); S6.5 Candidate — Not Authorized |
| Specific index/query optimizations | Phase 6 | **S6.4 SQ-01 Candidate Defined — Evidence Authorized (decision evidence only); Production — Not Authorized** — prior 2026-08-29 Need Specific Evidence rationale preserved in §6.7.1; M1 and M2/M3 independent, no evidence pooling; see §6.7.1 for SQ-01 shape, authorization boundary, and four-field evidence-task contract (isolated synthetic corpus, parity + EXPLAIN SEARCH vs SCAN, storage/write/rollback; no real data; no production/IPC/StoreSync/S6.5/threshold/baseline/SLA/`architecture.md` change; any schema/index remains ADR-gated) |
...
| Data-access implementation (windowed fetch, authority-aware actions, context closure) | Phase 6 | **S6.1–S6.3 Authorized & Implemented**; S6.4 Candidate Defined — Evidence Authorized (production Not Authorized); S6.5 Candidate — Not Authorized |
