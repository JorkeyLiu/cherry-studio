...
export const TopicManagePanel: React.FC<TopicManagePanelProps> = ({
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    // Soft-delete topics (move to trash). Phase 5.2B: all topics go through
    // SQLite (LOCK-521/524). Redux below only removes topics whose mutation
    // succeeded (LOCK-528).
    const results = await Promise.allSettled(
      topicsToDelete.map((topic) => softDeleteOrdinaryTopic(topic.id, topic.name).then(() => topic.id))
    )
