...
> **Last updated**: 2026-09-01 — S7.12 Startup Critical-Path Attribution Implemented & Closed (attribution-only, no production optimization, timing-dominant unknown; S7.13 recommended not activated); M4/M5 synthetic batch accepted (M4 `chatdb-m4-fts-duplication-50k` 50k + M5 `chatdb-m5-file-dual-state` medium, derived; S6.5 Candidate — Not Authorized); S7.1–S7.12 retained. Git owns history; no chained provenance.
...
| **7** | Startup Architecture | **S7.1–S7.12 Implemented & Closed 2026-09-01 (outcome/residual-risk; S7.12 attribution-only); S7.13+ deferred — See §6.8.2–§6.8.13; S7.12 topology-only attribution closed with no production optimization; S7.13 recommended not activated; Phase 7 remains partially Open; no ready-now batch without explicit activation** | Phase 2; Phase 3 for conversation-startup |
...
- **Status**: **S7.1–S7.12 Implemented & Closed 2026-09-01 (outcome/residual-risk; S7.12 attribution-only); S7.13+ deferred** — See §6.8.2–§6.8.13 for per-slice outcomes; S7.12 Startup Critical-Path Attribution closed with no production optimization, timing-dominant unknown (controlled non-adoption). Phase 7 remains partially Open; S7.13+ deferred — production optimization not authorized without explicit activation. Remaining Phase 7 tracks beyond S7.12 (background-window policy, S6.5, Phase 8) remain deferred.
...
- **Dependencies**: Phase 2; Phase 3 stable host for conversation-startup (already satisfied). S7.1–S7.12 all attribution/topology or renderer-local with no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/release/migration/sync/background/telemetry or Markdown/ReactMarkdown/editor KaTeX governance crossing — see §6.8.2–§6.8.13 for per-slice touch boundaries; S7.12 attribution-only, changes no production code/config. S7.13+ deferred.
- **Relationship**: See §8 — S7.1 bundle/activation, S7.2 readiness/hydration, S7.3 Antd locale on-demand, S7.4 translation JSON + Day.js locale, S7.5 XLSX export, S7.6 Home-message KaTeX, S7.7 Mermaid demand-activation, S7.8 code tooling, S7.9 auto-sync, S7.10 maintenance (0ms sweep + retention split), S7.11 critical bootstrap failure isolation, S7.12 attribution-only critical-path topology (see §6.8.2–§6.8.13); conversation lazy activation (S3.5) distinct; S7.13+ deferred (background-window policy, S6.5, Phase 8).
...
**Locked scope**: LOCK-002 (exactly three call-sites), LOCK-003 (static imports, synchronous order, no event-loss window/readiness/delay, no StoreSync service-internal/authority/contract/IPC/persistence), LOCK-004 (no performance/startup/bundle/byte/SLA claim; build proves topology only), LOCK-005 (S7.13+ deferred).
...
**Status**: **Implemented & Closed 2026-08-31 (outcome/residual-risk)** — code changes and focused tests applied; fresh pnpm build exit 0 proving topology and re-audit 0 findings; authoritative pnpm build:check exit 0 closes batch. Phase 7 remains partially Open — S7.13+ deferred after S7.12.
...
**Status**: **Implemented & Closed 2026-09-01 (attribution-only, controlled non-adoption)** — docs-only; no production optimization authorized. Phase 7 remains partially Open; S7.13+ deferred.
...
#### 6.8.14 S7.13 Recommended Next Direction — Startup Stage Instrumentation (Not Activated)

**Status**: **Recommended only — Not Activated**. S7.13 is not authorized for implementation; it exists solely as the rational next startup direction to select one later production optimization after attribution.

**Authorization boundary**: Not activated by this program. No production code/config, baseline, SLA, threshold, authority/contract/schema/migration change authorized until explicit activation.

**Proposed scope (minimal attribution-only slice if activated)**: Default-off/fail-closed startup-stage instrumentation/harness with synthetic disposable profile, privacy-safe bounded stage marks (no content/paths/credentials/raw DB size), isolating renderer blocking gates vs Main chain to identify the timing-dominant bottleneck. No authority/contract/schema/migration change; no baseline/SLA/threshold; docs-only activation requires four-field contract.

**Evidence contract — four fields (S7.13 proposed, not active)**:
- **Named decision/outcome**: S7.13 would be Implemented & Closed only when bounded stage marks identify one timing-dominant production optimization candidate (or explicitly retain non-adoption).
- **Claim**: One startup stage is timing-dominant under synthetic disposable profile with common timeline (directional only; no SLA/baseline/threshold).
- **Minimum sufficient method**: Default-off instrumentation, synthetic disposable profile, privacy-safe bounded marks, common Main+renderer timeline without production hooks beyond fail-closed marks; static `git diff --check` + harness gate.
- **Stopping condition**: Single dominant stage (or explicit unknown) with privacy-safe bounded marks and no prohibited changes; otherwise remain non-adoption.

**Constraint**: S7.13 must not imply activation; recommendation routing follows docs writing policy for partially Open Phase 7 — Phase 7 remains partially Open with S7.13+ deferred until explicit activation.
...
Independent/deferred tracks not blocked by S7.1–S7.12 — S7.1–S7.12 Implemented & Closed 2026-09-01 (outcome/residual-risk; S7.12 attribution-only), S7.13+ deferred:

| Track | Description | Independence | Status |
|---|---|---|---|
| App boot services | Service init order/parallelism (remaining beyond S7.11) | Independent | S7.13+ deferred (not selected without explicit activation) |
...
| Startup stage instrumentation (S7.13) | Minimal default-off/fail-closed synthetic bounded stage marks to select one later optimization; no authority/contract/schema/migration/baseline/SLA | Not activated, attribution-only harness if activated | **S7.13 Recommended — Not Activated (deferred); S7.13+ deferred** |
...
> **S7.12 note (2026-09-01, factual correction + attribution scope)**: `Redux rehydration` corrected to `localStorage` (`redux-persist/lib/storage`, key `persist:cherry-studio`) per `src/renderer/src/store/index.ts:23,312-314` and corroborated by `docs/sqlite-migration.md:114` and `cherry-chat-application-identity.md` (LOCK-COMPAT-003); `persist:cherry-studio` preserved. **S7.12 Implemented & Closed 2026-09-01 — attribution-only, no production optimization, timing-dominant unknown (controlled non-adoption; see §6.8.13)** — static topology only, not runtime timing proof; dev/runtime observation diagnostic only; no baseline/SLA/threshold/regression proof; Dexie ordinary open/upgrade timing unknown; Main restore/SQLite before window, handler/preload gap unresolved; trace/import/search lazy on macOS; PowerMonitor Windows-only out-of-scope; evidence limits per §6.8.13. **S7.13 Recommended — Not Activated** — minimal default-off/fail-closed synthetic bounded stage marks to select one later optimization; no authority/contract/schema/migration/baseline/SLA; S7.13+ deferred.
...
| Startup improvements (S7.1–S7.12) | Phase 7 | **S7.1–S7.12 Implemented & Closed 2026-09-01 (outcome/residual-risk; S7.12 attribution-only, controlled non-adoption) — See §6.8.2–§6.8.13; S7.12 static topology (§6.8.13): PersistGate (`persist:cherry-studio`) then ImportProjectionGate sequential blockers, Dexie module blocking/timing unknown, Main restore/SQLite before window, handler/preload gap unresolved, trace/import/search lazy, PowerMonitor out-of-scope; no IPC/preload/schema/migration/StoreSync/persist-key change; no production optimization/baseline/SLA/threshold/regression; validated docs-only (static topology, `git diff --check`); Phase 7 remains partially Open — S7.13+ deferred (S7.13 Recommended — Not Activated; minimal default-off/fail-closed synthetic bounded marks); no ready-now batch without explicit activation** |
