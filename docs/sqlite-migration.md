...
| **退出条件** | ✅ 普通聊天路径无 Dexie 依赖；✅ Phase 3.4 routing scaffolding 完全移除；✅ 性能不低于 Dexie 基线；✅ 所有现有测试通过；✅ CI 绿色（本地 gates；push/remote CI **Pending**，LOCK-MD8） |
...
| **退出条件** | ✅ Cherry Chat 备份/恢复（沿用现有产品流程、适配 chat.db）独立运作；✅ Cherry Studio ZIP 导入作为一次性操作独立运作；✅ L2/L3 产品语义清晰分离；✅ Group D/E 清理完成；✅ 文档更新；✅ CI 绿色（本地 gates；push/remote CI **Pending**，LOCK-MD8） |
...
> **closure 定位（LOCK-MD1）**：本节为 **Phase 6 交付收尾 / 最终交付证据（closure）**，记录本地交付关闭证据。**不是 Phase 7**：不定义 Phase 7、不重开 Phase 0–6。内容含 A-class/B-class 分开的 E2E 证据、六个交付阻塞项修复、本地 gates/ABI/清理，以及 push/remote CI 的显式 **Pending** 占位。

| 属性 | 值 |
|---|---|
| **状态** | **本地 closure 完成（2026-07-31）**；**push/remote CI：Pending（LOCK-MD8）**——本分支 `jorkey/refactor/sqlite-migration` 无 upstream、origin 分支不存在，push/remote CI 事实未知，E2E 非标准 CI 证据 |
...
| **退出条件** | ✅ 本地 closure 准确记录；✅ push/remote CI 显式 Pending 占位；✅ 未定义 Phase 7、未重开 Phase 0–6 |
...
| **LOCK-MD8** | push/remote CI 事实未知：显式 **Pending** 占位；纠正陈旧 'CI green' 声明；不虚构 push hashes / workflow runs；E2E 非标准 CI 证据 |
