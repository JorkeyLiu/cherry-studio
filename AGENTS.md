...
- [Sync MVP proposal](docs/sync-mvp.md) — first-phase synchronization boundary document: goals/non-goals, data scope, device-local vs cross-device authority, conflict matrix, and explicit open decisions; read it before starting synchronization work.
- [PowerSync spike plan](docs/sync-powersync-spike.md) — disposable, isolated PowerSync feasibility experiment plan: isolation contract, dependency strategy, Go/No-Go gates, evidence classification, and harness disposal rules.
