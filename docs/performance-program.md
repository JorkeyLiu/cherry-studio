...
| **并发度（concurrency）** | 单模型 / 多模型 / 跨话题 | 多模型：PERF-100 答案标签切换（每 group 3 模型、4 次切换，L3 数值）；单流：streaming E2E；**多流并行：PERF-102 测量首切片已验证（Active，N=1/2/3，2026-08-14：fresh 构建 + 三次 focused E2E 退出码均 0、各 12/12 gate、45 有限指标、schema v1、ABI 145，§12/§16 I）——N=1→2→3 呈方向性放大（redux/DOM first-content、input latency、longtask、frame-delta 均放大；commitInterval 基本稳定），归属未知、非阈值/根因归属（§16 I）**；跨话题无资产 | 跨话题并发加载无资产（PERF-102 覆盖多流并行方向；首切片已验证，多流并行的归属诊断待 §16 下一 Main 决策检查点批准） | 单模型/多模型快速确定性；跨话题按需/定时（需多话题 E2E 场景） |
...
| PERF-101 测量 spec | `perf101-topic-switch-measurement.spec.ts`（生产构建 E2E，`PERF101_SCALE` 六个取值） | 缓存未命中话题切换 click-to-first-useful-render + loadCommit（对角线验证通过 2026-08-14，§16 G；正交验证通过 2026-08-14，§16 H——六点部分网格 N/W 已可分离） | 300+ 与 profile 级话题/受控内容复杂度**按需未动**（LOCK-007，§16）；L4 大话题/复杂内容秒级报告未复现（§14 R14）；校准待 PERF-002 |
...
| R15 | PERF-102 首切片 L3 数值（§7/§16 I）被误读为阈值/基线，或把 N 方向性放大当作根因归属 | 方向失真 / 根因错配 / 验收无据 | **已识别（2026-08-14）**：§7/§16 I 显式 **L3 暂定**标注——N=1→2→3 的方向性放大（redux/DOM firstContent、input latency、longtask、frame-delta 均放大；commitInterval 基本稳定）**归属未知**，**无阈值、无运行时修复、无根因归属**（PERF-LOCK-003；唯一已提交阈值仍为冷开 `<500ms`）；任何诊断/修复方向须先经 §16 下一 Main 决策检查点批准，决策前不启动实现/验证 |
...
**当前 Active 工作流**：**PERF-102（P1，并行窄流可视流畅度）——2026-08-14 用户显式授权为唯一 Active**（§3：Active 同一层级唯一；§12）。测量优先首切片已实现（`tests/e2e/specs/conversation/perf102-multimodel-stream-measurement.spec.ts`）、Main 审计修正已应用、静态验证通过；**fresh 生产构建 + n1/n2/n3 三次 focused E2E 运行时验证全部通过（2026-08-14，§16 I）**——每次退出码 0、12/12 正确性 gate、45 有限指标、strict schema-v1、Electron ABI 145、git `9a218432da` dirty=true、样本 3、forbidden scan 干净；三个 artifacts 保留为 gitignored 本地 deliverable（§16 I）。**L3 暂定数值（§7 新行）**：N=1→2→3 呈**方向性放大**（redux/DOM firstContent、input latency、longtask、frame-delta 放大；DOM commitInterval 基本稳定）——**归属未知、无阈值、无运行时修复、无根因归属**。**n1 首轮样本 0 失败（2026-08-14，input-latency probe 两行值等于生产 Inputbar TextArea `autoSize.minRows=2`——测试不变量缺陷，非生产/流健康问题）已修正并静态复验，仅作实现验证历史保留、不是测量**。**下一单元 = Main 决策检查点（非执行，§16 下一单元）**：批准有界 PERF-102 诊断切片（基于 N 放大观察，推荐）或关闭（仅 L3 证据）或暂停；决策前不开始实现/验证。PERF-100 与 PERF-101 均已 **Done**（§12）；PERF-002（Planned）与 PERF-004（Planned 提案，多维矩阵方向）保持 Planned，范围未批准。激活来自显式授权，PERF-100/101 关闭**不自动激活**任何工作流。
...
**下一单元（Main 决策检查点，非执行）——PERF-102 首切片测量已完成并验证，Main/用户决定后续方向**：

PERF-102 测量优先首切片已完成并验证（2026-08-14，§16 I）：fresh 生产构建 + n1/n2/n3 三次 focused E2E 全部通过（各 12/12 正确性 gate、45 有限指标、schema v1、Electron ABI 145），L3 首切片数值已回写 §7（新行）/§8.1/§11/§12/§14。**本单元不执行任何实现/验证/测量**——仅由 Main/用户基于现有 L3 证据做出方向决策；决策前不开始任何实现/验证。

**决策依据（现有证据，全部为 L3 暂定）**：N=1→2→3 下可见流 first-content（redux 362.0→1275.6 ms、DOM 410.7→1301.3 ms，约 3.2–3.5x）、input latency（p50 9.7→16.5 ms、n3 p95 86.6 ms）、longtask（count 18→39、max 130→412 ms）、frame-delta 尾端（max 132.7→420.2 ms）均呈**方向性放大**，而 **DOM commitInterval 基本稳定**（p50 152.6→152.8 ms，与 150ms 流式 cadence 一致）。**归属未知**：未分离并发 fanout 成本、渲染竞争、调度延迟或测量负载；**无阈值、无运行时修复、无根因归属**（PERF-LOCK-003）。**Main 建议：诊断延续（推荐）**——N=3 呈现方向性放大、方向一致且非噪声形态，值得做一次有界诊断切片；但**归属未知，不得预设根因**。三项路径：

- **路径 A（推荐）：批准有界 PERF-102 诊断切片**——基于 N 放大观察的**测量/诊断优先**切片（延续首切片模式：production-build E2E 或受控诊断，扩展现有测量资产以分离并发 fanout/渲染竞争/调度因素；**不预先限定修复方案**）；范围、规模点、指标与验收由 Main 在授权时界定；L1 gate / L3 数值；**无阈值、无运行时修复**；授权后回写 §12/§16 为新的执行单元。
- **路径 B：关闭 PERF-102（仅 L3 证据）**——以首切片 L3 方向性证据作为关闭依据（同 PERF-101「关闭 101」模式）：**无阈值、无根因归属、不声称 L4 并行窄流流畅度报告已证伪**；标记 Done、释放 Active 槽位；**不自动激活**任何工作流。
- **路径 C：暂停**——PERF-102 保持 Active、记录恢复点（§3 `Paused` 语义按 §16 交接段记录），后续经显式授权恢复。

**成功条件（本决策单元）**：仅记录决策与状态回写；不产生新的测量/实现；L3 数值不被升级为阈值/基线；不做根因归属；不授权/描述任何运行时修复；决策后按 §15 回写 §12/§16/§17。

**非目标（不得做）**：

- **不改变生产运行时行为**；不新增 Main IPC 持久化计时；不给 mock 增加逐 chunk 时间戳；不定义任何新阈值（唯一已提交阈值仍为冷开 `<500ms`）；不触碰 PERF-101 状态（保持 Done）与既有 PERF-100/101 规格与 artifacts；不使用用户/profile 数据。
- **本决策单元不运行任何仓库 gate**（`pnpm lint` / `pnpm test` / `pnpm format` / `pnpm build` / E2E 均不重跑——首切片验证证据已记录于 §16 I；docs-only 写回不进入任何 E2E 输入，PERF-LOCK-005；如后续批准执行路径 A，再按 §15 运行相应 gate）。
...
**诚实性声明**：本交接只记录已完成证据（A–I 全部完成并验证，2026-08-14）。C/E、2026-08-13 数值、F 的 post-fix answer 数值、**G 的 PERF-101 对角线数值、H 的正交数值与 I 的 PERF-102 首切片数值均为 L3 暂定**——不是阈值、不是基线，未提升（PERF-LOCK-003）。F 的 focused 验证数字（实现会话报告）与独立审计 + fresh build/三次 E2E 的集成证据按 §6 分层记录。**最终交付 gate（2026-08-14，代码/测试精确表面）已由独立 Validation 会话完成并通过**：固定工具链 Node 24.11.1 / pnpm 10.27.0；`pnpm format` 退出码 0——对 code/test/config 零改动，仅 gitignored `test-results/.last-run.json`；**PERF-101 测量 spec sha256 前后一致 `56f63649ce18baa1362f247b8bd7a34f701fa2ac128906425e0498fa57780d38`**（验证期间 spec 字节未变）；`pnpm build:check` 退出码 0——嵌套 lint（oxlint 0 errors/76 baseline warnings、eslint 0 errors、node/web/aiCore typecheck 退出码 0、i18n pass、format clean）、openapi:check、full `pnpm test`（Node lane ABI 137）全部退出码 0——**full test 9,492 passed / 75 skipped / 0 failed，428 files**；Node lane 后 Electron ABI 145 已恢复且只读 SQL probe 通过（更早一次分套件数字——main 1775 pass/72 skip、renderer 1726 + 53 pass、aiCore 4152 pass、shared 380 pass、scripts 494 pass、e2e-utils 242 pass/3 skip——完整记录于 §17 2026-08-14 PERF-100 关闭行）。**证据保鲜推理（freshness）**：先前 fresh-build PERF-101 对角线/正交 E2E（§16 G/H）保持 fresh——**spec 字节未变（sha256 前后一致）且 docs 位于 E2E 输入之外**；本次最终写回为 **docs-only**、不改代码/测试表面、不进入 E2E 输入，最终 gate 证据对关闭提交保持权威（PERF-LOCK-005）。**交付 provenance（稳定来源）**：测量 artifacts 均在**父提交 HEAD `162b9f1e34` dirty=true** 上产生（artifact git 元数据即记录该 dirty 来源）；本次交付 = **活性文档写回 + PERF-101 测量 spec**，**经用户显式授权用于关闭提交（closure commit）**；确切提交内容以 git 历史为准（**不预先断言提交 hash**）；**未推送/未开 PR 仅为本次写回时点的交接状态，非永久不变量**。**PERF-102 首切片已由独立验证完成并通过（2026-08-14，§16 I）**——fresh 生产构建 + n1/n2/n3 三次 focused E2E 退出码均 0、各 12/12 gate、45 有限指标、schema v1、Electron ABI 145；L3 数值已回写 §7 新行；下一单元为 **Main 决策检查点（非执行，§16 下一单元）**，决策后如需执行路径 A（有界诊断切片）再按 §15 运行相应 gate。Q5 保持有界关闭（历史事件为执行上下文销毁，非已证明崩溃/重载；H7 未证明）。**PERF-101 已按用户显式决策关闭（Done，2026-08-14；测量子切片——对角线 §16 G + 正交 §16 H——完成并验证；基于当前证据无运行时修复需要/授权；不声称 L4 秒级报告已证伪，§14 R14 保留）**；**PERF-102 已由用户显式授权为唯一 Active（2026-08-14，§12）**，测量优先首切片已实现、Main 审计修正已应用、静态验证通过；**fresh 生产构建 + n1/n2/n3 三次 focused E2E 运行时验证全部通过（2026-08-14，§16 I）——L3 首切片数值已回写 §7 新行，呈方向性放大、归属未知、无阈值、无运行时修复、无根因归属**；**n1 首轮样本 0 失败（两行 probe 值等于生产 autoSize minRows=2 的测试不变量缺陷，非生产/流健康问题）仅作实现验证历史保留、不是测量**（§16/§17）；PERF-002/004 保持 Planned；激活来自显式授权，PERF-100/101 关闭不自动激活任何工作流；**下一单元 = Main 决策检查点（非执行，§16 下一单元）**。

**参考**：§9（基线 0）、§10.1/§10.2（L4 用户报告 + 静态调查，方向判断）、§7（暂定预算，L3 数值；含 F post-fix answer 新行、**G PERF-101 对角线新行与 H 正交新行**）、§8（多维规模矩阵；话题消息数与可见消息数行含对角线 + 正交六点、§8.2 标注六点网格与有效 min(N,W)）、§5.1（结果契约）、§11（harness 清单/资产保留；PERF-101 spec 行——`PERF101_SCALE` 六个取值）、§14（R1 部分缓解、R3/R4 关闭、R9 已分诊、R10 已满足、Q5 已关闭、R11 保留、**R12 已满足**、**R13 已缓解（正交数据）**、**R14 已识别（L4 大话题/复杂内容未复现）**、**R15 已识别（PERF-102 L3 数值被误读为阈值/根因归属）**、Q6）、测量 spec（`tests/e2e/specs/conversation/perf100-measurement.spec.ts`，`PERF100_SCALE` 环境变量）、**PERF-101 测量 spec（`tests/e2e/specs/conversation/perf101-topic-switch-measurement.spec.ts`，`PERF101_SCALE` 环境变量：unset/empty=quick、`s0-20`、`s0-100`、`n20-w10`、`n100-w10`、`n100-w50`；缓存未命中话题切换测量资产；**对角线验证通过 2026-08-14，§16 G；正交验证通过 2026-08-14，§16 H**）**、**PERF-102 测量 spec（`tests/e2e/specs/conversation/perf102-multimodel-stream-measurement.spec.ts`，`PERF102_SCALE` 环境变量：unset/empty=n1、`n1`、`n2`、`n3`；并发多模型流放大测量资产；**运行时验证通过 2026-08-14，§16 I：fresh 构建 + n1/n2/n3 三次 focused E2E 退出码均 0、各 12/12 gate、45 有限指标、schema v1、ABI 145——L3 数值已回写 §7 新行**）**、2026-08-14 实现行与关闭行（§17，F/G/H/I 完整细节）、artifact 身份与 checksums（§16 F/G/H/I，gitignored 本地 deliverable，**路径可能不持久**——见保留警示）、**下一单元（Main 决策检查点，非执行）**：**批准有界 PERF-102 诊断切片（基于 N 放大观察，推荐）或关闭 PERF-102（仅 L3 证据）或暂停**——PERF-102（唯一 Active，2026-08-14 用户显式授权，§12）首切片已完成并验证（§16 I）；L3 首切片数值呈方向性放大（redux/DOM firstContent、input latency、longtask、frame-delta 放大；DOM commitInterval 基本稳定）、**归属未知、无阈值、无运行时修复、无根因归属**；决策前不开始实现/验证；PERF-101 扩展保持 Planned/非 Active。
