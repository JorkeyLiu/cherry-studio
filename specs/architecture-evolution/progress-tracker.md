...
- **Phase 7 Startup — S7.1 + S7.2 + S7.3 Implemented & Closed 2026-08-30 (outcome/residual-risk).** S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized loading + tagged recovery. S7.2 renderer-local startup readiness loading (`Spin` `aria-busy`/`aria-label`, `data-testid="startup-readiness-loading"`) vs error (`Alert` `role=alert` + retry `Button` `data-testid="startup-readiness-retry"`, `startup.readiness.*` i18n with non-English placeholders replaced) with `failed→pending→ready/failed` retry rerunning captured dispatch→flush→ack without re-notifying `ReduxStoreReady`, subscription cleanup; plus B-08 staleness optimization (synchronous `takeRecords()` draining + snapshot-skip fast path, max 500 live Ranges unchanged, unconditional cross-chunk DOM rescan, `childList/subtree/characterData/attributes` coverage preserved); preserves LOCK-PROJECTION/LOCK-001 gating and LOCK-003 independence; no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/context-window/S7.1 change. S7.3 renderer-local Antd locale on-demand loading (12 locales: `zh-CN`/`zh-TW`/`en-US`/`de-DE`/`ru-RU`/`ja-JP`/`el-GR`/`es-ES`/`fr-FR`/`pt-PT`/`ro-RO`/`vi-VN` with normalized fallback, `antdLocaleLoaders` 12 explicit literal per-locale `import('antd/locale/...')` loaders with distinct emitted locale chunks + `antdLocaleCache`/stale-guard/default fallback; no dayjs/translation JSON, Inputbar, Main startup reorder, telemetry/retention/deletion, persistence, IPC, authority, identity, release, platform, or sync change). Evidenced by focused Vitest (readiness/gate/retry + B-08 staleness) + S7.3 54 locale tests (loader mapping/fallback + provider resolution/caching/stale-guard/error-fallback) + production build 12 per-locale dynamic chunks with eager 12-locale payload removed + independent audits (S7.2 pass; S7.3 2 blockers corrected → re-audit 0 findings) + authoritative `pnpm build:check` exit 0 in 15m11s on exact final worktree (renderer 339 files/4908 tests, all Main/main-native/main-heavy/aiCore/shared/scripts/e2e-utils pass; lint/typecheck/i18n/format/OpenAPI pass; `git diff --check` pass; Electron ABI 145 SQL probe pass; first aggregate run `consistent-type-imports` fail corrected with erased `Locale` import, single authorized rerun passed). Accepted residual risks: S7.2 transient loading flash + B-08 heuristic not full DOM diff; S7.3 cold first mount may briefly use Antd default locale until chunk resolves, failed request has no same-language in-place retry (recovery by language change/remount), directional chunk evidence not timing/SLA/formal threshold; retry best-effort if underlying resource unavailable; PERF workstreams independent/open. Phase 7 remains partially Open — S7.4+ deferred; no ready-now batch exists unless current user instruction explicitly activates another one. Rollback one semantic revert(s), no migration.
...
- **S7.1 + S7.2 + S7.3 Implemented & Closed 2026-08-30 (outcome/residual-risk) — canonical §6.8.2–§6.8.4:** S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized loading + tagged recovery; S7.2 renderer-local readiness loading/error/retry UX (`failed→pending→ready/failed`, subscription cleanup, `startup.readiness.*` i18n) + B-08 synchronous `takeRecords()` draining (max 500 live Ranges, unconditional cross-chunk DOM rescan, `childList/subtree/characterData/attributes` preserved, LOCK-PROJECTION/LOCK-001 and LOCK-003 intact); S7.3 renderer-local Antd locale on-demand loading (12 locales as 12 explicit literal per-locale `import('antd/locale/...')` loaders with distinct emitted locale chunks, `antdLocaleCache`/stale-guard/default fallback, eager 12-locale payload removed; no dayjs/translation JSON, Inputbar, Main reorder, telemetry/retention/deletion, persistence, IPC, authority, identity, release, platform, or sync change); renderer-only, no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync change; focused Vitest (S7.2 readiness/gate/retry + B-08) + S7.3 54 locale tests + production per-locale chunks + independent audits (S7.3 2 blockers → 0, re-audit pass) + authoritative `pnpm build:check` exit 0 in 15m11s (renderer 339/4908, all projects pass; lint/typecheck/i18n/format/OpenAPI pass; `git diff --check` pass; Electron ABI 145 SQL probe pass; first run `consistent-type-imports` fail corrected with erased `Locale` import, single authorized rerun passed); no threshold/baseline/SLA/formal measurement; ARCH-010 disposed; Phase 7 remains partially Open (S7.4+ deferred); no ready-now batch exists unless current user instruction explicitly activates another one; rollback one semantic revert(s), no migration.
...
- **None — no ready-now batch exists unless current user instruction explicitly activates another one.** S7.1+S7.2+S7.3 Implemented & Closed 2026-08-30 (outcome/residual-risk); S7.4+ remain Open/deferred per canonical §6.8; Phase 7 remains partially Open. Any future batch requires separate explicit authorization; SQ-01 may not be silently revived.
...
- Phase 7 S7.4+ and Phase 8 sync implementation — deferred (S7.1/S7.2/S7.3 are the only closed Phase 7 slices; no Main reorder, Inputbar, telemetry, or retention/deletion authorized; Antd locale splitting is Implemented & Closed via S7.3); Phase 8 requires governance activation and remains vendor-neutral.
