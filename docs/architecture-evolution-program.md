...
B-06 viewport bound, B-07 scroll snapshot cap/TTL/LRU, B-08 ContentSearch bound, and B-09 closure retention are implemented renderer-local mechanisms per §6.5.6; B-01..B-05 are approved initial renderer-local retention policy defaults, production implementation pending, not empirically optimal, not baselines/SLA; calibration harness optional, non-blocking; B-10/B-11 are observed preserved bounds. B-01..B-05 values below are the approved defaults — not yet enforced and not a threshold/baseline.

| # | Bounded subject | Target bound | Applies to | Enforcement | Note |
|---|---|---|---|---|---|
| B-01 | Inactive resident projections | max 8 topics | Evictable | Admission + LRU | Approved default; calibration optional; not empirically optimal |
| B-02 | Aggregate logical retained payload | max 32 MiB (canonical frame `phase4-logical-payload-v1`) | Evictable | Evict until ≤32 MiB | Approved default; heap ratio recorded separately; not a baseline/SLA |
| B-03 | Idle TTL | 30 min idle | Each evictable topic | TTL sweep | Approved default; lastAccess = activation or unpin |
| B-04 | Eviction policy | LRU | Evictable | On B-01/B-02 pressure | Approved default; recency = last activation/unpin, tie-break topic ID |
| B-05 | Single oversized topic | non-admissible after deactivation if >32 MiB alone | Any topic >32 MiB | Evict immediately when not pinned | Approved default; prevents one topic occupying budget |
...
Conditional observability targets (not phase gate unless bound to explicitly activated decision/outcome or formal quantitative/reusable claim): resident topic count (evictable vs pinned), logical bytes per-topic and aggregate vs 32 MiB, heap-amplification ratio, working-set payload, hit/miss by reason, completeness markers/generation, eviction events (TTL/oversized/LRU/deletion), TTL sweep execution, viewport groups/anchor stability, scroll snapshot counts, ContentSearch handles/descriptors, context-closure cache hit/miss. All local, no network export. Phase 4 exit remains outcome/residual-risk based; B-01/B-02 calibration is optional/non-blocking and does not block closure unless bound to explicitly activated decision/outcome or formal quantitative/reusable claim; calibration would be measurement-only directional before any production adoption. Implemented locally as bounded scalar-only: resident entry/completeness/generation via pure adapter plus read-path hit/miss reason, staged latency summaries (success+failure), and discarded publication attempts (post-stage validation only; staged fetch failure before validation is staged failure, not discarded) via renderer-local counters wired from `loadTopicMessagesThunk` and composed into Phase 4 snapshot/bound scalars; no IDs/content/paths/credentials/histories, no persistence/IPC/StoreSync; B-01..B-05 approved defaults pending implementation.

#### 6.5.9 Acceptance disposition

Phase 4 exit remains Open — outcome/residual-risk based per product-first model. Controlled regression and protected boundaries remain mandatory (§10.1). Lifecycle foundation (renderer-local registry + staged joint publication) and B-06..B-09 implemented renderer-local mechanisms per §6.5.5–§6.5.6 are evidenced by focused regression and integration snapshot tests (bounded scalar-only diagnostics; no B-01..B-05 enforcement). B-01..B-05 are approved initial renderer-local retention policy defaults, implementation pending, not empirically optimal, not baselines/SLA; calibration harness optional/non-blocking; no production retention/eviction/TTL/LRU adopted. Exit requires accepted outcomes + boundary-matched regression + governance/delivery validation + explicitly accepted residual risk — not harness completeness.
...
- **B-01..B-05** calibration harness is implemented, inactive by default, optional/non-blocking; B-01..B-05 are approved initial defaults pending implementation and the harness is measurement-only directional — not thresholds/baselines/capacity/eviction adoption. Execution provenance absent until run.
...
| Bounded caches | B-06..B-09 implemented renderer-local per §6.5.5–§6.5.6 (B-08 disposable; B-07 device-local Keyv; B-06/B-09 renderer-local, no persistence/schema change); evidenced via integration snapshot + exercised-workload validation (scalar-only, privacy-safe; no data retention); B-01..B-05 are approved initial renderer-local retention policy defaults, implementation pending, not empirically optimal/baselines/SLA, calibration optional/non-blocking, conditional observability for pinned working-set payload (optional, not a phase gate unless bound to an explicitly activated decision/outcome or formal quantitative/reusable claim); Phase 4 exit remains Open | Phase 4 / §6.5 design, exit Open |
...
| Cache invalidation rules and bounds | Phase 4 | **Design complete; exit Open** — B-06..B-09 implemented renderer-local per §6.5.5–§6.5.6; B-01..B-05 are approved initial defaults pending implementation, not thresholds/baselines/SLA, calibration optional/non-blocking; C-01/C-02/pinned directional only; integration snapshot + exercised-workload validation evidenced (scalar-only, privacy-safe); Phase 4 exit remains Open |
