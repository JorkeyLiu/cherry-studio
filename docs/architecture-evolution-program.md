...
> **Last updated**: 2026-09-01 — M4/M5 synthetic directional evidence batch accepted (M4 `chatdb-m4-fts-duplication-50k` 50k logical duplication + M5 `chatdb-m5-file-dual-state` medium — one 1,000-reference dataset, 4 scenarios, derived classification; schema v1, synthetic directional L3; S6.5 remains Candidate — Not Authorized); prior S6.3 closure + C-02 heap diagnostic + S7.1–S7.11 Implemented & Closed retained. Prior history in Git; no chained provenance.
