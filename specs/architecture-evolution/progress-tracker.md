...
- **Next evidence-enabled batch (Requires decision — not Ready now):** M4 extended storage-duplication diagnostics — real corpora, physical DB size, and post-measurement governance/ADR — each requires explicit Main/user activation; **1k, default 10k, and explicitly activated 50k synthetic profiles completed 2026-08-28 via `pnpm bench:m4-fts-dup` (exit 0, directional only; S6.5 remains Candidate — Not Authorized; exact metrics belong only in `performance-measurement.md` §6 and summary in `performance-workstreams.md` §2.4)** — no schema/query/storage/IPC change authorized.
...
- `S6.4` index/query production changes and `S6.5` file dual-state / FTS dedup changes — each needs slice evidence plus ADR/governance; **M4 storage-duplication volume diagnostic (`pnpm bench:m4-fts-dup` with `M4_FTS_DUP_BENCH=1`, bounded 10k default) — harness inactive by default for future invocations; 1k, default 10k, and explicitly activated 50k synthetic profiles executed 2026-08-28 (exit 0, directional only; S6.5 remains Candidate — Not Authorized; exact metrics belong only in `performance-measurement.md` §6 and summary in `performance-workstreams.md` §2.4)** — remaining real-corpus, physical-size measurement, and production inference still require explicit Main/user activation and governance.
