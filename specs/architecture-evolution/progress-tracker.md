...
- **Phase 7 Startup — S7.1 + S7.2 + S7.3 + S7.4 + S7.5 + S7.6 Implemented & Closed 2026-08-31 (outcome/residual-risk).** S7.6 renderer-local Home-message KaTeX atomic on-demand loading (literal dynamic imports, complete atomic chain, parser-compatible fenced-math detection, shared pending/cache/retry, bounded per-block failure with fallback/source Markdown, streaming/race/plugin preserved) alongside S7.5 XLSX export on-demand (literal dynamic import in `xlsxLoader` with private success/pending cache, concurrent deduplication, retry after rejection, typed named/default normalization, export/cancel/write/error preserved), S7.1 lazy secondary routes, S7.2 readiness/retry gating, S7.3 Antd locale splitting, and S7.4 translation JSON + Day.js locale on-demand (12 translation + 11 Day.js locale chunks, atomic activation, explicit literal imports); S7.5 alongside S7.4, S7.6 alongside S7.5; renderer-local only, no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/governance change and no MarkdownEditor/RichEditor/Main startup reorder/Redux/Dexie/SQLite/background windows/telemetry/retention/deletion/S6.5/Phase 8 expansion beyond Home-message KaTeX. Verified via directional split evidence (deferred `xlsx-DusDZK-x.js` with heavy XLSX markers absent from renderer entry, directional only, plus fresh build entry with literal dynamic import links and deferred KaTeX JS/CSS/contrib/parser assets, directional topology only), focused final suites 56 tests (S7.5) and 155 tests passed in final relevant command for S7.6 (loader/fallback/mhchem covered by prior 148-test passing evidence within total focused evolution), independent audits with no production blocker (findings corrected), and authoritative `pnpm build:check` exit 0, 524 files passed |1 skipped, 9599 tests passed |75 skipped (arithmetically correct sum), all lint/typecheck/i18n/format/OpenAPI projects pass plus `git diff --check` pass and Electron ABI 145 SQL probe pass; E2E not run for isolated renderer-local KaTeX/XLSX loading with no IPC/persistence/lifecycle; see canonical §6.8.6–§6.8.7 for durable outcome, selection rationale, and residual risks. Residual risks: first export/math render waits for deferred assets; conservative detector may over-trigger but preserves parser-supported forms and does not reinterpret content; at most 2 attempts per block then reset on block/qualification transition; load failure via fallback/source Markdown and logger path; no E2E/live observation; topology not quantitative; MarkdownEditor/RichEditor KaTeX not included; S7.1–S7.5 residuals unchanged; PERF workstreams independent/open. Phase 7 remains partially Open — S7.7+ deferred; no ready-now batch exists unless current user instruction explicitly activates another one; rollback one semantic revert(s), no migration.
...
- **None — no ready-now batch exists unless current user instruction explicitly activates another one.** S7.1+S7.2+S7.3+S7.4+S7.5+S7.6 Implemented & Closed 2026-08-31 (outcome/residual-risk); S7.7+ remain Open/deferred per canonical §6.8–§6.8.7; Phase 7 remains partially Open. Any future batch requires separate explicit authorization; SQ-01 may not be silently revived.
...
- Phase 7 S7.7+ and Phase 8 sync implementation — deferred (S7.1/S7.2/S7.3/S7.4/S7.5/S7.6 are the closed Phase 7 slices; no MarkdownEditor/RichEditor KaTeX/Inputbar/Main startup reorder/Redux/Dexie/SQLite/background windows/telemetry/retention/deletion/S6.5/Phase 8 expansion beyond Home-message KaTeX; XLSX on-demand is Implemented & Closed via S7.5 and KaTeX via S7.6 alongside S7.4 translation + Day.js locale on-demand and S7.3 Antd locale splitting; no ready-now batch); Phase 8 requires governance activation and remains vendor-neutral.
