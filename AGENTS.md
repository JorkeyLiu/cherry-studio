...
- **Identity, release, and platform meaning are governed by ADRs, not by local edits.** The updater/release freeze (no publish feeds, release workflows, tags, release notes, or updater endpoints) and the macOS-arm64-first platform scope (Windows/Linux identity work is deferred; make no platform promises) are locked by the [Application Identity ADR](docs/adr/cherry-chat-application-identity.md).
...
- **Exercise technical judgment; escalate product direction**: For non-product technical work, independently make the best decision based on the repository’s established product semantics, architecture, conventions, and applicable governance. Promptly involve the user when a decision would define, alter, or materially affect user value or behavior, feature scope or priority, user-facing trade-offs, product policy, compatibility promises, or any governance matter not already decided.
...
- `pnpm build:check` is the sole authoritative aggregate pre-commit gate for any code-surface change. **Do not run** `pnpm format`, `pnpm lint`, and `pnpm test` separately before `pnpm build:check` — `build:check` already runs lint + openapi:check + full test for the exact worktree state. Run `build:check` **once** per exact worktree state; a passing `build:check` is sufficient aggregate evidence for that state and proves its nested gates, so do not immediately repeat those nested gates.
- `pnpm verify:changed` is renderer-only local feedback, **never a completion gate, never a substitute for `pnpm build:check`, and never CI proof**. It fails closed (nonzero + instruction to run `pnpm build:check`) on any Main/preload/shared/package/config/scripts/unknown path; no partial green result.
...
The stack is Electron main + React/TypeScript renderer, SQLite in the main process, IndexedDB in the renderer, and an AI-SDK-based aiCore; exact versions and technology detail are in [docs/architecture/architecture.md](docs/architecture/architecture.md).
...
- **Projection, configuration, and compatibility state are not interchangeable authority.** The renderer legitimately owns configuration and projection state (Redux) and live non-chat data (Dexie); message-block and topic structures exist for message-block UI and legacy/import compatibility, not as ordinary runtime chat authority. The line is current ownership vs. legacy/import compatibility: state that exists to display or to import does not decide. Treating a projection as authoritative is a class of bug, not a style choice. The context-window anchor (`contextWindowAnchor`) is ordinary renderer assistant-settings persistence, governed by the [Context window governance](docs/adr/context-window.md) — it is configuration, never a chat authority.
- **Persisted state and imports carry migration semantics.** Both SQLite and Dexie schemas are versioned, and the L2 Cherry Studio ZIP import is a verification-and-atomic-promotion pipeline, not a blind copy. Changing an authority boundary means changing a contract and requires appropriate governance review.
...
- [docs/architecture/architecture.md](docs/architecture/architecture.md) — detailed architecture reference: main services, renderer directories, Redux slices, AI Core layering, database detail, IPC, multi-window, tracing, tech stack, source compatibility.
- [Application Identity ADR](docs/adr/cherry-chat-application-identity.md) — canonical governance for Cherry Chat identity, compatibility boundary, updater/release freeze, and platform scope.
- [Context window governance](docs/adr/context-window.md) — canonical governance for the stable topic context anchor, `contextCount` semantics, allowed anchor transitions, compatibility repair, and persistence boundary; read it before changing context-window behavior.
- [Projection completeness and authority intents](docs/adr/projection-completeness-authority.md) — canonical governance for loaded-projection semantics, typed completeness capabilities, stable-ID navigation/mutation, caller-local complete reads, and request-local execution overlay; read it before changing chat data-access, windowed reads, or authority mutations.
- [Model Metadata Governance](docs/adr/model-metadata.md) — canonical governance for model-centric reference (canonical identity plus canonical-lab reference serving), user-defined connections, enrichment-only, upstream-final, cache boundary; read it before changing provider/model resolution or metadata caching.
- [Personal multi-device sync](docs/work/multi-device-sync.md) — **active work document (personal-hosted relay; operation-log + thin HTTP relay with automatic online convergence and cursor-based recovery as target; limited validation, not production-ready; completed documents archive to docs/archived/; conditional fallbacks only)**: goal, current approach, established evidence, gap to target, next decision, temporary working judgments, evidence pointers, fallback activation condition; target semantics owned by the sync connection & channel ADR and the sync data convergence ADR below; read this before any sync work.
- [Sync connection & channel ADR](docs/adr/sync-connection-channel.md) — approved target for relay service connection/registration/channel/pairing semantics (`SYNC-CC-*`); no implementation authorized.
- [Sync data convergence ADR](docs/adr/sync-data-convergence.md) — approved target for complete-synchronization semantics over the syncable personal-data domain (`SYNC-DATA-*`); no implementation authorized.
- [tests/e2e/README.md](tests/e2e/README.md) — Playwright E2E standards and fixture usage.
- Archived material (historical/read-only, not current governance) lives in `docs/archived/` as flat files — e.g., `archived/architecture-evolution-program.md`, `archived/sqlite-migration.md`, `archived/performance-measurement.md`, `archived/sync-architecture-selection.md` — no category subdirectories; `docs/work/` is likewise flat with the single active `work/multi-device-sync.md`.
