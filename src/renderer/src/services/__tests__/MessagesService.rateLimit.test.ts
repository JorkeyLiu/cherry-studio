...
const { activityMock, providerMock } = vi.hoisted(() => ({
  activityMock: vi.fn(),
  providerMock: vi.fn()
}))
...
vi.mock('@renderer/hooks/useTopic', () => ({ getTopicById: vi.fn() }))
...
vi.mock('@renderer/store', () => ({ default: { getState: vi.fn(() => ({})), dispatch: vi.fn() } }))
...
import { checkRateLimit } from '../MessagesService'
