...
> **Last updated**: 2026-09-02 — S7.14 Startup Optimization Assessment and Finalization Closed via controlled non-adoption; Phase 7 Closed 2026-09-02 (outcome/residual-risk; S7.1–S7.13 retained Implemented & Closed, S7.12 attribution-only, S7.13 attribution-only instrumentation bounded privacy-safe, S7.14-E1 first-data attribution directional retained not upgraded — no production optimization, baseline, SLA, threshold, winner, or measurement infrastructure change; residual runtime-evidence risk closed 2026-09-02 via fresh enabled Electron runtime E2E — `STARTUP_STAGE_ATTR=1 pnpm build` exit 0 then `STARTUP_STAGE_ATTR=1 STARTUP_STAGE_SYNTHETIC=1 pnpm test:e2e -- tests/e2e/specs/startup/startup-stage-instrumentation.spec.ts` exit 0, 1 passed; user-approved close “了解。关闭。”); M4/M5 synthetic batch accepted 2026-09-01 (M4 `chatdb-m4-fts-duplication-50k` 50k + M5 `chatdb-m5-file-dual-state` medium, derived; S6.5 Candidate — Not Authorized) + 2026-09-02 extensions — S7.14-E1 first-data attribution (renderer.firstData, renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional; no winner/threshold) and S6.5-E1 M4 synthetic physical-page/file proxy (four raw metrics, sanitized failure paths, synthetic isolated; no S6.5 authorization); S7.1–S7.13 retained Closed. Git owns history; no chained provenance.
...
| **6** | DB-Health Implementation | **S6.1–S6.3 Authorized & Implemented; S6.4 SQ-01 Rejected 2026-08-29 — no ADR, no production implementation, current LIKE retained; S6.5 Candidate — Not Authorized; 2026-09-02 M4 physical proxy extension (S6.5-E1, synthetic isolated, four raw metrics, no S6.5 authorization) — no ready-now DB-health production batch** | Phase 5 contract |
...
- **Status**: **Closed 2026-09-02 (outcome/residual-risk) — S7.1–S7.13 Implemented & Closed (S7.12 attribution-only, S7.13 attribution-only instrumentation, bounded privacy-safe, no production optimization/baseline/SLA/threshold; S7.14-E1 first-data attribution evidence integrated 2026-09-02 directional, no winner/threshold; see §6.8.15); S7.14 Startup Optimization Assessment and Finalization Closed 2026-09-02 via controlled non-adoption (user-approved “了解。关闭。”; no demonstrably high-return winner at current first-screen architecture, see §6.8.16); Phase 7 Closed** — See §6.8.2–§6.8.16 for per-slice outcomes; S7.12 and S7.13 remain attribution-only closed with no production optimization; S7.14 finalization adopts controlled non-adoption per ARCH-009 and §10.3 outcome/residual-risk closure (no winner/threshold, no production optimization, no measurement infrastructure change).
...
- **Dependencies**: Phase 2; Phase 3 stable host for conversation-startup (already satisfied). S7.1–S7.13 all attribution/topology, renderer-local, or attribution-only instrumentation with no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/release/migration/sync/background/telemetry or Markdown/ReactMarkdown/editor KaTeX governance crossing — see §6.8.2–§6.8.16 for per-slice touch boundaries; S7.12 attribution-only, changes no production code/config; S7.13 instrumentation-only, bounded privacy-safe, no authority/contract/schema/migration/StoreSync/retention change; S7.14-E1 renderer-owned behavior plus shared diagnostic stage-union extension (no IPC/preload/shared application contract change), no authority, compatibility, SQLite schema/migration/index/trigger, StoreSync, identity, sync, release, or platform change; S7.14 assessment-and-finalization docs-only, no production/measurement change. Phase 7 Closed via S7.14; no S7.14+ deferred track — future startup work requires new explicit PERF/governance activation.
- **Relationship**: See §8 — S7.1 bundle/activation, S7.2 readiness/hydration, S7.3 Antd locale on-demand, S7.4 translation JSON + Day.js locale, S7.5 XLSX export, S7.6 Home-message KaTeX, S7.7 Mermaid demand-activation, S7.8 code tooling, S7.9 auto-sync, S7.10 maintenance (0ms sweep + retention split), S7.11 critical bootstrap failure isolation, S7.12 attribution-only critical-path topology (see §6.8.2–§6.8.13), S7.13 startup stage instrumentation (see §6.8.14), S7.14-E1 first-data attribution (see §6.8.15), S7.14 assessment and finalization via controlled non-adoption (see §6.8.16); conversation lazy activation (S3.5) distinct; Phase 7 Closed.
...
**Status**: **Implemented & Closed 2026-08-31 (outcome/residual-risk)** — code changes and focused tests applied; fresh pnpm build exit 0 proving topology and re-audit 0 findings; authoritative pnpm build:check exit 0 closes batch. Phase 7 remained partially Open at S7.11 closure (S7.14+ deferred after S7.13 at that time); now Phase 7 Closed via S7.14 §6.8.16.
...
**Status**: **Implemented & Closed 2026-09-01 (attribution-only, controlled non-adoption)** — docs-only; no production optimization authorized. Phase 7 remained partially Open at S7.12 closure (S7.14+ deferred after S7.13 at that time); now Phase 7 Closed via S7.14 §6.8.16.
...
**Residual risks (accepted at S7.12 time)**: Startup timing bottleneck remained unknown until S7.13 instrumentation; static topology may diverge from production wall-clock; Main-before-window ordering proven only topologically, not timed; handler/preload/window gap remained open; Dexie open/upgrade and projection timing remained unmeasured; Windows PowerMonitor not evaluated; no threshold/baseline/SLA; Phase 7 remained partially Open at that time (now Closed via S7.14 §6.8.16).
...
**Status**: **Implemented & Closed 2026-09-02 (attribution-only instrumentation, outcome/residual-risk)** — attribution-only harness; no production optimization, baseline, SLA, threshold, or winner selected. Phase 7 remained partially Open at S7.13 closure (S7.14+ deferred at that time); now Phase 7 Closed via S7.14 §6.8.16.
...
- **Follow-up enabled Electron runtime verification (2026-09-02, residual enabled-E2E risk closed)**: fresh `STARTUP_STAGE_ATTR=1 pnpm build` exit 0 (instrumentation build purpose) then `STARTUP_STAGE_ATTR=1 STARTUP_STAGE_SYNTHETIC=1 pnpm test:e2e -- tests/e2e/specs/startup/startup-stage-instrumentation.spec.ts` exit 0, 1 passed, not skipped — verified Main/renderer `__CHERRY_STARTUP_STAGE_VALIDATED=1` markers, enabled state, bounded/privacy-safe closed records (9 Main + 4 renderer), epoch comparability, Main stages (`main.restore`/`cleanupExtractions`/`promotionGate`/`catalogRecovery`/`chatDbInit`/`orphanRecovery`/`createWindow`/`windowReady`/`registerIpc`), and renderer `importProjectionReady` → `ordinaryTreeReady` milestone ordering via existing disposable profile fixture; E2E harness change limited to callback arity fix in `tests/e2e/specs/startup/startup-stage-instrumentation.spec.ts`; independent audit passed. Prior residual risk (no fresh enabled E2E) is now closed; S7.13 remains Implemented & Closed without starting S7.14+ optimization. No baseline, SLA, threshold, winner, or production optimization claimed. Harness remains fail-closed and inert on ordinary `pnpm test:e2e`; enabled run remains synthetic disposable only.
...
**Residual risks (accepted at S7.13 time)**: Enabled path requires both build and runtime gates plus exact disposable validation; ordinary builds/profile stay inert (zero overhead beyond `performance.now()` anchors and dynamic-import guards; enabled path verified 2026-09-02 via fresh `STARTUP_STAGE_ATTR=1 pnpm build` exit 0 + enabled E2E `STARTUP_STAGE_ATTR=1 STARTUP_STAGE_SYNTHETIC=1 pnpm test:e2e -- tests/e2e/specs/startup/startup-stage-instrumentation.spec.ts` exit 0, 1 passed — disposable profile fixture, harness callback arity fix only, audit passed) and the 9 Main + 4 renderer bounded timeline is observable via bounded privacy-safe closed records; however omitted Dexie open, handler/preload, and first-data boundaries meant no timing-dominant winner could be ranked from evidence at that time — S7.13 remained attribution-only with no winner/baseline/SLA/threshold/production optimization and S7.14 remained deferred at that time (now Phase 7 Closed via S7.14 §6.8.16 for that and subsequent gaps). Renderer enablement best-effort lexical owned-root proof without `userDataDir` — Main exact validation is authoritative. Overflow never triggered (13 stages < 32 cap). First-data/Dexie milestone intentionally omitted — no safe boundary exists without scope expansion, remains intentionally unknown at that time; handler/preload ordering gap likewise unresolved. No threshold/baseline/SLA; Phase 7 remained partially Open at that time.

#### 6.8.15 S7.14-E1 First Ordinary Active-Topic Data Settlement Attribution (2026-09-02, renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional)

**Authorization**: S7.14-E1 explicitly authorized as renderer-owned attribution-only isolated slice (renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change) 2026-09-02; no S7.14 implementation/closure, no production optimization, baseline, SLA, threshold, winner, or authority/schema/migration/StoreSync change authorized. S7.1–S7.13 remain Implemented & Closed and are not reopened.

**Selection rationale**: S7.13 instrumentation closed with intentionally omitted first-data/Dexie milestones (no safe boundary without scope expansion) and unresolved handler/preload gap — timing-dominant production root cause still unknown. S7.14-E1 selected to attribute the first ordinary active-topic fetchMessagesWindow settlement interval after ordinaryTreeReady as renderer-local one-shot bounded numeric-only evidence, without inferring eligibility from cross-process state.

**Scope — Must do (bounded, renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change)**:
- Extend shared diagnostic stage union with `renderer.firstData` in `packages/shared/diagnostics/startupStage.ts` (shared diagnostic stage-union extension only; no IPC/preload/shared application contract change); `RendererStartupDiagnostics` adds one-shot interval from `ordinaryTreeReady` perf timestamp to settlement (`markFirstDataSettlement`/`instrumentFirstDataWindow`/`canRecordFirstDataNow`/`consumeFirstDataWindow`/`hasOrdinaryTreeReady`, `ordinaryTreeReadyPerfMs` + `firstDataRecorded` guarded, bounded finite non-negative `durationMs`/`elapsedMs`/`epochMs`, idempotent per stage, fail-closed when disabled/no-ready/already-recorded, bounded scalar `status`/`reason` ≤64 no `/` `\`, 32-record cap, comparable anchors, no content/paths/IDs/credentials/raw DB size).
- `loadTopicMessagesThunk` in `src/renderer/src/store/thunk/messageThunk.ts` instruments first eligible active-topic window settlement after `ordinaryTreeReady` as elapsed interval, one-shot numeric-only, default-off fail-closed. Eligibility before dispatch via topic existence (`assistants.assistants[].topics`) + pre-dispatch active-topic equality (`messages.currentTopicId` trimmed) — independent of `setCurrentTopicId` self-assignment; ineligible first candidate (invalid/missing/cache-hit/non-active topic) consumes window via `consumeFirstDataWindow` fail-closed so later History/navigation/force loads cannot emit. Eligible candidate arms `instrumentFirstDataWindow(windowPromise, isStillEligible)` where settlement-time predicate mirrors thunk's stale validation (current-topic trim equality, latest `loadTopicMessagesRequestSeq` supersede, deletion generation via `isDeletionStale`, applicability generation) — discarded settlements (stale/no-topic/topic-moved/deletion/generation/superseded) are not attributed and remain fail-closed; success → `ok`, rejection → `error`; exactly one `renderer.firstData` record.

**Scope — Must preserve**: startup order, Main SQLite ordinary-chat authority, StoreSync/Dexie/import projection semantics, all IPC/preload/persistence/schema/migration/StoreSync boundaries, S7.13 9 Main + 4 renderer bounded timeline; `persist:cherry-studio` preserved; no Dexie open forcing.

**Scope — Must not**: change IPC/preload/shared application contract (shared diagnostic stage-union extension only; no application contract change), persistence/schema/migration, StoreSync, Dexie open, `persist:cherry-studio` key, baseline/SLA/threshold/winner, or production optimization; no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/release/platform change; no governance crossing.

**Locked scope (product vocabulary)**: S7.1–S7.13 remain closed and are not reopened; renderer-owned behavior plus shared diagnostic stage-union extension (no IPC/preload/shared application contract change), synthetic isolated measurement-only, no production optimization/schema/migration/index/trigger/authority/compatibility/threshold/baseline/StoreSync/identity/sync/release/platform change; four-field evidence contract preserved, results directional, residual unknowns remain.

**Evidence contract — four fields (S7.14-E1)**:
- **Named decision/outcome**: S7.14-E1 first ordinary active-topic data settlement attribution is accepted as renderer-local directional decision evidence only when bounded privacy-safe closed `renderer.firstData` records are produced under default-off fail-closed gating; outcome does not close S7.14, does not select a winner, and does not authorize production optimization, threshold, baseline, SLA, or contract/schema change.
- **Claim**: `renderer.firstData` elapsed interval from `ordinaryTreeReady` to first eligible active-topic `fetchMessagesWindow` settlement is bounded numeric-only, one-shot, default-off fail-closed, with stale/eligibility gating (active-topic existence + pre-dispatch equality, first ineligible/cache-hit/non-active consumes window without attribution, stale/no-topic/deletion/generation/superseded discarded at settlement without attribution, success `ok` / rejection `error`, exactly one record, ordered after `ordinaryTreeReady`).
- **Minimum sufficient method**: Code changes in `packages/shared/diagnostics/startupStage.ts` + `src/renderer/src/services/startupStageDiagnostics.ts` + `src/renderer/src/store/thunk/messageThunk.ts` plus closed-union test and isolated harness reuse; focused Vitest via canonical Node ABI lane — `src/renderer/src/services/__tests__/startupFirstData.test.ts` (default-off inert, fail-closed without ready, success/error one-shot, reset, lifecycle helpers, stale predicate discard, 9 suites) + `src/renderer/src/store/thunk/__tests__/messageThunk.firstData.test.ts` (active-topic success/error, invalid/missing/no-topic/cache-hit/non-active consume, before-ready fail-closed, stale/deletion/generation/superseded discard — 14 suites, real dispatch mutation, no mock leakage) with stale-race correction; file-scoped lint/typecheck/format, `git diff --check`; independent audits (first-data stale race corrected, final sharded audits passed with zero blockers); `pnpm build` topology and authoritative `pnpm build:check` close the batch.
- **Stopping condition**: Focused tests pass true/isolated (all active/stale/eligibility suites + no unhandled rejection/leakage), file-scoped checks pass, diagnostic remains default-off fail-closed when disabled/no-ready/already-recorded, ineligible first candidate consumes window, stale settlements not attributed, audit 0 findings, `pnpm build:check` exit 0 on exact worktree.

**Status**: **Evidence integrated 2026-09-02 (renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional; S7.14 not implemented/closed at that time)** — code changes and focused tests applied (first-data stale race corrected, focused tests pass); independent audit passed (final sharded audit 0 findings); `git diff --check` pass. Phase 7 remained partially Open at S7.14-E1 integration time (S7.14+ deferred at that time); now Phase 7 Closed via S7.14 §6.8.16.

**Outcome (attribution, semantic boundary)**:
- **Closed union extension**: `RENDERER_STARTUP_STAGES` now includes `renderer.firstData` (`STARTUP_STAGE_VALUES` 9 Main + 5 renderer, 14 total < 32 cap); prior 13 stages preserved.
- **One-shot interval**: `ordinaryTreeReadyPerfMs` captured from same `performance.now()` as milestone; `markFirstDataSettlement` records `renderer.firstData` interval `now - ordinaryTreeReadyPerfMs` with bounded scalar `status`/`reason`; `firstDataRecorded` guard ensures exactly one record; `resetStartupState` clears state in `VITEST` seam only.
- **Eligibility gating**: `eligibleBefore` requires `hasOrdinaryTreeReady && canRecordFirstDataNow && topicId.trim().length>0 && topicExistsBefore && activeBefore`; `windowOpenBefore && !eligibleBefore` ⇒ `consumeFirstDataWindow()` so later History/navigation/force cannot emit; cache-hit paths (`cachedIds.length>0` or `residentTopic` hit) also consume when eligibleBefore.
- **Settlement instrumentation**: `instrumentFirstDataWindow(windowPromise, isStillEligible)` attaches `then(resolve→ok,rejection→error)` with predicate checking `currentTopicId` trim equality, request-seq supersede, deletion generation, applicability generation — mirrors thunk's established stale validation; discarded settlements remain fail-closed without `firstDataRecorded`.
- **Preserved**: `persist:cherry-studio`; Main SQLite ordinary-chat authority; StoreSync/Dexie/import projection; all boundaries; S7.13 behavior unchanged; no IPC/preload/schema/migration/StoreSync.
- **Changed-file surface (semantic-risk boundary)**: shared: `packages/shared/diagnostics/startupStage.ts`, `packages/shared/diagnostics/__tests__/startupStage.test.ts`; renderer: `src/renderer/src/services/startupStageDiagnostics.ts`, `src/renderer/src/store/thunk/messageThunk.ts`; tests: `src/renderer/src/services/__tests__/startupFirstData.test.ts`, `src/renderer/src/store/thunk/__tests__/messageThunk.firstData.test.ts` (renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, bounded numeric-only, one-shot, privacy-safe, default-off fail-closed; no handler/preload/Dexie change).
- **No winner/baseline/SLA/threshold/production optimization/contract change** — attribution-only isolated evidence to select one later optimization after synthesis; S7.14 remained deferred at that S7.14-E1 integration time (now Phase 7 Closed via S7.14 controlled non-adoption §6.8.16).

**Verification / evidence limits**:
- Focused Vitest via canonical Node ABI lane covering default-off inert, fail-closed without ready, success/error one-shot, reset, lifecycle helpers, stale predicate discard (9 suites) + thunk-level comprehensive (active success/error, invalid/missing/no-topic/cache-hit/non-active consume, before-ready, stale/topic-moved/deletion/generation/superseded, 14 suites) — first-data stale race corrected and verified; `git diff --check` pass, file-scoped lint/typecheck/format pass.
- Independent audits passed with no findings after correction (stale race fix + final sharded audit 0 findings); no IPC/preload/shared application contract change (shared diagnostic stage-union extension only), no authority, compatibility, schema/migration/index/trigger, StoreSync, identity, sync, release, or platform change.
- `pnpm build` and authoritative `pnpm build:check` verification is separate; this batch is validated by focused verification + audit; no ready-now production batch follows.
- No winner, baseline, SLA, threshold, or production optimization established; handler/preload ordering gap, Dexie open/upgrade timing, and first-data real-corpus/physical incidence remain unresolved directional unknowns.

**Residual risks (accepted at S7.14-E1 time)**: Enabled path requires build + runtime gate plus S7.13 exact disposable validation; ordinary builds/profile stay inert (default-off). `renderer.firstData` is renderer-local attribution-only, one-shot bounded interval, not a timing-dominant winner or baseline/SLA; stale/moved/deletion/generation/superseded settlements are correctly discarded but consume eligibility only via pre-dispatch or cache-hit consumption, not via stale predicate discard — next eligible records correctly. First-data real-corpus/physical cost/incidence/threshold, Dexie open/upgrade timing, and handler/preload ordering remained unresolved — S7.14-E1 remained directional only and S7.14 remained not implemented/closed at that time (now S7.14 Closed via controlled non-adoption §6.8.16 for that and subsequent gaps). Renderer predicate is best-effort without cross-process active-topic contract — fail-closed rather than inferring. No threshold/baseline/SLA; Phase 7 remained partially Open at that time.

#### 6.8.16 S7.14 Startup Optimization Assessment and Finalization — Controlled Non-Adoption (2026-09-02) — Phase 7 Closed

**Authorization**: S7.14 explicitly authorized as assessment-and-finalization to close Phase 7 via controlled non-adoption, per user approval “了解。关闭。” 2026-09-02; no production code, tests, config, or measurement infrastructure change authorized; S7.1–S7.13 remain Implemented & Closed and are not reopened; S7.14-E1 remains directional evidence not upgraded to quantitative.

**Selection rationale / assessment**: First-screen startup path (bootstrap → persistRehydrate → importProjectionReady → ordinaryTreeReady → first eligible active-topic fetchMessagesWindow settlement) re-reviewed at implemented state: existing renderer-local optimizations cover lazy activation (S3.5 ContentSearch/EditMode; S7.1 five secondary routes lazy with Home eager and bounded fallback), Antd/translation/Day.js locale on-demand (S7.3/S7.4 distinct chunks, atomic activation, stale guards), heavy payload on-demand (S7.5 XLSX, S7.6 KaTeX atomic, S7.7 Mermaid demand, S7.8 Shiki/CodeMirror lazy), auto-sync demand at 8s (S7.9), 0ms maintenance (S7.10), bootstrap failure isolation (S7.11), attribution (S7.12 static topology, S7.13 stage instrumentation, S7.14-E1 firstData one-shot), plus data-access efficiencies (R-02 latest / R-03 around with coverage-checked cache joins, resident-topic completeness hit, parallel requests, generation/applicability stale guards). Re-review found bounded window, resident hit, and parallel/stale-guard paths already implemented; no further first-screen structural split shows demonstrably high-return user-perceivable benefit at current evidence sufficient to keep Phase 7 open.

**Candidate examined — not adopted**: Messages with user-visible auxiliary segment projection/metadata joint publication is a coupled `Promise.all` (`messages` + `segments`) in `loadTopicMessagesThunk`. Conceptually could be staged (messages first, segments second) but would require new two-phase residency semantics and new invalidation rules (partial-topic residency vs `whole-topic`, staged generation validation, segment-only miss handling). Benefit is unproven as user-perceivable (user-visible auxiliary segment projection/metadata — not required for first-batch message-body rendering, no timing winner ranked by S7.13/S7.14-E1 attribution; S7.14-E1 remains directional), and scope expansion would create new lifecycle/semantics debt. Therefore not implemented as S7.14 production optimization; left as potential future candidate only if explicit symptom triggers activation. Segments not claimed as proven bottleneck.

**Scope — Must do (bounded, docs-only)**: Record controlled non-adoption assessment, close S7.14 and Phase 7 as outcome/residual-risk closure per ARCH-009 and §10.3; no production code/tests/config/measurement change.

**Scope — Must preserve**: S7.1–S7.13 Implemented & Closed not reopened; S7.13 9 Main + 4 renderer bounded timeline and S7.14-E1 `renderer.firstData` directional evidence not upgraded to quantitative; `persist:cherry-studio` and all authority/compatibility/IPC/persistence/schema/migration/StoreSync/Dexie semantics preserved.

**Scope — Must not**: Select a timing winner, declare performance improvement/baseline/SLA/threshold/real-corpus conclusion, change production code/tests/config/measurement infrastructure, or permanently prohibit future optimization (future activation via explicit PERF/governance only).

**Locked scope (product vocabulary)**: S7.1–S7.13 remain closed not reopened; S7.14 defined as Startup optimization assessment and finalization with controlled non-adoption and Phase 7 Closed; no production optimization/baseline/SLA/threshold/winner or measurement infrastructure change; residual unknowns explicitly accepted.

**Evidence contract — four fields (S7.14 finalization)**:
- **Named decision/outcome**: S7.14 Startup Optimization Assessment and Finalization is Closed via controlled non-adoption and Phase 7 is Closed only when assessment records that no demonstrably high-return next optimization warrants keeping Phase 7 open and residual risks are explicitly accepted per §10.3 outcome/residual-risk closure.
- **Claim**: At current first-screen architecture, no startup optimization with demonstrably high-return user-perceivable benefit is identified that justifies continued Phase 7 occupation or winner selection; the sole structural candidate (messages/segments joint publication) remains unproven for user-perceivable gain and would require new two-phase residency/invalidation semantics.
- **Minimum sufficient method**: Docs-only first-screen re-review of implemented optimizations (lazy activation, bounded window, resident hit, parallel requests, stale guards; S7.1–S7.13 + S7.14-E1 directional evidence) plus candidate decomposition analysis of messages/segments coupling limited to structural/semantic cost without new production instrumentation or measurement claim; user approval “了解。关闭。” is activation for this controlled non-adoption.
- **Stopping condition**: Controlled non-adoption recorded consistently in program (§6.8.16) and tracker, Phase 7 status synchronized to Closed, no production code/tests/config/measurement infrastructure changed, `git diff --check` passes, residual risks explicitly accepted, future work gated to explicit PERF/governance activation.

**Status**: **Closed 2026-09-02 via controlled non-adoption (docs-only, outcome/residual-risk)** — no production optimization, baseline, SLA, threshold, winner, or measurement infrastructure change. Phase 7 Closed.

**Outcome (governance, semantic boundary)**:
- **Finalization**: Phase 7 Closed 2026-09-02 via S7.14 controlled non-adoption; user-approved close.
- **Assessment basis**: ARCH-009 (no absolute performance threshold required for progression) and §10.3 outcome/residual-risk closure apply; evidence cost proportionality respects already-implemented optimizations.
- **Non-adoption**: No timing winner selected; segments not claimed as proven bottleneck; no byte-saving/startup SLA/threshold/baseline/real-corpus conclusion.
- **Preserved**: `persist:cherry-studio`; Main SQLite ordinary-chat authority; all IPC/preload/persistence/schema/migration/StoreSync boundaries; S7.13/S7.14-E1 directional evidence retained not upgraded.
- **Changed-file surface (semantic-risk boundary)**: docs-only — `docs/architecture-evolution-program.md`, `specs/architecture-evolution/progress-tracker.md`; no production code/tests/config/measurement change.
- **No winner/production optimization** — Phase 7 closure is controlled non-adoption, not proof of optimality.

**Verification / evidence limits**:
- Docs-only finalization; no `pnpm build:check` or new measurement required per docs-only gate policy; `git diff --check` must pass; no spurious current-state `Phase 7 partially Open` / `S7.14+ deferred` / `no ready-now batch` remnants after sync (historical mentions preserved with explicit “at that time” qualification).
- Assessment is directional re-review only, not quantitative timing comparison; S7.13/S7.14-E1 attribution remains directional not upgraded.

**Residual risks (explicitly accepted per §10.3 and user-approved controlled non-adoption)**:
- Handler/preload/window-readiness ordering gap unresolved (S7.12).
- Dexie open/upgrade timing unknown.
- Real-corpus first-data/physical-cost/incidence/threshold and production benefit of messages/segments split remain unknown (S7.14-E1 directional only).
- Messages with user-visible auxiliary segment projection/metadata joint publication coupling remains as observed structure with unproven user-perceivable cost (not required for first-batch message-body rendering); staged split would need new two-phase residency/invalidation semantics if ever activated.
...
| **M4** | FTS/normalized storage-duplication volume diagnostic — no corpus writes after seed; one isolated WAL checkpoint/TRUNCATE normalization step outside timed probe; then read-only numeric-only aggregation (row counts, char/UTF-8 bytes, FTS smoke; logical duplication only, no observed physical DB size) + 2026-09-02 synthetic physical-page/file proxy extension (four raw metrics, `WAL checkpoint TRUNCATE` + `page_count`/`page_size`/`freelist_count`/`stat` isolated DB file bytes, no derived bytes) | None — harness implemented, inactive by default (`M4_FTS_DUP_BENCH=1` + `M4_FTS_DUP_SCALE=1k|10k|50k` default 10k), `m4FtsDuplication.bench.ts` + `m4FtsDuplication.ts` + pure helper tests; isolated `mkdtemp` + `registerChatDbNormalize` + `runMigrations` schema-v1 + existing `generateCorpus`; 1k/10k/50k synthetic profiles are measurement-only directional L3 via `pnpm bench:m4-fts-dup` (no corpus writes after seed; one isolated WAL checkpoint/TRUNCATE normalization step outside timed probe; then read-only metric collection; no threshold/baseline/dedup authorization; S6.5 remains Candidate — Not Authorized; real corpora/physical DB size unresolved; exact metrics in `performance-measurement.md` §6); requires explicitly activated decision/outcome + four fields to count as valid/progress; 2026-09-01 accepted: `chatdb-m4-fts-duplication-50k` 50k synthetic directional L3 (full record in §7.3.2); 2026-09-02 integrated: S6.5-E1 M4 synthetic physical-page/file proxy (four raw metrics, sanitized failure paths, synthetic isolated, directional; full record in §7.3.3; no S6.5 authorization); future runs inactive by default |
...
All are read-only with respect to production/user state (M8 permitted owned temporary backup/restore writes inside isolated `mkdtemp` root only; M4 has no corpus writes after seed; one isolated WAL checkpoint/TRUNCATE normalization step outside timed probe; then read-only metric collection; M5 uses only owned synthetic SQLite rows plus explicit synthetic catalog/physical booleans). They do not require governance/ADR but require explicitly activated decision/outcome with all four evidence-task fields before execution can count as valid/progress (`performance-program.md` §4B); explicit activation alone is insufficient. Harness implementation or synthetic execution is not production authorization and does not authorize S6.5.

**Current DB-health frontier:** S6.4 SQ-01 Rejected 2026-08-29 — no ADR, no production implementation, current LIKE retained; M2/M3 evidence batch closed. M1 sort-order shift (9 combinations), M4 bounded synthetic profiles (1k/10k/50k — 50k 2026-09-01 `chatdb-m4-fts-duplication-50k`) + 2026-09-02 physical-page/file proxy extension (S6.5-E1, four raw metrics `page_count`/`page_size`/`freelist_count`/`dbFileBytes` via WAL checkpoint + stat, sanitized failure paths, synthetic isolated, directional; full record in §7.3.3), M5 bounded synthetic matrix (small 100 refs + medium — one 1,000-reference dataset, 4 scenarios `chatdb-m5-file-dual-state`), M8 L3 archive health, and B-01..B-05 calibration are implemented/executed only as measurement-only directional L3 evidence (synthetic isolated, privacy-safe, directional-only; details in §7.3/§7.3.2/§7.3.3 and `performance-measurement.md` §6; Git owns provenance; B-01..B-05 calibration optional/non-blocking; M1 independent diagnostic; M8 synthetic archive/restore only). No production authority/schema/IPC/persistence change; no threshold/baseline/capacity/eviction/policy, runtime-consistency proof, S6.5 authorization, or Phase 4/5/6 closure follows. Any real-corpus/physical-size diagnostic or production S6.5 work remains ADR/governance-gated; no DB-health production batch ready.
...
- **M4** 1k, 10k, and 50k synthetic directional evidence — via `pnpm bench:m4-fts-dup` (directional only, measurement-only; S6.5 remains Candidate — Not Authorized; real corpora and physical DB size remain unresolved; exact metrics in `performance-measurement.md` §6 and full record in §7.3.2) + 2026-09-02 physical proxy extension (four raw metrics `page_count`/`page_size`/`freelist_count`/`dbFileBytes` via WAL checkpoint + stat, sanitized failure paths, synthetic isolated, directional; full record in §7.3.3; S6.5-E1 not authorized; real-cost/benefit/threshold unresolved). 2026-09-01 accepted: `chatdb-m4-fts-duplication-50k` 50k synthetic directional L3 (logical duplication only). Further M4 real-corpus/physical-size work requires explicitly activated decision/outcome + four fields and governance/ADR; no DB-health production batch is Ready now.
...
### 7.3.3 2026-09-02 S6.5-E1 M4 Synthetic Physical-Page/File Proxy — Four-Field Contract, Provenance, and Decision

**Authorization**: S6.5-E1 explicitly authorized as synthetic isolated measurement-only extension to M4 2026-09-01; no S6.5 production authorization, schema/migration/index/trigger, IPC/preload/shared contract, authority, or threshold/baseline/SLA. S7.14-E1 companion is renderer-local attribution-only. Both are directional only; residual production unknowns remain.

**Scope — Must do (bounded, synthetic isolated only)**:
- Extend `m4FtsDuplication.bench.ts` to perform single `PRAGMA wal_checkpoint(TRUNCATE)` outside timed bench probe, inspect `busy` via `parseCheckpointBusy`/`assertCheckpointComplete` (number/object/array shapes, fail-closed when busy≠0 or unverifiable), then collect exactly four approved raw physical metrics `page_count`/`page_size`/`freelist_count`/`dbFileBytes` (PRAGMA `page_count`/`page_size`/`freelist_count` + `fs.statSync` isolated DB file bytes after checkpoint), build via `buildM4PhysicalMetrics` (finite integer, >0 for `pageCount`/`pageSize`/`dbFileBytes`, exactly four IDs, no derived `pageBytes`/`freelistBytes`, no path in IDs, bounded numeric-only, privacy invariants). Timed bench probe remains read-only (no checkpoint/truncation/stat).
- Sanitized failure paths: `buildM4CheckpointFailureMessage`/`buildM4BusyCheckpointFailureMessage`/`buildM4StatFailureMessage`/`buildM4FtsSmokeFailureDetail` fixed categories (`checkpoint_failed`/`stat_failed`/`unverifiable`/`fts_smoke_failed`) without interpolating native `e.message` that may contain isolated DB path (audit privacy finding); `fts.smoke` gate detail via fixed helper, bounded ≤256 bytes, path-free.
- `benchResult.ts` emission contract updated to permit owned synthetic isolated DB file bytes via approved four-field raw proxy as directional synthetic proxy only (not observed/user DB size).

**Scope — Must preserve**: isolated `mkdtemp` + WAL + `registerChatDbNormalize` + `runMigrations` schema-v1 + deterministic 50k corpus; logical M4 gates (`parity.rowCounts`, `fts.smoke`, `corpus.completeness`, `metrics.finite`); S6.5 Candidate — Not Authorized; no IPC/schema/StoreSync/Main change.

**Scope — Must not**: emit derived bytes, path-containing IDs/messages, observed/user DB size, baseline/threshold/SLA; change production schema/migration/index/trigger; authorize S6.5 production dedup.

**Locked scope (product vocabulary)**: synthetic isolated measurement-only; four-field evidence contract preserved, results directional, residual unknowns remain; no baseline/threshold/phase-closure.

**Evidence contract — four fields (S6.5-E1 M4 physical proxy 2026-09-02)**:
- **Named decision/outcome**: S6.5-E1 M4 physical-page/file proxy is accepted as synthetic directional decision evidence only when four approved raw physical metrics are collected after verifying `WAL checkpoint TRUNCATE` `busy==0`; outcome does not authorize S6.5, nor measure physical cost/benefit, nor establish threshold/baseline, nor authorize schema/index/trigger.
- **Claim**: At 50k `message_blocks`, four raw physical proxies — `page_count` (PRAGMA `page_count`), `page_size` (PRAGMA `page_size`), `freelist_count` (PRAGMA `freelist_count`), isolated synthetic DB file byte size (`stat` after `WAL checkpoint TRUNCATE`) — are numeric-only, bounded, integer, privacy-safe, exactly four IDs, no derived `pageBytes`/`freelistBytes`, no path, collected only after `busy==0` else fail-closed before stat/artifact.
- **Minimum sufficient method**: Owned `mkdtemp` synthetic SQLite (50k blocks) + single `PRAGMA wal_checkpoint(TRUNCATE)` outside timed probe with `parseCheckpointBusy`/`assertCheckpointComplete` (number/object/array, fail-closed on busy or unverifiable) + four PRAGMA/stat collections validated via `buildM4PhysicalMetrics` (finite integer, >0 for `pageCount`/`pageSize`/`dbFileBytes`, no derived, no path, units `bytes` only on `page_size`/`dbFileBytes`); sanitized fixed-category failure messages (no native path interpolation); bench probe remains read-only; focused tests covering parsing/busy, four-metric exactness, no derived, privacy audit, real SQLite integration via `m4PhysicalProxy.test.ts` + `m4FtsDuplication.test.ts`; `git diff --check`; independent audit with sanitized paths; authoritative gate separate.
- **Stopping condition**: All focused tests pass (parsing/busy, four-metric exactness, no derived, path-free privacy audit, real PRAGMA/stat integration) with bounded numeric-only metrics, busy/unverifiable aborts before stat/artifact with no path leakage, audit 0 findings, authoritative gate exit 0; otherwise no artifact.

**Status**: **Evidence integrated 2026-09-02 (synthetic, isolated, directional; S6.5 not authorized)** — harness extension and sanitized helpers applied; focused tests pass; final audit passed (M4 final audit 0 findings, sanitized failure paths verified); no production optimization authorized.

**Outcome (measurement-only, semantic boundary)**:
- **Physical proxy**: Single WAL checkpoint `TRUNCATE` outside timed probe, busy/status inspected fail-closed before any stat/artifact; after `busy==0`, four raw PRAGMA/stat metrics collected as numeric-only bounded integers with correct units; no derived bytes emitted; artifact schema v1 still requires all tinybench tasks + all gates passed with finite non-negative metrics, otherwise fail-closed.
- **Privacy sanitization**: Checkpoint/stat/FTS-smoke failure messages are fixed categories without interpolating native error text that may contain temporary isolated DB path; audit verifies no `/tmp`/`/private`/`chat.db`/`chatdb-m4-fts-dup` leakage even when native error contains macOS/Windows/relative path shapes.
- **Preserved**: owned isolated synthetic DB only, no user data/path/content/credential/observed DB size; no schema/migration/index/trigger, IPC/preload/shared contract, authority, StoreSync, identity, sync, release, platform change; S6.5 remains Candidate — Not Authorized.
- **Changed-file surface (semantic-risk boundary)**: Main: `src/main/services/chatDb/__tests__/m4FtsDuplication.bench.ts`, `src/main/services/chatDb/__tests__/m4FtsDuplication.ts`, `src/main/services/chatDb/__tests__/m4FtsDuplication.test.ts`, `src/main/services/chatDb/__tests__/benchResult.ts`; tests: `src/main/services/chatDb/__tests__/m4PhysicalProxy.test.ts` (synthetic isolated, four raw metrics, fail-closed, privacy-safe; no production code/config).
- **No production authorization** — synthetic isolated proxy only; does not measure physical cost/benefit, real-corpus incidence, or threshold.

**Verification / evidence limits**:
- Focused Vitest via canonical Node ABI lane covering `parseCheckpointBusy` (number/array/object), `assertCheckpointComplete` (busy==0 only), `buildM4PhysicalMetrics` (exactly four raw, no derived, bounded, integer, >0 checks, path-free IDs), privacy audit (fixed categories path-free even when native error contains absolute/relative/Windows paths), real SQLite integration (`page_count`/`page_size`/`freelist_count`/`stat` after checkpoint, busy==0, no path leakage, second stat read-only) — all pass.
- Independent final audit passed with 0 findings after sanitized failure paths; `git diff --check` pass.
- Authoritative gate separate; this batch validated by focused verification + audit; no ready-now production batch follows.
- No threshold/baseline/SLA/capacity/policy/closure; real-corpus/physical-size frontier, physical cost/benefit, incidence, write-amplification remain unresolved directional unknowns; S6.5 remains Candidate — Not Authorized.

**Residual risks (accepted)**: Synthetic isolated 50k corpus on single-machine synthetic profile — not real-corpus/physical DB size, not physical cost/benefit, not write-amplification or threshold; four raw proxies are directional only and do not quantify production storage saving or performance; WAL checkpoint is outside timed probe but still a single checkpoint per bench run; physical file size is after `TRUNCATE` on isolated `mkdtemp` DB only; no production authority/schema/IPC change; no baseline/SLA; Phase 6 partially Open.
...
Independent/deferred tracks not blocked by S7.1–S7.14 — S7.1–S7.13 Implemented & Closed 2026-09-02 (outcome/residual-risk; S7.12 attribution-only, S7.13 attribution-only instrumentation), S7.14-E1 first-data attribution evidence integrated 2026-09-02 (renderer-local, isolated, directional; S7.14 not upgraded at that time), S7.14 Closed 2026-09-02 via controlled non-adoption; Phase 7 Closed (see §6.8.16):

| Track | Description | Independence | Status |
|---|---|---|---|
| App boot services | Service init order/parallelism (remaining beyond S7.11) | Independent | **Phase 7 Closed via S7.14 controlled non-adoption 2026-09-02 — no demonstrably high-return winner at current first-screen architecture; see §6.8.16; future startup work requires explicit PERF/governance activation** |
...
| Startup stage instrumentation (S7.13) | Independent `__STARTUP_STAGE_ATTR__` gate (default-off fail-closed), Main-authoritative exact disposable validation + opaque `__CHERRY_STARTUP_STAGE_VALIDATED` marker, bounded privacy-safe closed records (one per stage/session, no content/paths/credentials/raw DB size), 9 Main + 4 renderer boundaries without changing startup order/authority/IPC/preload/persistence/schema/migration/StoreSync/Dexie-open; renderer `importProjectionReady` → `ordinaryTreeReady` both verified, first-data/Dexie milestone intentionally omitted; no authority/contract/schema/migration/baseline/SLA/winner | Attribution-only instrumentation | **S7.13 Implemented & Closed 2026-09-02 (attribution-only instrumentation, bounded privacy-safe, no production optimization/baseline/SLA/threshold/winner; residual enabled-E2E risk closed 2026-09-02 via fresh `STARTUP_STAGE_ATTR=1 pnpm build` exit 0 + `STARTUP_STAGE_ATTR=1 STARTUP_STAGE_SYNTHETIC=1 pnpm test:e2e -- startup-stage-instrumentation.spec.ts` exit 0, 1 passed); see §6.8.14** |
| First-data attribution (S7.14-E1) | Renderer-owned one-shot `renderer.firstData` interval `ordinaryTreeReady` → first eligible active-topic `fetchMessagesWindow` settlement; closed union 14 stages, bounded numeric-only, default-off fail-closed, stale/eligibility gating (active-before + consumption, stale predicate mirrors thunk validation); renderer-owned behavior plus shared diagnostic stage-union extension (no IPC/preload/shared application contract change), no Dexie/schema/StoreSync | Attribution-only isolated | **Evidence integrated 2026-09-02 (renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional; S7.14 not upgraded at that time, no winner/threshold/baseline; see §6.8.15); now Phase 7 Closed via S7.14 §6.8.16** |
| Startup optimization assessment and finalization (S7.14) | Controlled non-adoption assessment of first-screen startup at implemented state (lazy activation, bounded window, resident hit, parallel requests, stale guards already implemented; messages/segments joint publication candidate examined — would require new two-phase residency/invalidation semantics, unproven user-perceivable benefit, not implemented; no winner/threshold/baseline/SLA/real-corpus claim) | Docs-only, no IPC/schema/StoreSync/measurement | **S7.14 Closed 2026-09-02 via controlled non-adoption (user-approved “了解。关闭。”; Phase 7 Closed; see §6.8.16) — no production code/tests/config/measurement change; accepted residuals: handler/preload ordering, Dexie open/upgrade timing, first-data/physical cost/incidence, messages/segments coupling — future explicit PERF/governance activation only** |
...
| FTS storage dedup | Phase 6 | **Candidate S6.5 — Not Authorized**; bounded synthetic M4 profiles (1k/10k/50k — 50k 2026-09-01 `chatdb-m4-fts-duplication-50k`, synthetic directional L3, logical duplication only) + 2026-09-02 physical proxy (S6.5-E1, four raw metrics `page_count`/`page_size`/`freelist_count`/`dbFileBytes` via WAL checkpoint + stat, sanitized failure paths, synthetic isolated, directional; full record in §7.3.3; S6.5 remains Candidate — Not Authorized) are complete as directional evidence only, but no threshold/baseline/benefit/physical-cost or production authorization follows; real-corpus/physical-size/physical-cost/incidence/threshold remain unresolved (details in §7.3.2/§7.3.3 and `performance-measurement.md` §6); any further diagnostic or production work requires an explicit decision, privacy review where applicable, and governance/ADR |
