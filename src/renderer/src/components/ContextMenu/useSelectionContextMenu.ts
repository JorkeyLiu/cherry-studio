...
export interface SelectionContextMenu {
  /** Menu items for the current text selection (empty when nothing is selected). */
  items: MenuProps['items']
  /** Capture the current text selection when the menu opens. */
  onOpenChange: (open: boolean) => void
}

/**
 * Shared normal-mode selection context menu behavior: when the menu opens,
 * capture the current text selection and offer copy / quote actions.
 * Used by the generic ContextMenu and by the stable MessageContextMenu host
 * in normal (non-edit) mode.
 */
export function useSelectionContextMenu(): SelectionContextMenu {
