...
- [Sync architecture selection](docs/sync-architecture-selection.md) — **canonical S8 selection validation authority (Active; goal-first primary operation-log + thin HTTP relay realtime validation; MVP slice implementation + audited hardening complete — limited validation, manual-only, not production-ready; personal multi-device direction LOCK-PERSONAL-001/004/005/006/007/008/009/010 implemented-but-pending-validation — transactional outbox, SSE hint + reconciliation never data authority, field patches/clocks/conflict log, personal-scope conflict/checkpoint/delete semantics; independent audit/final gate pending, no new final counts; PowerSync / cr-sqlite / Turso / Automerge conditional fallbacks only, no blind parallel comparison; broader S8 remains open as primary-path validation)**: product target + realtime goal (automatic online convergence + existing-cursor recovery after short disconnection; Syncthing-like local-first personal multi-device, no Cherry Chat account, no team collaboration, device pairing, one user-selected service), current-state constraints, balanced four-dimension method, goal-first primary/fallback taxonomy, research findings, goal-conditioned scenario inventory, primary validation plan with conditional fallback spikes, decision rules, and write-amplification replay gate; MVP route `application operation-log + thin HTTP relay` (manual, LWW, additive sync metadata, filtered payloads, reference relay; ordering/validation/tombstone hardening) is implemented; read this before any sync work.
- [Sync MVP proposal](docs/sync-mvp.md) — historical record (Phase 0/1 proposal superseded by `sync-architecture-selection.md`; retained for prior locks/inventory audit).
- [PowerSync Spike Historical Record](docs/sync-powersync-spike.md) — historical experiment record (constraint-specific No-Go preserved as evidence for S8 re-evaluation, not a permanent ban).
