...
- **Phase 6 DB-Health — `S6.1..S6.3` Authorized & Implemented; `S6.4`/`S6.5` Candidate — Not Authorized.** `S6.1` windowed reads (`R-02`/`R-03`), `S6.2` authority-aware actions, `S6.3` context closure and cache joins are implemented. `S6.4`/`S6.5` remain blocked. **M4 storage-duplication volume diagnostic 1k, default 10k, and explicitly activated 50k synthetic profiles executed 2026-08-28 via `pnpm bench:m4-fts-dup` (exit 0, directional evidence only; S6.5 remains Candidate — Not Authorized; real corpora/physical DB size unresolved; no threshold/baseline/dedup authorized; harness inactive by default for future invocations; exact metrics belong only in `performance-measurement.md` §6 and summary in `performance-workstreams.md` §2.4)**.
- **Phase 7 Startup / Phase 8 Future Sync — deferred, Open.** No activation; sync remains vendor-neutral compatibility only.
- **Post-M4 frontier (Requires decision — not Ready now):** All bounded synthetic M4 storage-duplication profiles (1k, 10k, and 50k) are complete as measurement-only directional evidence. Real-corpus evidence and physical database-size evidence remain unresolved. M5 remains unexecuted and unactivated, with no M5 harness. Any further M4 real-corpus/physical-size diagnostic or M5 diagnostic activation requires explicit Main/user activation, an explicit decision, privacy review where applicable, and applicable governance/ADR; no production batch is Ready now and no schema/query/storage/IPC change is authorized.
...
- `S6.4` index/query production changes and `S6.5` file dual-state / FTS dedup changes — each needs slice evidence plus ADR/governance; **all bounded M4 synthetic profiles (1k/10k/50k) are complete as directional evidence, while real-corpus and physical-size evidence remain unresolved** — any further M4 diagnostic, M5 diagnostic activation, or production inference still requires explicit Main/user activation, an explicit decision, privacy review where applicable, and governance/ADR; M5 remains unexecuted/unactivated with no harness, and S6.5 remains Candidate — Not Authorized.
