...
- **Phase 7 Startup — S7.1 + S7.2 + S7.3 + S7.4 Implemented & Closed 2026-08-30 (outcome/residual-risk).** S7.4 renderer-local i18n translation JSON + Day.js locale on-demand loading (12 translation + 11 Day.js locale chunks via explicit literal per-locale dynamic imports, atomic initial/subsequent activation with fallback/stale-guard/callback completion, Inputbar/Home and S7.1/S7.2/S7.3 unchanged, no Markdown/KaTeX/XLSX/Main startup/telemetry/retention/deletion expansion; renderer-local only, no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/governance change) alongside S7.1 lazy secondary routes, S7.2 readiness/retry gating, and S7.3 Antd locale splitting. Verified via directional build topology, focused atomic activation verification, audit pass-with-findings (0 blockers, 1 acceptable coverage risk), and authoritative gates passed (build and `pnpm build:check`); see canonical §6.8.5 for durable outcome, selection rationale, and residual risks. Residual risks: cold initial configured-language fallback, same-language failure no inline retry, IIFE not fully isolated, i18next callback narrow, chunk evidence directional only; S7.1–S7.3 residuals unchanged; retry best-effort; PERF workstreams independent/open. Phase 7 remains partially Open — S7.5+ deferred; no ready-now batch exists unless current user instruction explicitly activates another one; rollback one semantic revert(s), no migration.
...
- **S7.1 + S7.2 + S7.3 + S7.4 Implemented & Closed 2026-08-30 (outcome/residual-risk) — canonical §6.8.2–§6.8.5:** S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized loading + tagged recovery; S7.2 renderer-local readiness loading/error/retry UX + B-08 synchronous `takeRecords()` draining; S7.3 renderer-local Antd locale on-demand (12 locales, per-locale dynamic chunks, cache/stale-guard/fallback); S7.4 renderer-local translation JSON + Day.js locale on-demand (12 translation + 11 Day.js locale chunks, atomic activation, explicit literal imports, stale-guard, fallback); verification per §6.8.5 (directional build topology, focused atomic verification, audit pass-with-findings 0 blockers/1 acceptable coverage risk, authoritative gates passed); no governance crossing; Phase 7 remains partially Open (S7.5+ deferred); no ready-now batch unless explicitly activated; rollback one semantic revert(s), no migration.
...
- **None — no ready-now batch exists unless current user instruction explicitly activates another one.** S7.1+S7.2+S7.3+S7.4 Implemented & Closed 2026-08-30 (outcome/residual-risk); S7.5+ remain Open/deferred per canonical §6.8; Phase 7 remains partially Open. Any future batch requires separate explicit authorization; SQ-01 may not be silently revived.
...
- Phase 7 S7.5+ and Phase 8 sync implementation — deferred (S7.1/S7.2/S7.3/S7.4 are the closed Phase 7 slices; no Main reorder, Inputbar deferred beyond S7.4 scope, Markdown/KaTeX/XLSX/Main startup/telemetry/retention/deletion not expanded; translation + Day.js locale on-demand is Implemented & Closed via S7.4 alongside S7.3 Antd locale splitting); Phase 8 requires governance activation and remains vendor-neutral.
