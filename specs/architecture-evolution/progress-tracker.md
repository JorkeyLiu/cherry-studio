...
- **Phase 6 DB-Health — `S6.1..S6.3` Authorized & Implemented; `S6.4`/`S6.5` Candidate — Not Authorized.** `S6.1` windowed reads (`R-02`/`R-03`), `S6.2` authority-aware actions, `S6.3` context closure and cache joins are implemented. `S6.4`/`S6.5` remain blocked. **M4 storage-duplication volume diagnostic default 10k executed 2026-08-28 via `pnpm bench:m4-fts-dup` (exit 0, directional evidence only; S6.5 remains Candidate — Not Authorized; 1k/50k/real corpora/physical DB size unresolved; no threshold/baseline/dedup authorized; harness inactive by default for future invocations; details in `performance-measurement.md` §6 and `performance-workstreams.md` §2.4)**.
- **Phase 7 Startup / Phase 8 Future Sync — deferred, Open.** No activation; sync remains vendor-neutral compatibility only.
- **Next evidence-enabled batch (Requires decision — not Ready now):** M4 extended storage-duplication diagnostics — 1k/50k synthetic profiles, real corpora, physical DB size, and post-measurement governance/ADR — each requires explicit Main/user activation; **default 10k execution completed 2026-08-28 via `pnpm bench:m4-fts-dup` (exit 0, directional only; S6.5 remains Candidate — Not Authorized; details in `performance-measurement.md` §6 and `performance-workstreams.md` §2.4)** — no schema/query/storage/IPC change authorized.
...
- `S6.4` index/query production changes and `S6.5` file dual-state / FTS dedup changes — each needs slice evidence plus ADR/governance; **M4 storage-duplication volume diagnostic (`pnpm bench:m4-fts-dup` with `M4_FTS_DUP_BENCH=1`, bounded 10k default) — harness inactive by default for future invocations; default 10k executed 2026-08-28 (exit 0, directional only; S6.5 remains Candidate — Not Authorized; details in `performance-measurement.md` §6 and `performance-workstreams.md` §2.4)** — execution of any further scale or production inference still requires explicit Main/user activation and governance.
