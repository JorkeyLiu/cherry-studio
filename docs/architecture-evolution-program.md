...
| **M8** | Backup/restore health — L3 archive metadata (**Planned/Open · design-only, unexecuted · see §7.2 contract; no harness/command registered in this phase**) | None — design-only; future harness if activated is Node `main-native` (ABI 137) per §7.2, not a schema/ADR gate to run design |
...
**Coverage limits**: Single-machine synthetic diagnostics only — one 50k search corpus profile (M2/M3) and one 1k-message runtime profile (M7); no S2-scale, no multi-device, no churn/profile, no M1 insert-shift evidence (**M1 harness absent** noted above); single synthetic profile does not generalize to production workload; heat/heap not covered beyond M7 cold-open/load/write.
...
#### 7.2 Design contract — M8 Backup/restore (L3 archive metadata) health (设计-only · 未执行 · Planned/Open)

> **Scope lock — design and static audit only (LOCK-001)**: This section is **documentation-only design contract** derived from static inspection that is **read-only with respect to production/user state; synthetic owned temporary backup/restore writes inside the isolated `mkdtemp` root are permitted and owned per §7.2-3** (see lane/isolation). **No M8 harness was run, no package command/script/test/bench file was registered or added, no production runtime/schema/migration/IPC/preload/persistence authority/`architecture.md` was modified**. All benchmark ids/commands/env names below are **reserved design names only** for a future activation decision; they do not exist in `package.json` or wiring in this phase.
> **Static evidence inspected (no execution)**: `src/main/services/BackupManager.ts:787-1040` local backup pipeline; `1493-1608` restore extraction/metadata gate; `1634-1775` `restoreDirect` validation/staging and unconditional `app.relaunch`/`app.exit`; `src/main/services/chatDb/l3ArchiveMetadata.ts:21-173` L3 v6/v7 metadata; `src/main/services/chatDb/backup.ts:40-189` `better-sqlite3` backup snapshot + integrity validation; `src/main/services/zipSecurityValidation.ts:96-324` archive containment/limits; `src/main/services/chatDb/__tests__/backupManager.production.test.ts` existing real-FS production tests; `BackupManager.ts:653-671` excluded promotion/WAL artifacts; `src/main/services/chatDb/__tests__/benchResult.ts:47-518` + `docs/performance-measurement.md:37-63` schema v1 / gitignored `test-results/bench-results`; Vitest `main-native` forked for native DB ABI. **LOCK-002 / LOCK-003 / LOCK-004 apply in full — see governance at end of this section.**

**1 — Identity and scope (reserved design names only; no registration in this phase)**
- Stable future benchmark id: `chatdb-m8-l3-archive-health` (to be declared as `benchmark.id` and filename prefix if ever activated).
- Proposed future activation command: `pnpm bench:m8-l3-archive` and env gate `M8_L3_ARCHIVE_BENCH=1` — **design-reserved only; no `package.json` script or env wiring is added in this phase**; `M8_L3_ARCHIVE_BENCH=1` alone is **not** governance authorization.
- Explicit activation means **separate M8 activation decision + env gate** (`M8_L3_ARCHIVE_BENCH=1`); either alone does not authorize execution. This design does **not** activate M8.
...
- **Activation & status**: M8 remains **Planned/Open and unexecuted/unactivated** after this documentation. M8 warrants **directional L3 diagnostic evidence only** (if ever activated). **No package command is registered, no bench/test file is added, no run is claimed** in this phase. Proposed `benchmark.id`/`pnpm bench:m8-l3-archive`/`M8_L3_ARCHIVE_BENCH=1` remain **reserved design names only** — no package wiring or harness exists (§7.2-1/10).
...
- **Explicit non-claims (LOCK-003)**: M8 **cannot** establish backup/restore thresholds, baselines, capacity/defaults, **cannot** close **Phase 4/5/6**, **cannot** close **PERF-DB-HEALTH**, **cannot** authorize **S6.4/S6.5**. **M4/M5/M6 remain separately governance/ADR-gated**. Any future implementation touching **schema/migration/IPC/authority** must **stop and obtain its own governance decision** (see §10.1). M8 is **Planned/Open** in `performance-workstreams.md` §2.4 and `performance-measurement.md` §6 (design-only note); cross-references in those documents point to this §7.2 as the authoritative design contract.

**10 — Audit-required execution clarifications (no new thresholds/levels/fields/APIs; still unactivated)**

- **Per-sample root & no-clobber**: see §7.2-3/5 — `BackupManager` local publish is **no-clobber**, so **warmup=1 + samples=5 each use a fresh per-sample root (preferred) with unique archive path**; per-sample fixture construction and archive/staging cleanup are **outside timed intervals**; cleanup occurs in `finally` after the sample; **no artifact until all samples/gates pass**.
...
- **Scope preservation**: these clarifications **do not introduce new thresholds, scale levels, schema fields, or production APIs**, and **do not authorize activation** — activation still requires separate M8 decision + env gate.
...
- **Exact toolchain required before any future M8 command**: **Node v24.11.1 / pnpm v10.27.0** (pinned `.nvmrc`/`.node-version`/`package.json`; `performance-measurement.md` §1). **Current observed `node v22.23.1` is not acceptable** and must be corrected before execution. This is an **execution prerequisite, not an activation** — toolchain files are **not modified** in this phase (LOCK-001), and `M8_L3_ARCHIVE_BENCH=1` alone does not authorize execution.
