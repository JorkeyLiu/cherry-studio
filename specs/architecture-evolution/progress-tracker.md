...
- **Phase 6 DB-Health — `S6.1..S6.3` Authorized & Implemented; `S6.4`/`S6.5` Candidate — Not Authorized.** `S6.1` windowed reads (`R-02`/`R-03`), `S6.2` authority-aware actions, `S6.3` context closure and cache joins are implemented. `S6.4`/`S6.5` remain blocked. **M4 storage-duplication volume diagnostic harness (`m4FtsDuplication.bench.ts` + `m4FtsDuplication.ts` + `m4FtsDuplication.test.ts`, `M4_FTS_DUP_BENCH=1` / `pnpm bench:m4-fts-dup`, default 10k bounded, `performance-measurement.md` §6) implemented, inactive by default, remains unexecuted; S6.5 remains Candidate — Not Authorized; harness is not execution and does not authorize S6.5.**
- **Phase 7 Startup / Phase 8 Future Sync — deferred, Open.** No activation; sync remains vendor-neutral compatibility only.
- **Next evidence-enabled batch (Requires decision — not Ready now):** M4 storage-duplication volume diagnostic execution (`pnpm bench:m4-fts-dup` with `M4_FTS_DUP_BENCH=1`, optional `M4_FTS_DUP_SCALE=1k|10k|50k` default `10k`) — harness infrastructure exists, remains unexecuted; execution requires explicit Main/user activation; S6.5 remains Candidate — Not Authorized.
...
- `S6.4` index/query production changes and `S6.5` file dual-state / FTS dedup changes — each needs slice evidence plus ADR/governance; **M4 storage-duplication volume diagnostic execution (`pnpm bench:m4-fts-dup` with `M4_FTS_DUP_BENCH=1`, bounded 10k default, `M4_FTS_DUP_SCALE` override) — harness implemented, inactive by default, remains unexecuted; execution requires explicit Main/user activation, S6.5 remains Candidate — Not Authorized, harness is not execution**.
