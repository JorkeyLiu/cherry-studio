...
- **Phase 7 Startup — S7.1 + S7.2 + S7.3 + S7.4 + S7.5 + S7.6 + S7.7 + S7.8 + S7.9 Implemented & Closed 2026-08-31 (outcome/residual-risk); S7.10+ Open (deferred) — S7.9 renderer-local auto-sync tooling demand activation (`src/renderer/src/init.ts` removes BackupService/NutstoreService static activation, literal dynamic imports at existing 8s demand check through proxy chunks, SettingsPage retains static imports, 8s/switch/interval/lastSyncTime/retry/IPC/Redux/settings preserved, no Main/preload/shared/SQLite/Dexie change; bounded outcome directional topology/activation only); S7.1–S7.8 as per canonical §6.8.2–§6.8.9 (renderer-local lazy/demand activation, directional topology/activation only); focused regression, production build topology, independent audits, and authoritative build:check passed; see canonical §6.8.10. Phase 7 remains partially Open — S7.10+ deferred.**
...
- **S7.1 + S7.2 + S7.3 + S7.4 + S7.5 + S7.6 + S7.7 + S7.8 + S7.9 Implemented & Closed 2026-08-31 (outcome/residual-risk) — canonical §6.8.2–§6.8.10:** S7.9 renderer-local auto-sync demand activation (`src/renderer/src/init.ts` static activation removed, literal dynamic imports at 8s through proxy chunks, SettingsPage retains static imports, bounded `loggerService` logging, 8s/switch/interval/lastSyncTime/retry/IPC/Redux/settings preserved, no Main/preload/shared/SQLite/Dexie/StoreSync/identity/migration change; bounded outcome directional topology/activation only); S7.1 five secondary routes lazy, S7.2 readiness loading/error/retry + B-08 draining, S7.3 Antd locale on-demand, S7.4 translation JSON + Day.js locale on-demand, S7.5 XLSX export on-demand, S7.6 KaTeX atomic on-demand, S7.7 Mermaid demand-activation, S7.8 code tooling demand activation — all renderer-local, directional topology/activation only; focused regression, production build topology, independent audits, and authoritative build:check passed; see canonical §6.8.10. No governance crossing; Phase 7 remains partially Open (S7.10+ deferred).
...
- **None — no ready-now batch exists unless current user instruction explicitly activates another one.** S7.1+S7.2+S7.3+S7.4+S7.5+S7.6+S7.7+S7.8+S7.9 Implemented & Closed 2026-08-31 (outcome/residual-risk) per canonical §6.8.2–§6.8.10; S7.10+ remain Open/deferred per canonical §6.8–§6.8.10; Phase 7 remains partially Open. Any future batch beyond S7.9 requires separate explicit authorization; SQ-01 may not be silently revived.
...
- Phase 7 S7.10+ and Phase 8 sync implementation — deferred (S7.1/S7.2/S7.3/S7.4/S7.5/S7.6/S7.7/S7.8/S7.9 are the closed Phase 7 slices; S7.10+ deferred — no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/release/migration/sync/background/telemetry expansion beyond auto-sync demand activation and no Markdown/ReactMarkdown or editor KaTeX Inputbar/Main startup reorder/Redux/Dexie/SQLite/background windows/S6.5/Phase 8 expansion beyond S7.9; Shiki metadata and CodeEditor/CodeMirror via S7.8 alongside Mermaid via S7.7, KaTeX via S7.6 and XLSX via S7.5 alongside S7.4 translation + Day.js locale and S7.3 Antd locale; S7.9 closed; no ready-now batch beyond S7.10+); Phase 8 requires governance activation and remains vendor-neutral.
