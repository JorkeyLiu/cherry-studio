...
- **Phase 7 Startup — S7.1 + S7.2 + S7.3 + S7.4 + S7.5 Implemented & Closed 2026-08-30 (outcome/residual-risk).** S7.5 renderer-local XLSX export on-demand (literal dynamic import in `xlsxLoader` with private success/pending cache, concurrent deduplication, retry after rejection, typed named/default normalization, export/cancel/write/error preserved) alongside S7.1 lazy secondary routes, S7.2 readiness/retry gating, S7.3 Antd locale splitting, and S7.4 translation JSON + Day.js locale on-demand (12 translation + 11 Day.js locale chunks, atomic activation, explicit literal imports); S7.4 alongside S7.3, S7.5 alongside S7.4; renderer-local only, no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/governance change and no Markdown/KaTeX/Inputbar/Main startup reorder/Redux/Dexie/SQLite/background windows/telemetry/retention/deletion/S6.5/Phase 8 expansion. Verified via directional split evidence (deferred `xlsx-DusDZK-x.js` with heavy XLSX markers absent from renderer entry, directional only), focused final suite 56 tests passed, independent audits with no production blocker (findings corrected), and authoritative `pnpm build:check` exit 0, 1018s, 477 test files passed |2 skipped, 9345 tests passed |7 skipped, all lint/typecheck/i18n/format/OpenAPI projects pass plus fresh `pnpm build` exit 0, 26s wall / vite 7.50s, `git diff --check` and Electron ABI 145 probe pass; E2E not run for isolated renderer-local loader/export behavior; see canonical §6.8.6 for durable outcome, selection rationale, and residual risks. Residual risks: first export waits for chunk load; dynamic-load failure via Table error path with later retry; chunk topology not a performance quantification; no E2E/diagnostic observation needed; named/default normalization covered; S7.1–S7.4 residuals unchanged; PERF workstreams independent/open. Phase 7 remains partially Open — S7.6+ deferred; no ready-now batch exists unless current user instruction explicitly activates another one; rollback one semantic revert(s), no migration.
...
- **S7.1 + S7.2 + S7.3 + S7.4 + S7.5 Implemented & Closed 2026-08-30 (outcome/residual-risk) — canonical §6.8.2–§6.8.6:** S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized loading + tagged recovery; S7.2 renderer-local readiness loading/error/retry UX + B-08 synchronous `takeRecords()` draining; S7.3 renderer-local Antd locale on-demand (12 locales, per-locale dynamic chunks, cache/stale-guard/fallback); S7.4 renderer-local translation JSON + Day.js locale on-demand (12 translation + 11 Day.js locale chunks, atomic activation, explicit literal imports, stale-guard, fallback); S7.5 renderer-local XLSX export on-demand (literal dynamic import, private success/pending cache, concurrent deduplication, retry after rejection, typed named/default normalization, export/cancel/write/error preserved); verification per §6.8.6 (directional split `xlsx-DusDZK-x.js` with markers absent from entry, directional only, focused 56 tests, audit no blocker findings corrected, `pnpm build:check` exit 0, 1018s, 477 files |2 skipped, 9345 tests |7 skipped, all projects pass plus fresh build 26s/vite 7.50s, diff-check/ABI pass; E2E not run for isolated XLSX loader/export); no governance crossing; Phase 7 remains partially Open (S7.6+ deferred); no ready-now batch unless explicitly activated; rollback one semantic revert(s), no migration.
...
- **None — no ready-now batch exists unless current user instruction explicitly activates another one.** S7.1+S7.2+S7.3+S7.4+S7.5 Implemented & Closed 2026-08-30 (outcome/residual-risk); S7.6+ remain Open/deferred per canonical §6.8–§6.8.6; Phase 7 remains partially Open. Any future batch requires separate explicit authorization; SQ-01 may not be silently revived.
...
- Phase 7 S7.6+ and Phase 8 sync implementation — deferred (S7.1/S7.2/S7.3/S7.4/S7.5 are the closed Phase 7 slices; no Markdown/KaTeX/Inputbar/Main startup reorder/Redux/Dexie/SQLite/background windows/telemetry/retention/deletion/S6.5/Phase 8 expansion; XLSX on-demand is Implemented & Closed via S7.5 alongside S7.4 translation + Day.js locale on-demand and S7.3 Antd locale splitting; no ready-now batch); Phase 8 requires governance activation and remains vendor-neutral.
