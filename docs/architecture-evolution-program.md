...
**Residual risks (accepted)**: Enabled path requires both build and runtime gates plus exact disposable validation; ordinary builds/profile stay inert (zero overhead beyond `performance.now()` anchors and dynamic-import guards; enabled path verified 2026-09-02 via fresh `STARTUP_STAGE_ATTR=1 pnpm build` exit 0 + enabled E2E `STARTUP_STAGE_ATTR=1 STARTUP_STAGE_SYNTHETIC=1 pnpm test:e2e -- tests/e2e/specs/startup/startup-stage-instrumentation.spec.ts` exit 0, 1 passed — disposable profile fixture, harness callback arity fix only, audit passed). Renderer enablement best-effort lexical owned-root proof without `userDataDir` — Main exact validation is authoritative. Overflow never triggered (13 stages < 32 cap). First-data/Dexie milestone intentionally omitted — no safe boundary exists without scope expansion, remains intentionally unknown. Timing-dominant bottleneck remains unknown until a future analysis of the bounded timeline; S7.13 itself selects no winner/baseline/SLA/threshold/production optimization. S7.13 remains Implemented & Closed (LOCK-001); S7.14+ deferred. No threshold/baseline/SLA; Phase 7 partially Open.
