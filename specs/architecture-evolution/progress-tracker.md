...
- **Phase 6 DB-Health — `S6.1..S6.3` Authorized & Implemented; `S6.4`/`S6.5` Candidate — Not Authorized.** `S6.1` windowed reads, `S6.2` authority-aware actions, `S6.3` context closure and cache joins implemented. **S6.4 reviewed 2026-08-29 — outcome Need Specific Evidence; remains Candidate — Not Authorized** — M1 and M2/M3 are separate candidate semantic-risk boundaries, no pooling; any schema/index ADR-gated; see canonical program §6.7.1 for full rationale and candidate-bound evidence requirements. `S6.5` remains blocked. M4 measurement-only directional (1k/10k/50k via bench:m4-fts-dup); M5 bounded synthetic directional only, inactive by default; M6 harness implemented, not yet executed, analysis-only.
...
- `S6.4` (index/query) **reviewed 2026-08-29 — Need Specific Evidence; remains Candidate — Not Authorized** and `S6.5` (file dual-state / FTS dedup) **Candidate — Not Authorized** — M1 and M2/M3 are separate candidate semantic-risk boundaries, no pooling; any schema/index remains ADR-gated, query-only still requires candidate + parity + improvement; see program §6.7.1 for full rationale and candidate-bound evidence requirements; M5 synthetic does not authorize production, M6 analysis-only; no production/IPC/StoreSync/threshold/baseline/SLA/`architecture.md` change.
...
- No additional cohesive ready-now batch is currently identified; the queue remains empty until the next reconnaissance pass. S6.4 remains Candidate — Not Authorized (reviewed 2026-08-29 — Need Specific Evidence) with no implementation batch ready; S6.5 likewise not ready. (B-01..B-05 renderer-local retention enforced and evidenced by focused regression (Vitest), direct diagnostic UI observation (diagnostic only, not regression proof), independent audit pass, and authoritative `pnpm build:check` pass — no new governance crossing; Phase 4 closed.)
...
- `S6.4` index/query — **reviewed 2026-08-29, Need Specific Evidence; remains Candidate — Not Authorized, no ready-now implementation batch** — M1 and M2/M3 are separate candidate semantic-risk boundaries, no pooling; any schema/index ADR-gated; see program §6.7.1 for full rationale and candidate-bound evidence requirements (isolated production-shaped corpus, real user data not required/authorized; requires decision/outcome + four fields).
