...
- [Architecture evolution program](docs/architecture-evolution-program.md) — target evolution strategy, qualities, technical debt, phases, and decision routing through S7 terminal state (sync lives in the standalone current document above); `architecture.md` documents implemented state.
...
- [Projection completeness and authority intents](docs/projection-completeness-authority.md) — canonical governance for loaded-projection semantics, typed completeness capabilities, stable-ID navigation/mutation, caller-local complete reads, and request-local execution overlay; read it before changing chat data-access, windowed reads, or authority mutations.
- [Personal multi-device sync](docs/multi-device-sync.md) — **current sync reference (personal-hosted relay; operation-log + thin HTTP relay with automatic online convergence and cursor-based recovery as target; limited validation, not production-ready; conditional fallbacks only)**: goal, current approach, established evidence, gap to target, next decision, temporary working judgments, evidence pointers, fallback activation condition; target semantics owned by the sync connection & channel ADR and the sync data convergence ADR below; read this before any sync work.
- [Sync connection & channel ADR](docs/sync-connection-channel.md) — approved target for relay service connection/registration/channel/pairing semantics (`SYNC-CC-*`); no implementation authorized.
- [Sync data convergence ADR](docs/sync-data-convergence.md) — approved target for complete-synchronization semantics over the syncable personal-data domain (`SYNC-DATA-*`); no implementation authorized.
- [Sync architecture selection](docs/sync-architecture-selection.md) — fallback/reference candidate analysis (conditional reuse only on concrete current-path blocker with clear advantage; not current status).
...
- [Performance engineering](docs/performance-program.md) — stable methodology entry for performance work: amplification-first prioritization, hypothesis-driven lifecycle, evidence principles, and completion/closure rules; discovery link to the durable measurement contract (`docs/performance-measurement.md`) and current open workstreams (`docs/performance-workstreams.md`). Git history owns run history; it is not a living cross-session handoff source.
