...
| M6 synthetic sync metadata gap analysis harness — `src/main/services/chatDb/__tests__/m6SyncMetadataGap.bench.ts` + `m6SyncMetadataGap.ts` + focused tests | `pnpm bench:m6-sync-gap` (`M6_SYNC_GAP_BENCH=1`，`M6_SYNC_GAP_SCALE=small` 默认且唯一 profile) | Node | 隔离临时 SQLite 上 existing schema-v1 migration 的只读 required-table 与 documented sync-metadata absence fingerprint | **harness Implemented, inactive by default, not yet executed**；synthetic、analysis-only、directional、numeric-only；无 schema/migration/sync design、无 production/user state、无 real profile/ZIP/Dexie/Files、无 threshold/baseline/policy；future invocations remain inactive by default，全部 task/gate 成功后才输出 schema v1 artifact |
