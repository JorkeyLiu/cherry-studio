...
> **Status**: **S8 Selection Validation — Active; MVP implementation slice complete (limited validation, not production-ready)**. Selection validation remains canonical authority. MVP route `application operation-log + thin HTTP relay` implemented at `cef4689726` (31 files; Main-owned better-sqlite3 preserved; manual endpoint-driven sync; deterministic LWW; additive sync metadata migration `005_sync_metadata`; filtered payloads; reference relay `scripts/sync-relay/server.ts`). No final vendor/architecture selection beyond this MVP; broader S8 remains open. Not production-ready full synchronization.
> **Implementation validation**: focused sync tests 33 + relay HTTP 10 and full `pnpm build:check` 89 files / 2137+ tests passed at implementation — limited validation, not full product or E2E/UI regression proof.
...
> **Limitations (MVP slice)**: automatic/background sync, real-time push, E2EE, account/device pairing, attachments/binary/`file_path`, FTS/UI/context sync, complete chat mutation coverage, full conflict UI, production relay deployment are not implemented; post-commit capture window, manual-only operation, LWW applies (see §10, program §6.9, and `src/main/services/sync/chatDbHook.ts`).
...
> **MVP implementation note (2026-09-03)**: Spike S8-SP3 path `application operation-log + relay` has an **MVP implementation slice** at `cef4689726` (see Status and §6.9). This is a limited manual HTTP relay slice — not the full spike matrix and not a final vendor selection. Broad S8 selection remains open alongside the MVP slice; other finalists remain as remaining formal/selective future alternatives.
