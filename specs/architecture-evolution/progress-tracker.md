...
- **Phase 7 Startup — S7.1 + S7.2 Implemented & Closed 2026-08-30 (outcome/residual-risk).** S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized loading + tagged recovery. S7.2 renderer-local startup readiness loading (`Spin` `aria-busy`/`aria-label`, `data-testid="startup-readiness-loading"`) vs error (`Alert` `role=alert` + retry `Button` `data-testid="startup-readiness-retry"`, `startup.readiness.*` i18n with non-English placeholders replaced) with `failed→pending→ready/failed` retry rerunning captured dispatch→flush→ack without re-notifying `ReduxStoreReady`, subscription cleanup; plus B-08 staleness optimization (synchronous `takeRecords()` draining + snapshot-skip fast path, max 500 live Ranges unchanged, unconditional cross-chunk DOM rescan, `childList/subtree/characterData/attributes` coverage preserved); preserves LOCK-PROJECTION/LOCK-001 gating and LOCK-003 independence; no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/context-window/S7.1 change. Evidenced by focused Vitest (readiness/gate/retry ordering + B-08 staleness) + independent audit pass + authoritative `pnpm build:check` exit 0 (all six Vitest projects) on exact final worktree. Accepted residual risks: transient loading flash; retry best-effort if underlying resource remains unavailable; B-08 staleness heuristic (`textLength`/`childCount` after synchronous drain) not full DOM diff; PERF workstreams independent/open. Phase 7 remains partially Open — S7.3+ deferred; no ready-now batch exists unless current user instruction explicitly activates another one. Rollback one semantic revert(s), no migration.
...
- **S7.1 + S7.2 Implemented & Closed 2026-08-30 (outcome/residual-risk) — canonical §6.8.2–§6.8.3:** S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized loading + tagged recovery; S7.2 renderer-local readiness loading/error/retry UX (`failed→pending→ready/failed`, subscription cleanup, `startup.readiness.*` i18n) + B-08 synchronous `takeRecords()` draining (max 500 live Ranges, unconditional cross-chunk DOM rescan, `childList/subtree/characterData/attributes` preserved, LOCK-PROJECTION/LOCK-001 and LOCK-003 intact); renderer-only, no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/context-window/S7.1 change; focused Vitest + independent audit + authoritative `pnpm build:check`; no threshold/baseline/SLA; ARCH-010 disposed; Phase 7 remains partially Open (S7.3+ deferred); no ready-now batch exists unless current user instruction explicitly activates another one; rollback one semantic revert(s), no migration.
...
- **None — no ready-now batch exists unless current user instruction explicitly activates another one.** S7.1+S7.2 Implemented & Closed 2026-08-30 (outcome/residual-risk); S7.3+ remain Open/deferred per canonical §6.8; Phase 7 remains partially Open. Any future batch requires separate explicit authorization; SQ-01 may not be silently revived.
...
- Phase 7 S7.3+ and Phase 8 sync implementation — deferred (S7.1/S7.2 are the only authorized Phase 7 slices; no Main reorder, Antd locale split, Inputbar, telemetry, or retention/deletion authorized); Phase 8 requires governance activation and remains vendor-neutral.
