...
| **SYNC-S8-005** | **S8 Selection Validation is active; documentation/research only; no implementation authorization, except for the explicitly user-authorized limited MVP exception at `cef4689726` plus audited hardening at `6b40834971` (application operation-log + thin HTTP relay, manual-only, limited validation, not production-ready; LOCK-RT-004 limits retained); all broader S8 implementation remains unauthorized/open.** Entry is met by user activation of S8; exit requires primary-path validation documented, four-dimension trade-offs and residual risks accepted, and readiness validated. | **Locked** |
