...
| B-01..B-05 calibration harness — `src/main/services/chatDb/__tests__/b0105Calibration.bench.ts` + `b0105Calibration.ts` + `b0105Calibration.benchContract.ts` + focused tests | `pnpm bench:b0105-calibration` (`B0105_CALIBRATION=1`，可选 `B0105_CALIBRATION_SCALE=small|medium` 默认 `medium`) | Node | 确定性合成逻辑负载边界、TTL eligibility、numeric recency/LRU ordering、fit-step arithmetic 与 single-topic oversized classification | **harness implemented, inactive by default, optional/non-blocking, requires explicitly activated decision/outcome + four fields to count as valid/progress**；measurement-only directional；B-01..B-05 为已批准初始预设、实现待定，非经验最优/基线/SLA；校准未采纳策略/阈值/容量/逐出；无生产 retention/eviction/TTL/LRU/admission/persistence/IPC/schema、无用户数据；全部 task/gate 成功后才输出 schema v1 numeric-only artifact |
...
> **M4 FTS 重复存储体积诊断（合成测量、方向性 L3）**：1k/10k/50k 三 synthetic profile 为 measurement-only directional L3 via `pnpm bench:m4-fts-dup`（现有 schema-v1 迁移、隔离临时库、逻辑行/字符/UTF-8 字节重复体积、无物理库大小观测量、隐私安全、fail-closed）；非阈值/非基线；S6.5 仍为 Candidate — Not Authorized；future invocations inactive by default；artifact 为 gitignored schema v1（`chatdb-m4-fts-duplication-*`）；详细逐次数值与 provenance 由 Git 历史承担，不在本文保留。

> **M8 Backup/restore (L3 archive) — Authorized & Harness Implemented, inactive by default for future invocations, measurement-only directional L3 with gitignored schema-v1 artifact; measurement-only, no threshold/baseline/closure/authorization; future invocations inactive by default; requires explicitly activated decision/outcome + four fields to count as valid/progress; detailed per-run values and provenance in Git history, not reproduced here; design contract see `architecture-evolution-program.md` §7.2 (LOCK-001..007).**

> **M1 provenance note (diagnostic L3 only)**: M1 `chatdb-sort-order-shift` via `pnpm bench:sort-order-shift` (isolated temp DB, 9 combos N=100/500/1000×M=1/10/50, directional L3 only, no threshold/adoption/S6.4/S6.5 authorization; detail in `performance-workstreams.md` §2.4; provenance in Git).
