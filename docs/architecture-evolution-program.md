...
Renderer Redux projections accumulate without eviction; data-access is full-topic fetch with early-return on any cached IDs; viewport windowing expands without cap; scroll snapshots never expire; ContentSearch `Range[]` handle array grows without cap when mounted; context-info anchor-to-end may be unbounded when `contextCount=null`; background stream queues have no global cap; undo 50 and streaming throttle 100/5 min are observed bounds.
...
All numeric defaults are **architecture capacity defaults requiring calibration before implementation** — not thresholds or baselines.
...
| B-06 | Viewport rendered groups | target max 200 groups, 20-group steps, opposite-edge trim | Active viewport | Trim while preserving navigation/scroll anchors | No entity eviction by viewport trim |
| B-07 | Scroll snapshot cache | max 256 topics, 90-day TTL, immediate delete on hard delete | Device-local keyv | LRU+TTL via renderer-local index | Soft-delete retains snapshot |
| B-08 | ContentSearch live handles | max 500 live `Range` handles, bounded descriptors | Per search session | Materialize only current 500-match chunk | Search over current rendered DOM only |
| B-09 | Context-info retained result | max one active topic; invalidates on entities/blocks/assistant/context settings/topic id | Renderer memo | Single-topic memo | Unlimited `contextCount` semantics unchanged |
...
- **Scroll**: Index `topicId`+`lastAccess`; enforcement at startup + on each write; TTL first then LRU, topic-ID tie-break; missing index rebuilds from keys.
- **Viewport**: Expanding older trims newest overflow and vice versa; if trim would remove anchor, recenter up to 200 around anchor; restore pixel offset; no entity eviction.
- **ContentSearch**: At most one 500-match descriptor chunk; ranges materialized only for current chunk/match; navigation outside chunk rescans rendered DOM; generation change invalidates.
- **Invalidation**: Context-info eager on listed inputs; entity projection on authoritative mutation via generation; no incremental patch in Phase 4.
...
| Bounded caches | Design complete — evictable defaults (B-*) defined, pinned measured; implementation pending | Phase 4 / §6.5 design, exit Open |
...
| Cache invalidation rules and bounds | Phase 4 | **Design complete; exit Open** — capacity defaults pending calibration (B-01..B-11); C-01/C-02/pinned directional only |
