...
**Focused tests / validation status**:
- New/updated focused tests listed above are the S6.1 consumption evidence surface; they exercise the window request/validation/merge/stale-discard paths listed.
- Full repository gates (`pnpm build:check` / `pnpm test` / `pnpm lint` / `pnpm format` / `pnpm typecheck`) have **not yet run for this integrated worktree** (known baseline per task — no full-gate PASS/FAIL is claimed).
- No fresh `pnpm build` / E2E / coverage run is claimed for this consumption delta beyond the focused window tests; no artifact hashes, quantitative calibration outputs, or new `test-results/` artifacts are invented.
