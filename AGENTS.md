...
- [Sync architecture selection](docs/sync-architecture-selection.md) — **canonical S8 selection validation authority (Active; MVP implementation slice complete at `cef4689726` — limited validation, not production-ready; broader S8 remains open)**: product target (Syncthing-like local-first, no Cherry Chat account, device pairing, one user-selected service), current-state constraints, balanced four-dimension method, candidate taxonomy/shortlist, research findings, equal-scenario criteria, four-spike plan (S8-SP1..SP4), decision rules, and write-amplification gate; MVP route `application operation-log + thin HTTP relay` (manual, LWW, additive sync metadata, filtered payloads, reference relay) is implemented; read this before any sync work.
