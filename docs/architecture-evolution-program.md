...
| FTS storage dedup | Phase 6 | **Candidate S6.5 — Not Authorized**; depends on M4 (1k, default 10k, and explicitly activated 50k synthetic profiles executed 2026-08-28 via `pnpm bench:m4-fts-dup`, directional only, no threshold/baseline/benefit; real corpora/physical DB size unresolved; exact metrics in `performance-measurement.md` §6 and status summary in `performance-workstreams.md` §2.4); ADR if schema |
