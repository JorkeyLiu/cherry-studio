...
| **PERF-004**（提案） | 统一规模矩阵 harness + 跨运行曲线/方差/knee 消费（multi-level parameterization / variance / knee consumer） | Planned（提案，未批准） | 目标：把 §8.1 多维矩阵落成可复用参数化 harness，并消费跨运行曲线/方差/拐点（§11.1 差距）；当前无 Active 工作流（§16；PERF-101 已关闭），进入 Active 需 Main 授权（§3、Q6） |
| **PERF-003**（提案） | 用户影响驱动的优先级清单 | **Retired（2026-08-13）** | 优先级已由用户批准（PERF-100/101/102），本提案的「产出优先级清单」目标由该批准取代；保留历史记录 |

> 优先级顺序（PERF-LOCK-004 落地）：**PERF-100（P0）→ PERF-101 / PERF-102（P1）**（PERF-001 使能切片已完成，2026-08-13）。**PERF-101 已于 2026-08-14 经 Main 决策检查点（§16）由用户显式授权为唯一 Active，并按用户显式决策（「关闭 101」）于同日关闭（Done，§12/§16）——当前无 Active 工作流**；PERF-102 保持 **Approved**，仅在 Main 另行授权时方可标记 Active（§3：Active 同一层级唯一；并行需 Main 批准且每个保持自己的验收与交接）。Approved 工作流只在获准进入执行时标记 Active；下一 Active 的选择由 §16 的 Main 授权检查点显式作出，关闭**不自动激活**任何工作流。
>
> PERF-002 保留稳定 ID 与方向草案，**范围未批准**；具体范围由 Main 在本程序推进时决策，并回写本表。
>
> PERF-004（统一规模矩阵 harness/曲线方差消费，§8.1/§11.1）为 **Planned 提案**，不参与优先级顺序；当前无 Active 工作流（§16；PERF-101 已关闭），PERF-004 进入 Active 需 Main 授权（§14 Q6）。
...
| Q6 | 统一规模矩阵 harness 与跨运行曲线/方差/knee 消费（§8.1 矩阵落地、§11.1 差距、PERF-004 提案）由谁在何时以何形态执行；`bench:shared`/`bench:aicore` 是否补覆盖 | 方向不清 / 重复造轮子 | 提案已登记（§12 PERF-004，Planned）；当前无 Active 工作流（§16；PERF-101 已关闭），PERF-004 进入 Active 需 Main 授权（§12/§16）；待 Main 决策 |
| R12 | answer-tab 原子选择（F，§16）未经独立审计与 fresh 生产构建验证即关闭 PERF-100 | PERF-100 关闭决策无据 / 验收失真 | **已满足（2026-08-14）**：独立审计 verdict = pass-with-findings、0 blockers（findings 无 in-scope blocker，无需修正）；fresh 未改动 worktree `pnpm build` 退出码 0 + quick/s0-20/s0-100 三次 focused E2E 退出码均 0（Phase 1–4 全过、12/12 gate、strict schema-v1、35 有限指标、Electron ABI 145，§16 F/§12）；focused 验证数字为实现会话报告（§17 历史），不替代独立审计与 fresh build（已按 §16 F 执行）；PERF-100 关闭决策已由 Main 基于上述证据做出；Main 决策检查点已完成（2026-08-14，§16）；**PERF-101 已按用户显式决策关闭（Done，§12/§16）**；下一单元为 **Main 授权检查点（非执行，§16）**——选择至多一个下一 Active 工作流（PERF-102 或另行显式批准的按需 PERF-101 扩展） |
...
## 16. 会话交接（Session Handoff）— 下一可执行单元：Main 授权检查点（非执行；选择至多一个下一 Active 工作流）

> 新会话（或任何接手本程序的会话）从本节开始执行，不需要回顾历史会话。

**当前状态（2026-08-14，PERF-100 与 PERF-101 均已关闭；当前无 Active 工作流；下一单元为 Main 授权检查点（非执行））**：PERF-100（P0）独立审计与 fresh 生产构建 quick/s0-20/s0-100 验证**全部完成并通过**（§16 F），PERF-100 已标记 **Done**（§12）。**Main 决策检查点（2026-08-14）已按 PERF-LOCK-004 比较 PERF-101 vs PERF-102，用户显式授权 PERF-101 为唯一 Active（§12/§16）**；PERF-102 保持 Approved、PERF-002/004 保持 Planned——激活来自显式授权，任何关闭**不自动激活**工作流。**PERF-101 缓存未命中话题切换测量子切片已完成并验证（2026-08-14）**：对角线（§16 G：quick/s0-20/s0-100，N=W）+ **正交（§16 H：N20/W10、N100/W10、N100/W50，独立 benchmark ID/profileCode）**——production-build Playwright E2E 测量资产、正确性 gate 先于计时、click-to-first-useful-render + loadCommit 指标、schema v1 artifact、每次 12/12 gate、L1 gate / L3 数值暂定、**无阈值、无修复**——六点部分网格已就位、O(N)/O(W) 已可分离，数值回写 §7/§8/§11/§14。**用户显式决策「关闭 101」（2026-08-14）已记录**：PERF-101 标记 **Done（2026-08-14；diagonal + orthogonal S0 measurement slices）**（§12），关闭依据为 S0 六点部分网格证据——固定生产默认 W=10 时 N20→N100 非单调（192.7→164.1 ms）、未见持续话题规模放大，固定 N=100 时 W10→W50→W100 强增长（164.1→528.9→858.4 ms）；**L3 方向性证据、无阈值、无根因归属；基于当前证据无运行时修复需要、也未授权任何修复**；**不声称 L4 秒级报告已证伪**（§14 R14 保留为残余风险）。**PERF-101 关闭后当前无 Active 工作流**，下一可执行单元为 **Main 授权检查点（非执行，§16 下一单元）**：选择**至多一个**下一 Active 工作流（候选：PERF-102，或另行显式批准的按需 PERF-101 扩展——扩展保持 Planned/非 Active，300+ 保持 LOCK-007 按需）；授权前不开始任何实现/验证。最终强制 `pnpm format` / `pnpm lint` / `pnpm test` 已由独立 Validation 会话在本文档上一写回（PERF-101 Active 授权）后的状态上执行并通过（2026-08-14，退出码均 0，Node 24.11.1 / pnpm 10.27.0；`pnpm test` Node lane 后 Electron ABI 145 已恢复；`pnpm format` 对 code/test/config 零改动，仅 gitignored `test-results/.last-run.json`，先前 fresh-build PERF-100 E2E 证据保持有效，PERF-LOCK-005）；**本次写回（PERF-101 关闭决策）为纯文档变更，未重跑强制 gate**（§16 诚实性声明）。**最终交付写回（2026-08-14，docs-only；关闭提交前最后一次手off）**：最终交付 gate 已在代码/测试精确表面上完成并通过——`pnpm format` 与 `pnpm build:check` 退出码均 0（嵌套 lint/openapi/full test 全部退出码 0），full test **9,492 passed / 75 skipped / 0 failed（428 files）**，Node lane 后 Electron ABI 145 已恢复且只读 SQL probe 通过，PERF-101 测量 spec sha256 前后一致 `56f63649ce18baa1362f247b8bd7a34f701fa2ac128906425e0498fa57780d38`（证据保鲜：spec 字节未变且 docs 位于 E2E 输入之外，§16 G/H 保持 fresh；详见诚实性声明）；本交付（文档写回 + PERF-101 测量 spec）**经用户显式授权用于关闭提交**，确切提交内容以 git 历史为准；下一单元仍为 **Main 授权检查点（非执行）**——选择至多一个下一 Active 工作流，授权前不开始任何实现/验证。
...
**当前 Active 工作流**：**无**（PERF-100 与 PERF-101 均已 **Done**，§12；§3：Active 同一层级唯一）。**PERF-101（P1）已关闭**——缓存未命中话题切换测量子切片（对角线 §16 G + 正交 §16 H）已作为完成子切片关闭，**用户 2026-08-14 显式决策关闭（无运行时修复需要/授权）**，下一单元为 **Main 授权检查点（非执行）**（§16 下一单元）；**PERF-102（P1）保持 Approved**——仅在 Main 另行显式授权后标记 Active（并行需 Main 批准，每个保持自己的验收与交接）；PERF-002（Planned）与 PERF-004（Planned 提案，多维矩阵方向）保持 Planned，范围未批准。激活来自显式授权，PERF-100/101 关闭**不自动激活**任何工作流。
...
**下一可执行单元（非执行）— Main 授权检查点：选择至多一个下一 Active 工作流**：

PERF-100 与 PERF-101 均已关闭，当前**无 Active 工作流**。本单元**只做决策、不执行实现/验证**（决策权在 Main/用户），候选中性并列如下：

- **选项 1 — 授权 PERF-102（Approved，P1 并行窄流）为唯一 Active**：PERF-101 关闭记录（Done）已就绪；PERF-102 进入 Active 需**单独显式授权**（§3 Active 唯一、§12）；授权后按 §7 校准流程为 PERF-102 建立暂定预算并启动其测量优先切片。
- **选项 2 — 另行显式批准的按需 PERF-101 扩展**：仅在用户**明确优先复现 L4 大话题/复杂内容秒级话题切换报告**时选择（§14 R14；S0 六点未复现，复现需 300+ 与受控内容复杂度）。建议形态：**N300/W10**（话题规模 300，LOCK-007 按需级）与**受控内容复杂度**（长消息/复杂内容；**不发明用户频率/内容数据**，PERF-LOCK-003/006）；按需资产**不进入每次提交 CI**；产出仍为 **L3 暂定数值**。**PERF-101 扩展保持 Planned/非 Active，除非本选项被显式批准**；300+ 保持 LOCK-007 按需；扩展完成后的处置（再关闭或进入修复）在批准时一并明确。
- **选项 3 — 暂不选择**：保持无 Active（PERF-101 关闭后程序可在无 Active 状态停留），后续任一授权由 Main 单独作出。

**Main 推荐（stated recommendation，非决策）**：**授权 PERF-102 为下一 Active（选项 1）**——PERF-101 已在 S0 内关闭、无运行时修复，PERF-102 为已批准（Approved）的 P1 候选，是优先级顺序（PERF-LOCK-004：PERF-100 → PERF-101/102）中的既有下一顺位；PERF-101 按需扩展（选项 2）仅在用户明确优先复现 L4 大话题/复杂内容案例时选择（需显式单独批准、保持按需、不进入每次提交 CI）。**至多一个结果**；未获显式授权前**不激活任何工作流**。

**决策成功条件（当前单元 = Main 授权检查点，非执行）**：

- **至多一个结果**：授权一个下一 Active 工作流（如 PERF-102）**或**批准一个**有界** PERF-101 按需扩展（范围/规模点/CI 位置显式记录）**或**保持无 Active；三者互斥，至多一个。
- **PERF-101 扩展不得自动转为 Active**：除非本检查点显式批准（300+ 保持 LOCK-007 按需，不进入每次提交 CI）；PERF-101 保持 Done 直至另行决策。
- **决策前不开始任何实现/验证**；本检查点不运行 `pnpm`、不修改生产/运行时代码、不改架构/持久化/兼容/隐私语义（PERF-LOCK-008、§13）。
- **不声称阈值/根因归属**：所有数值保持 **L3 暂定**（§7）；关闭或后续扩展都不把 S0 观察升级为阈值或根因（PERF-LOCK-003、§14 R13/R14）；不声称 L4 秒级报告已复现/已证伪。
- **不删除任何 artifacts/历史**（§16 A–H、§17、PERF-100/对角线/正交 checksums 保留不改写）；**不提交/推送 git**。
- 决策结果回写本文件（§12 状态、§16 下一单元、§17 变更记录）。

**非目标（不得做）**：

- 本检查点**不实现、不验证、不测量**；**不自动激活任何工作流**（包括 PERF-102）——激活须经本检查点的显式授权（决策权在 Main/用户）；**PERF-101 扩展不自动转为 Active**（保持 Planned/非 Active，直至显式批准）。
- **不做**：新增矩阵级别（300+/profile 级按需未动，除非 PERF-101 按需扩展被显式批准）、cut 基准、DB 体积 harness、广泛 memoization/窗口重设计、新性能阈值（唯一已提交阈值仍为冷开 `<500ms`）；不把任何 L3 数值（2026-08-13 / C / E / F post-fix answer 与 PERF-101 对角线 §16 G / 正交 §16 H 新产出）提升为阈值或基线。
- **不发明用户频率/内容数据**（PERF-LOCK-003/006，§14 R14）；不声称 S0 六点观察为根因、不声称 L4 秒级报告已复现/已证伪。
- 不重述最终强制 gate 为待执行/未通过（`pnpm format`/`pnpm lint`/`pnpm test` 已由独立 Validation 会话通过，2026-08-14）；**本次写回（PERF-101 关闭决策）为纯文档变更、未重跑强制 gate，上一状态 gate 证据保持有效**（§16 诚实性声明）；不声称 H7 为历史失败原因、不声称崩溃已修复。
...
**诚实性声明**：本交接只记录已完成证据（A–H 全部完成并验证，2026-08-14）。C/E、2026-08-13 数值、F 的 post-fix answer 数值、**G 的 PERF-101 对角线数值与 H 的正交数值均为 L3 暂定**——不是阈值、不是基线，未提升（PERF-LOCK-003）。F 的 focused 验证数字（实现会话报告）与独立审计 + fresh build/三次 E2E 的集成证据按 §6 分层记录。**最终交付 gate（2026-08-14，代码/测试精确表面）已由独立 Validation 会话完成并通过**：固定工具链 Node 24.11.1 / pnpm 10.27.0；`pnpm format` 退出码 0——对 code/test/config 零改动，仅 gitignored `test-results/.last-run.json`；**PERF-101 测量 spec sha256 前后一致 `56f63649ce18baa1362f247b8bd7a34f701fa2ac128906425e0498fa57780d38`**（验证期间 spec 字节未变）；`pnpm build:check` 退出码 0——嵌套 lint（oxlint 0 errors/76 baseline warnings、eslint 0 errors、node/web/aiCore typecheck 退出码 0、i18n pass、format clean）、openapi:check、full `pnpm test`（Node lane ABI 137）全部退出码 0——**full test 9,492 passed / 75 skipped / 0 failed，428 files**；Node lane 后 Electron ABI 145 已恢复且只读 SQL probe 通过（更早一次分套件数字——main 1775 pass/72 skip、renderer 1726 + 53 pass、aiCore 4152 pass、shared 380 pass、scripts 494 pass、e2e-utils 242 pass/3 skip——完整记录于 §17 2026-08-14 PERF-100 关闭行）。**证据保鲜推理（freshness）**：先前 fresh-build PERF-101 对角线/正交 E2E（§16 G/H）保持 fresh——**spec 字节未变（sha256 前后一致）且 docs 位于 E2E 输入之外**；本次最终写回为 **docs-only**、不改代码/测试表面、不进入 E2E 输入，最终 gate 证据对关闭提交保持权威（PERF-LOCK-005）。**交付 provenance（稳定来源）**：测量 artifacts 均在**父提交 HEAD `162b9f1e34` dirty=true** 上产生（artifact git 元数据即记录该 dirty 来源）；本次交付 = **活性文档写回 + PERF-101 测量 spec**，**经用户显式授权用于关闭提交（closure commit）**；确切提交内容以 git 历史为准（**不预先断言提交 hash**）；**未推送/未开 PR 仅为本次写回时点的交接状态，非永久不变量**。下一单元（Main 授权检查点，非执行）不涉及新测试资产验证，若后续执行任一被授权工作流，其验证按 §16 成功条件执行并在验证后按 §15 重跑强制 gate。Q5 保持有界关闭（历史事件为执行上下文销毁，非已证明崩溃/重载；H7 未证明）。**PERF-101 已按用户显式决策关闭（Done，2026-08-14；测量子切片——对角线 §16 G + 正交 §16 H——完成并验证；基于当前证据无运行时修复需要/授权；不声称 L4 秒级报告已证伪，§14 R14 保留）——当前无 Active 工作流**；PERF-102 保持 Approved、PERF-002/004 保持 Planned；激活来自显式授权，PERF-100/101 关闭不自动激活任何工作流。

**参考**：§9（基线 0）、§10.1/§10.2（L4 用户报告 + 静态调查，方向判断）、§7（暂定预算，L3 数值；含 F post-fix answer 新行、**G PERF-101 对角线新行与 H 正交新行**）、§8（多维规模矩阵；话题消息数与可见消息数行含对角线 + 正交六点、§8.2 标注六点网格与有效 min(N,W)）、§5.1（结果契约）、§11（harness 清单/资产保留；PERF-101 spec 行——`PERF101_SCALE` 六个取值）、§14（R1 部分缓解、R3/R4 关闭、R9 已分诊、R10 已满足、Q5 已关闭、R11 保留、**R12 已满足**、**R13 已缓解（正交数据）**、**R14 已识别（L4 大话题/复杂内容未复现）**、Q6）、测量 spec（`tests/e2e/specs/conversation/perf100-measurement.spec.ts`，`PERF100_SCALE` 环境变量）、**PERF-101 测量 spec（`tests/e2e/specs/conversation/perf101-topic-switch-measurement.spec.ts`，`PERF101_SCALE` 环境变量：unset/empty=quick、`s0-20`、`s0-100`、`n20-w10`、`n100-w10`、`n100-w50`；缓存未命中话题切换测量资产；**对角线验证通过 2026-08-14，§16 G；正交验证通过 2026-08-14，§16 H**）**、2026-08-14 实现行与关闭行（§17，F/G/H 完整细节）、artifact 身份与 checksums（§16 F/G/H，gitignored 本地 deliverable，**路径可能不持久**——见保留警示）、**下一可执行单元**（§16 下一可执行单元）：**Main 授权检查点（非执行）**——选择至多一个下一 Active 工作流：选项 1 授权 PERF-102（Approved，需单独显式授权）**或**选项 2 另行显式批准的按需 PERF-101 扩展（如 N300/W10 与受控内容复杂度，LOCK-007 按需、不进入每次提交 CI；L4 大话题/复杂内容复现，§14 R14；**扩展保持 Planned/非 Active 直至显式批准**）**或**选项 3 保持无 Active；授权前不开始任何实现/验证；PERF-101 关闭记录（Done，2026-08-14）已就绪，PERF-102 激活仍需单独显式授权。
