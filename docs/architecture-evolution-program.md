...
> **Last updated**: 2026-08-31 — S7.10 Implemented & Closed (renderer-local maintenance activation: 0ms bounded post-bootstrap scroll sweep + retention background registration split with check-before-refresh fix). S7.1–S7.10 Implemented & Closed. Prior detailed history in Git log; no chained provenance header is maintained.
...
| **7** | Startup Architecture | **S7.1 + S7.2 + S7.3 + S7.4 + S7.5 + S7.6 + S7.7 + S7.8 + S7.9 + S7.10 Implemented & Closed 2026-08-31 (outcome/residual-risk) — S7.10 renderer-local maintenance activation (scroll 0ms bounded post-bootstrap sweep + retention background registration split with check-before-refresh fix) — S7.1 five secondary routes lazy, Home/sidebar/navigation/App gates eager; S7.2 renderer-local readiness loading/error/retry UX with projection gating + ReduxStoreReady ordering preserved; S7.3 renderer-local Antd locale on-demand loading (12 locales, per-locale dynamic chunks, eager payload removed); S7.4 renderer-local i18n translation JSON + Day.js locale on-demand loading (12 translation chunks + 11 Day.js locale chunks, atomic activation, explicit literal dynamic imports); S7.5 renderer-local XLSX export on-demand (literal dynamic import, private success/pending cache, concurrent deduplication, retry after rejection, typed named/default normalization, export/cancel/write/error preserved); S7.6 renderer-local Home-message KaTeX atomic on-demand loading (literal dynamic imports, complete atomic chain, parser-compatible fenced-math detection, shared pending/cache/retry, bounded per-block failure with fallback/source Markdown, streaming/race/plugin preserved); S7.7 renderer-local Mermaid demand-activation (unconditional `useMermaid()` removed from eager `CodeStyleProvider`, `MermaidPreview` remains demand owner; Mermaid remains deferred until preview use); S7.8 renderer-local code tooling demand activation (Shiki theme-metadata demand activation and shared CodeEditor/CodeMirror lazy activation — provider mount does not load metadata, custom theme first highlight awaits metadata while default/auto uses fallback, AsyncInitializer reset enables retry, thin CodeEditor wrapper + literal dynamic import in eager entry with ReactCodeMirror/useCodeMirror/prepareCodeChanges in separate async chunk, public API/ref preserved behind internal lazy/Suspense with localized loading shell (Ant Design `Spin`/loading text/`aria-busy`, `CodeEditorLoadingFallback`) and bounded local error fallback, no false chunk retry action); S7.9 renderer-local auto-sync tooling demand activation — bootstrap removes BackupService/NutstoreService static activation, literal dynamic imports at existing 8s demand check through proxy chunks, SettingsPage retains static imports, 8s/switch/interval/lastSyncTime/retry/IPC/Redux/settings preserved; directional topology/activation only); S7.11+ Open (deferred); no ready-now batch exists unless current user instruction explicitly activates another one** | Phase 2; Phase 3 for conversation-startup |
...
- **Status**: **S7.1 + S7.2 + S7.3 + S7.4 + S7.5 + S7.6 + S7.7 + S7.8 + S7.9 + S7.10 Implemented & Closed 2026-08-31 (outcome/residual-risk); S7.11+ Open (deferred)** — S7.1 renderer-only lazy five secondary routes (Home/sidebar/navigation/App gates eager); S7.2 renderer-local readiness loading/error/retry UX with projection gating + ReduxStoreReady ordering preserved (B-08 max 500 live Ranges unchanged); S7.3 renderer-local Antd locale on-demand loading (12 locales as per-locale dynamic chunks, eager 12-locale payload removed from entry); S7.4 renderer-local i18n translation JSON + Day.js locale on-demand loading (12 translation chunks + 11 Day.js locale chunks, atomic activation, explicit literal dynamic imports, Inputbar/Home and S7.1/S7.2/S7.3 unchanged); S7.5 renderer-local XLSX export on-demand (literal dynamic import, private success/pending cache, concurrent deduplication, retry after rejection, typed named/default normalization, export/cancel/write/error preserved); S7.6 renderer-local Home-message KaTeX atomic on-demand loading (literal dynamic imports, complete atomic chain, parser-compatible fenced-math detection, shared pending/cache/retry, bounded per-block failure with fallback/source Markdown, streaming/race/plugin preserved); S7.7 renderer-local Mermaid demand-activation (unconditional `useMermaid()` removed from eager `CodeStyleProvider`, `MermaidPreview` remains demand owner; Mermaid remains deferred until preview use); S7.8 renderer-local code tooling demand activation (Shiki theme-metadata demand activation and shared CodeEditor/CodeMirror lazy activation — provider mount does not load metadata, custom persisted theme first highlight awaits metadata while default/auto uses fallback, AsyncInitializer supported reset enables demand retry, thin CodeEditor wrapper + literal dynamic import in eager entry with ReactCodeMirror/useCodeMirror/prepareCodeChanges in separate async chunk, public API/ref preserved behind internal lazy/Suspense with localized loading shell (Ant Design `Spin`/loading text/`aria-busy`, `CodeEditorLoadingFallback`) and bounded local error fallback, no false chunk retry action; directional topology/activation only); S7.9 renderer-local auto-sync tooling demand activation — bootstrap removes BackupService/NutstoreService static activation, literal dynamic imports at existing 8s demand check through proxy chunks, SettingsPage retains static imports, 8s/switch/interval/lastSyncTime/retry/IPC/Redux/settings preserved; directional topology/activation only; S7.10 renderer-local maintenance activation (scroll 0ms bounded post-bootstrap global sweep + retention background registration split with check-before-refresh fix; Keyv creation/init and lifecycle/deletion/byte-cache stay eager/sync — see §6.8.11); directional topology/activation only. **Phase 7 remains partially Open — S7.11+ deferred; no ready-now batch exists unless current user instruction explicitly activates another one.** Remaining Phase 7 tracks (Main startup reorder, Inputbar/MarkdownEditor/RichEditor KaTeX beyond auto-sync scope, Redux/Dexie/SQLite, background windows, S6.5, telemetry/retention/deletion, Phase 8) remain not authorized.
...
- **Dependencies**: Phase 2; Phase 3 stable host for conversation-startup (already satisfied). S7.1 touches only renderer bundle/activation; S7.2 touches only renderer readiness UX + B-08 optimization; S7.3 touches only renderer Antd locale loading; S7.4 touches only renderer translation JSON + Day.js locale loading with atomic activation and explicit literal imports; S7.5 touches only renderer XLSX export loading via `xlsxLoader` literal dynamic import with private cache/dedup/retry/normalization; S7.6 touches only renderer Home-message KaTeX atomic on-demand loading via literal dynamic imports with complete atomic chain, shared pending/cache/retry, bounded per-block failure with fallback and preserved streaming/race/plugin semantics; S7.7 touches only removal of unconditional `useMermaid()` activation from eager `CodeStyleProvider` with `MermaidPreview` demand ownership preserved; S7.8 touches only Shiki theme-metadata demand activation and shared CodeEditor/CodeMirror lazy activation (thin wrapper + literal dynamic import in eager entry, demand-owned metadata/CodeMirror) with provider mount without ensure, custom-theme await vs default/auto fallback, supported retry, API/ref preserved, localized loading shell with Ant Design `Spin`/loading text/`aria-busy` and bounded local error fallback, no false chunk retry action; S7.9 touches only renderer init auto-sync demand activation (literal dynamic imports at existing 8s, bounded `loggerService` logging, preserve 8s/switch/interval/lastSyncTime/retry/IPC/Redux/settings semantics, no service API or Main/preload/shared/SQLite/Dexie change) — historical Implemented & Closed; S7.10 touches only renderer-local maintenance activation (scroll 0ms bounded sweep + retention background split with check-before-refresh, Keyv creation/init stays in bootstrap, first-read/first-write/clear/hard-delete immediate, plain setTimeout(0), no policy/lifecycle crossing) — Implemented & Closed 2026-08-31; all no Main/preload/shared/IPC/SQLite/Dexie/StoreSync/identity/release/migration/sync/background/telemetry or Markdown/ReactMarkdown/editor KaTeX governance beyond S7.10 — S7.9–S7.10 historical, only S7.11+ deferred — and no Inputbar/Home regression beyond S7.10 and no Main startup reorder/Redux/Dexie/SQLite/background windows/S6.5/Phase 8 expansion beyond S7.10.
- **Relationship**: See §8 — S7.1 is the bundle/activation track, S7.2 is the readiness/hydration hardening track, S7.3 is the Antd locale on-demand track, S7.4 is the translation JSON + Day.js locale on-demand track, S7.5 is the XLSX export on-demand track, S7.6 is the Home-message KaTeX atomic on-demand track, S7.7 is the Mermaid demand-activation track (unconditional `CodeStyleProvider` activation removed, `MermaidPreview` demand-owned); S7.8 is the code tooling demand-activation track (Shiki metadata demand-owned and shared CodeEditor/CodeMirror lazy with thin wrapper; provider mount without metadata load, custom-theme await vs default/auto fallback, supported retry, API/ref preserved, localized loading shell with Ant Design `Spin`/loading text/`aria-busy` and bounded local error fallback, no false chunk retry action; directional topology/activation only); S7.9 is the auto-sync tooling demand-activation track (bootstrap removes BackupService/NutstoreService static activation, literal dynamic imports at existing 8s demand check, bounded `loggerService` logging, preserve 8s/switch/interval/lastSyncTime/retry/IPC/Redux/settings semantics) — historical Implemented & Closed; S7.10 is the maintenance activation track (scroll 0ms bounded sweep + retention background split, Keyv creation/init stays in bootstrap, check-before-refresh, eager deletion/byte-cache, plain setTimeout(0)) — Implemented & Closed 2026-08-31; conversation lazy activation (S3.5) remains distinct and not duplicated; remaining boot-service tracks (Redux rehydration, Dexie init, SQLite cold open, background windows) remain deferred beyond S7.10 — S7.9–S7.10 historical, only S7.11+ deferred.
...
Independent/deferred tracks not blocked by S7.1/S7.2/S7.3/S7.4/S7.5/S7.6/S7.7/S7.8/S7.9/S7.10 — S7.1–S7.10 Implemented & Closed 2026-08-31 (outcome/residual-risk), S7.11+ deferred:

| Track | Description | Independence | Status |
|---|---|---|---|
| App boot services | Service init order/parallelism (remaining beyond S7.10) | Independent | S7.11+ deferred (not selected, lifecycle races) |
...
| Startup improvements (S7.1/S7.2/S7.3/S7.4/S7.5/S7.6/S7.7/S7.8/S7.9/S7.10) | Phase 7 | **S7.1 + S7.2 + S7.3 + S7.4 + S7.5 + S7.6 + S7.7 + S7.8 + S7.9 + S7.10 Implemented & Closed 2026-08-31 (outcome/residual-risk) — S7.10 renderer-local maintenance activation (scroll 0ms bounded post-bootstrap global sweep + retention background registration split with check-before-refresh fix; Keyv creation/init and lifecycle/deletion/byte-cache stay eager/sync — see §6.8.11) — S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized fallback + tagged recovery; S7.2 renderer-local readiness loading/error/retry UX (pending `Spin` vs failed `Alert`+retry, `failed→pending→ready/failed`, subscription cleanup, `startup.readiness.*` i18n) + B-08 `takeRecords()` draining with max 500 live Ranges, unconditional cross-chunk DOM rescan, `childList/subtree/characterData/attributes` preserved, LOCK-PROJECTION/LOCK-003 intact; S7.3 renderer-local Antd locale on-demand (12 locales as per-locale dynamic chunks, cache + stale-guard + fallback, eager payload removed); S7.4 renderer-local translation JSON + Day.js locale on-demand (12 translation + 11 Day.js locale chunks, atomic activation, explicit literal imports, stale-guard, fallback; selection rationale consistent with S7.1/S7.3 bundle deferral without Inputbar/Home regression); S7.5 renderer-local XLSX export on-demand (literal dynamic import, private success/pending cache, concurrent deduplication, retry after rejection, typed named/default normalization, export/cancel/write/error preserved); S7.6 renderer-local Home-message KaTeX atomic on-demand loading (literal dynamic imports, complete atomic chain, parser-compatible fenced-math detection, shared pending/cache/retry, bounded per-block failure with fallback/source Markdown, streaming/race/plugin preserved); verification: S7.1 production chunks + Vitest + Playwright, S7.2 focused Vitest + audit, S7.3 per-locale chunks + Vitest + audit, S7.4 directional chunk topology (12 translation + 11 Day.js locale chunks, representative payload absent from entry, directional only) + focused atomic/stale-guard/callback verification + audit pass-with-findings (0 blockers, 1 acceptable coverage risk) + authoritative gates passed (build and `pnpm build:check`), S7.5 directional split `xlsx-DusDZK-x.js` with markers absent from entry (directional only) + focused 56 tests + audit pass (no blocker, findings corrected) + authoritative `pnpm build:check` exit 0 (1018s, 477 files |2 skipped, 9345 tests |7 skipped) + fresh build 26s/vite 7.50s + diff-check/ABI pass, S7.6 fresh build entry with literal dynamic import links and deferred KaTeX JS/CSS/contrib/parser assets (directional topology only, no byte/startup/SLA/baseline/threshold, no fixed chunk names/sizes) + focused 155 tests passed final relevant command (loader/fallback/mhchem covered by prior 148-test passing evidence within total focused evolution) + authoritative `pnpm build:check` exit 0, 524 files passed |1 skipped, 9599 tests passed |75 skipped (arithmetically correct sum), lint/typecheck/i18n/format/OpenAPI pass + git diff --check and ABI 145 probe pass, E2E not run for isolated renderer-local KaTeX loading with no IPC/persistence/lifecycle, S7.7 fresh build emitting separate Mermaid chunks including `mermaid.core-xh62IZ_A.js` with entry containing dynamic import map but `CodeStyleProvider` body containing no Mermaid activation (directional topology/activation only, not a byte-saving/startup/SLA/baseline/threshold claim) + focused 17/17 (provider boundary suite + `MermaidPreview` hook ownership) + independent audit pass and re-audit pass + authoritative `pnpm build:check` exit 0, 23m18s, files 45 pass|1 skip (main), 356 (renderer), 13 (aiCore), 18 (shared), 29 (scripts), 20 pass|1 skip (e2e-utils), tests 1855 pass|4 skip (main), 5070 (renderer), 380 (aiCore), 807 (shared), 712 (scripts), 524 pass|3 skip (e2e-utils), all lint/typecheck/i18n/format/OpenAPI pass on exact final worktree plus fresh `pnpm build` exit 0, `git diff --check` pass, E2E not run for isolated renderer activation boundary, S7.8 fresh production build exit 0 with entry containing thin CodeEditor wrapper/literal dynamic import only and separate async chunk with `ReactCodeMirror`/`useCodeMirror`/`prepareCodeChanges` plus `CodeStyleProvider` body containing no mount Shiki ensure (directional topology/activation only, not a byte-saving/startup/SLA/baseline/threshold/hashed-chunk claim) + final code audits pass 0 findings + prior two aggregate attempts failed on lint/typecheck and were corrected + authoritative `pnpm build:check` exit 0, 1011s, 575 files passed|2 skipped, 11539 tests passed|79 skipped across main core/native/heavy, renderer 358 files/5098 tests, aiCore 13/380, shared 18/807, scripts 29/712, e2e-utils 20 pass|1 skip/524 pass|3 skip, all lint/typecheck/i18n/format/openapi pass on exact final worktree plus fresh `pnpm build` exit 0 and `git diff --check` pass, E2E not run for isolated renderer activation boundary; docs-only edit means final code-surface gate evidence remains valid; S7.9 fresh `pnpm build` exit 0 (23s) with entry `index-DU1QcOXC.js` uses awaited dynamic boundaries through proxy chunks, SettingsPage retains static imports (directional topology/activation only), focused 14/14, independent code/docs audits pass 0 findings, authoritative `pnpm build:check` exit 0 (812s, 576 files passed |2 skipped, 11553 tests passed |79 skipped, renderer 359 files / 5112 tests including focused 14/14), all lint/typecheck/i18n/format/OpenAPI pass, Electron ABI 145 restore + SQL probe pass; E2E not run for isolated renderer-local activation boundary with no IPC/persistence/lifecycle; S7.10 fresh `pnpm build` exit 0 with entry `index-DXjM4Z9p.js` with `residentRetention-BjzUo1ip.js` proving timer boundary (directional topology/activation only, artifact hash only canonical writable), focused 31/31, code audit pass 0 findings, docs label finding corrected, authoritative `pnpm build:check` exit 0 (579 files passed |2 skipped, 11,584 tests passed |79 skipped, renderer 362 files / 5,143 tests, all lint/typecheck/i18n/format/OpenAPI pass, Electron ABI 145 SQL probe pass); E2E not run for isolated renderer-local maintenance activation with no IPC/persistence/lifecycle; no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/release/migration/sync/background/telemetry or Markdown/ReactMarkdown/editor KaTeX governance and no Main startup reorder/Redux/Dexie/SQLite/background windows/S6.5/Phase 8 change beyond maintenance activation; Shiki metadata via S7.8 demand wrappers only, CodeEditor/CodeMirror via S7.8 lazy only, XLSX via S7.5 only, KaTeX via S7.6 only, Mermaid demand-activation via S7.7 only, auto-sync via S7.9 only (bootstrap demand activation, SettingsPage static), maintenance via S7.10 only (scroll 0ms sweep + retention background split, check-before-refresh, eager deletion/byte-cache, plain setTimeout(0)); no byte-saving/startup SLA/performance magnitude/baseline/threshold/causal claim; residuals: S7.1 first-nav transient, S7.2 transient flash + heuristic `takeRecords()` limited, S7.3 cold first mount default-locale flash + no same-language retry, S7.4 cold initial configured-language fallback until resources resolve + same-language failure no inline retry (later language change/reload) + IIFE not fully isolated + i18next callback narrow + chunk directional only, S7.5 first export waits for chunk load + dynamic-load failure via Table error path with later retry + chunk topology not a performance quantification + no E2E needed + named/default normalization covered, S7.6 first math render waits for deferred assets + conservative detector may over-trigger but preserves parser-supported forms and does not reinterpret content + at most 2 attempts per block then reset on block/qualification transition + load failure via fallback/source Markdown and logger path + no E2E/live observation + topology not quantitative + MarkdownEditor/RichEditor not included, S7.7 first Mermaid preview waits for deferred chunk load + `CodeStyleProvider` no longer eagerly activates + topology/activation directional only + demand-owned via `MermaidPreview` only + no E2E needed, S7.8 first custom-theme highlight and first CodeEditor render wait for deferred chunks + default/auto uses fallback + `AsyncInitializer` supported reset enables retry + CodeEditor `Suspense` fallback is a localized loading shell with Ant Design `Spin`/loading text/`aria-busy` (`CodeEditorLoadingFallback`, `aria-busy`/`aria-label`, `data-testid="code-editor-loading"`) with bounded local error fallback (`data-testid="code-editor-error"`), no false chunk retry action + topology/activation directional only + Markdown/ReactMarkdown/editor KaTeX not included, S7.9 first demanded auto-sync awaits chunk load + load failure bounded to `loggerService` with no inline retry (later toggle/reload) + 8s semantics unchanged + topology directional only + SettingsPage static intentionally retained, S7.10 startup sweep deferred 0ms may briefly leave expired snapshots until post-bootstrap task fires — mitigated by per-key check-before-refresh (first read null, failed physical delete retains index with retry) + first retention enforce before background relies on eager subscriber/handler only + 0ms adds one bounded task + LRU failure may temporarily over-budget — accepted residual; no performance magnitude/SLA/baseline/causal claim; PERF independent/open; rollback one semantic revert(s), no migration; Phase 7 remains partially Open (S7.11+ deferred); no ready-now batch exists unless current user instruction explicitly activates another one** |
