...
- Data convergence per the approved [Sync Data Convergence ADR](./sync-data-convergence.md) (`SYNC-DATA-*`) is not implemented beyond local capture plus an unwired local merge: the deterministic local baseline candidate (see §3) is a future bootstrap input only, and the bounded local merge engine is not wired to relay/bootstrap, cannot resolve unversioned same-ID conflicts, and does not establish convergence or a no-gap watermark — no watermark-bound replay handoff, no manifest/digest reconciliation with targeted repair, no versioned syncable-data inventory beyond the provisional subset, and no per-domain convergence status. The current topic/message/message-block operation-log scope is not complete-product sync; structured content, ordering/segments/branches, and attachments/attachment bytes have no convergence contract.
