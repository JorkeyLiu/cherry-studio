...
- [Model Metadata Governance](docs/adr/model-metadata.md) — canonical governance for model identity (serving vs canonical vs display), dual-source responsibilities, enrichment-only, exact lookup, cache boundary; read it before changing provider/model resolution or metadata caching.
