...
> **Last updated**: 2026-08-30 — S7.2 implemented closure (600-900 line target). Prior detailed history in Git log; no chained provenance header is maintained.
...
- **Status**: **S7.1 + S7.2 Implemented & Closed 2026-08-30 (outcome/residual-risk)** — S7.1 renderer-only lazy five secondary routes (Home/sidebar/navigation/App gates eager); S7.2 renderer-local readiness loading/error/retry UX with projection gating + ReduxStoreReady ordering preserved (B-08 max 500 live Ranges unchanged). **Phase 7 remains partially Open — S7.3+ Open (deferred); no ready-now batch exists unless current user instruction explicitly activates another one.** Remaining Phase 7 tracks (Main startup reorder, Antd locale splitting, Inputbar, telemetry/retention/deletion) remain not authorized.
- **Entry**: Phase 2 complete; explicit approval. Conversation-startup (S3.5) already closed; S7.1/S7.2 independent of Phase 4/5/6 and do not depend on M4/M5/M6.
...
- **Dependencies**: Phase 2; Phase 3 stable host for conversation-startup (already satisfied). S7.1 touches only renderer bundle/activation; S7.2 touches only renderer readiness UX + B-08 optimization; both no Main/preload/shared/IPC/SQLite/Dexie/StoreSync/identity/sync/context-window change.
- **Relationship**: See §8 — S7.1 is the bundle/activation track, S7.2 is the readiness/hydration hardening track; conversation lazy activation (S3.5) remains distinct and not duplicated; remaining boot-service tracks remain deferred under S7.3+.
...
Independent/deferred tracks not blocked by S7.1/S7.2 (S7.3+ deferred):

| Track | Description | Independence | Status |
|---|---|---|---|
| App boot services | Service init order/parallelism | Independent | S7.3+ deferred (not selected, lifecycle races) |
...
| Startup improvements (S7.1/S7.2) | Phase 7 | **S7.1 + S7.2 Implemented & Closed 2026-08-30 (outcome/residual-risk) — S7.1 five secondary routes lazy as distinct production chunks, Home/sidebar/navigation/App gates eager, localized fallback + tagged recovery; S7.2 renderer-local readiness loading/error/retry UX (pending `Spin` vs failed `Alert`+retry, `failed→pending→ready/failed`, subscription cleanup, `startup.readiness.*` i18n) + B-08 `takeRecords()` draining with max 500 live Ranges, unconditional cross-chunk DOM rescan, `childList/subtree/characterData/attributes` preserved, LOCK-PROJECTION/LOCK-003 intact, no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/sync/S7.1 change; verification: S7.1 production chunks + Vitest + Playwright + S7.2 focused Vitest (readiness/gate/retry + B-08) + independent audit + authoritative `pnpm build:check`; no threshold/baseline/SLA; rollback one semantic revert(s), no migration; Phase 7 remains partially Open (S7.3+ deferred); no ready-now batch exists unless current user instruction explicitly activates another one** |
