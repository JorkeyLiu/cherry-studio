...
> **Last updated**: 2026-09-10 — public digest `jcs-sha256-v1` plus watermark-monotonic/idempotency clarification propagation (`SYNC-CC-019`/`SYNC-CC-020` align to `SYNC-DATA-023`/`SYNC-DATA-028`/`SYNC-DATA-029`/`SYNC-DATA-032`, target-only, code-less, no implementation authorized; closes F-01/F-02/F-03 with direct document evidence)
...
- **This ADR owns**: target service connection/registration semantics, channel/pairing semantics, device-code semantics, request lifecycle, channel creation/join/unpair/dissolve rules, per-channel sequencing requirements, relay per-channel current-effective baseline envelope storage/distribution infrastructure with serialized publish gating and in-flight bootstrap retention (`SYNC-CC-019`) plus single-stage client-declared publish/fetch operation semantics and status codes (`SYNC-CC-020`), offline-bootstrap distribution without presence election, and the Disconnect-vs-Unpair contract.
- **This ADR does not own**: current implementation status/evidence/gaps/next step ([multi-device-sync.md](./multi-device-sync.md)); data-convergence semantics — baseline origin/content truth, publisher barrier/snapshot, envelope content categories, watermark three-segment boundary, baseline publish serialization with watermark-monotonic/coverage gating and in-flight bootstrap protection, receiver merge, retention coverage, reconciliation, and convergence status ([sync-data-convergence.md](./sync-data-convergence.md), `SYNC-DATA-019`–`SYNC-DATA-031`); fallback candidate analysis ([sync-architecture-selection.md](./sync-architecture-selection.md)); application identity, updater/release freeze, platform scope ([cherry-chat-application-identity.md](./cherry-chat-application-identity.md)); SQLite chat authority and migration process ([sqlite-migration.md](./sqlite-migration.md)); context-window semantics ([context-window.md](./context-window.md)); initial snapshot/existing-data convergence content rules beyond the infrastructure duty stated here (separate sync-data problem owned by `SYNC-DATA-*` — see §12).
- **This ADR does not authorize**: protocol implementation, endpoint path strings, HTTP verb spellings, chunking/streaming, size thresholds, compression, retained physical history counts, lease/ack endpoints, timeouts, schema migrations, capacity thresholds, retained history version counts, retained physical old-baseline counts, old-baseline physical reclamation mechanics, GC mechanics, IPC changes, or any code change. Publish/fetch operation semantics and status codes are locked by `SYNC-CC-020` as a code-less contract; transport spellings and limits stay deferred. It creates no permissions/admin/merge/GC/snapshot/WAN/TLS-management decisions beyond what is written here.
...
Endpoint path strings, HTTP verb spellings, chunking/streaming, size thresholds, compression, retained physical history counts, lease/ack endpoints, timeouts, and SSE event names are implementation decisions under a later activated step; this ADR constrains only the behaviors above plus the `SYNC-CC-020` operation semantics and status codes.
...
- **Channel baselines**: per channel, one current effective baseline blob plus schema/digest/watermark metadata (SYNC-CC-017). A new publish replaces the effective baseline atomically only when gated by SYNC-CC-019 (serialized per-channel decision; monotonic floor `N >= current` in non-`empty` state with below-current `409`, plus recoverable/coverage gate that never substitutes for the floor; effective baseline unchanged on reject) and with in-flight bootstraps protected per SYNC-CC-019 (retain sufficient baseline/log/delete evidence or an equivalent recoverable mechanism until completion or explicit failure). Publish/fetch follows the single-stage operation semantics and status codes in SYNC-CC-020 (`empty` versus `current`; atomic replace; idempotent `200` only on full current-match with channel binding, N, digest scheme/version, digest, canonical payload; stale `409` for replaced-candidate retry without permanent fingerprint; `400`/`401`/`403`/`404` `{error:'baseline-not-found'}` on empty/`409` (same-N divergence, below-current watermark, or overwrite conflict)/`413`/`500` with `{error}` bodies; `jcs-sha256-v1` recompute proving byte-identity only with atomic envelope binding per `SYNC-DATA-032`; member-only fetch; hint-only SSE). With no compaction implemented, full log retention is the minimal protection path, not a never-compact lock. Prior-version physical reclamation mechanics, retained physical old-baseline counts, endpoint path strings, HTTP verb spellings, chunking/streaming, size thresholds, compression, lease/ack endpoints, timeouts, SSE event names, and GC mechanics are deferred. This row is envelope storage/distribution infrastructure only; baseline content truth, publisher barrier/snapshot, watermark proof, receiver merge, retention coverage, and the public digest contract are owned by `SYNC-DATA-*`.
...
- Initial snapshot and existing-data convergence content rules (what historical state a newly paired device receives, publisher barrier/snapshot, envelope content categories, watermark proof with monotonic/coverage gating, in-flight bootstrap protection, receiver merge, retention coverage, reconciliation) are a separate sync-data problem owned by [sync-data-convergence.md](./sync-data-convergence.md) (`SYNC-DATA-019`–`SYNC-DATA-031`). This ADR assumes a working per-channel operation stream plus relay-persisted current-effective baseline distribution exists; it does not define backfill content, barrier orchestration, watermark proof, merge, retention coverage, or conflict-resolution behavior.
...
7. Per-channel current-effective baseline envelope holds: one effective baseline blob plus schema/digest/watermark metadata per channel with serialized monotonic (`N >= current` in non-`empty` state, below-current `409`; coverage never substitutes)/coverage-gated atomic replacement, envelope validation with save/distribute only (`jcs-sha256-v1` recompute proving byte-identity only, atomic envelope binding per `SYNC-DATA-032`), in-flight bootstrap N+1 replay/repair retention per SYNC-CC-019, single-stage publish/fetch operation semantics plus idempotent-`200`-only-on-full-current-match / stale-`409`-without-permanent-fingerprint / `400`/`401`/`403`/`404` (`{error:'baseline-not-found'}` on empty)/`409` (same-N divergence, below-current watermark, or overwrite conflict)/`413`/`500` with `{error}` bodies and member-only fetch plus hint-only SSE per SYNC-CC-020, and offline fetch availability without presence/source election.
...
- Snapshot/backfill content, publisher barrier/snapshot, envelope content categories, watermark proof with monotonic/coverage gating, in-flight bootstrap protection, receiver merge, retention coverage, and reconciliation: owned by `SYNC-DATA-019`–`SYNC-DATA-031`. Consequence: pairing a fresh device plus the relay baseline infrastructure does not by itself implement what history it receives; that behavior still requires its own explicitly activated step.
- Prior-version baseline physical reclamation mechanics, retained physical old-baseline counts, endpoint path strings, HTTP verb spellings, chunking/streaming, size thresholds, compression, capacity thresholds, retained history version counts, lease/ack endpoints, timeouts, SSE event names, and GC mechanics: deferred. Consequence: only the single-current-effective-baseline replacement semantic with SYNC-CC-019 serialized gating and in-flight retention plus the SYNC-CC-020 operation semantics and status codes is locked; how old blobs are reclaimed, how completion/failure is signalled, which transport spellings and limits apply, and how many physical old baselines are kept stays undecided.
