...
import type { ValidationRequest } from '../../utils/zodValidator'
...
export const listKnowledgeBases = async (req: ValidationRequest, res: Response): Promise<Response> => {
  try {
    ...
    //           Redux access requires the Cherry Chat window to be open.
    ...
    try {
      ...
      if (isReduxUnavailableError(error)) {
        ...
        return res.status(503).json({
          error: {
            message: 'Knowledge bases are only available when the Cherry Chat window is open',
  ...
  try {
    ...
    if (!bases || bases.length === 0) {
      return res.json({
        ...
        total: 0,
        searched_bases: [],
        warnings: ['No knowledge bases configured. Please add knowledge bases in Cherry Chat.']
      })
    }
