...
- [Sync architecture selection](docs/sync-architecture-selection.md) — **canonical S8 selection validation authority (Active, documentation/research only, no implementation authorization)**: product target (Syncthing-like local-first, no Cherry Chat account, device pairing, one user-selected service), current-state constraints, balanced four-dimension method, candidate taxonomy/shortlist, research findings, equal-scenario criteria, four-spike plan (S8-SP1..SP4), decision rules, and write-amplification gate; read this before any sync work.
