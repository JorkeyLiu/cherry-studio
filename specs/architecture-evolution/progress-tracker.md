...
- **Phase 4 Bounded Memory and Cache — exit Open (outcome/residual-risk based).** Lifecycle foundation implemented as renderer-local, non-persistent, non-StoreSync groundwork: per-topic `chatData`/`segments`/`residentTopic` completeness with monotonic `applicabilityGeneration` and same-generation staged joint publication via one dispatch. `B-06` viewport bound (renderer-local, target max 200 groups, opposite-edge trim, anchor-preserving), `B-07` device-local persistent Keyv scroll cache (renderer-local index, max 256 topics / 90-day TTL LRU), `B-08` ContentSearch bound (renderer-local disposable session, at most one 500 live-Range chunk), `B-09` active-topic retention (max one) are implemented. Diagnostics are bounded scalar-only (no policy/persistence/IPC change). B-01..B-05 are approved initial renderer-local retention policy defaults, implementation pending, not empirically optimal, not baselines/SLA, calibration optional/non-blocking, harness inactive by default and requires explicitly activated decision/outcome + four fields to count as valid/progress. Controlled regression and protected boundaries remain mandatory (§10.1).
- **Phase 5 Data-Access Contract — design complete, exit Open.** Read intents `R-02..R-06`, completeness semantics, deterministic `sort_order -> id` ordering, viewport vs context separation, and coordinated shared-contract review are direction-approved. No implementation beyond the contract is claimed.
- **Phase 6 DB-Health — `S6.1..S6.3` Authorized & Implemented; `S6.4`/`S6.5` Candidate — Not Authorized.** `S6.1` windowed reads, `S6.2` authority-aware actions, `S6.3` context closure and cache joins implemented. `S6.4`/`S6.5` remain blocked. M4 measurement-only directional evidence (1k/10k/50k via bench:m4-fts-dup). M5 file dual-state harness implemented/executed only as bounded synthetic directional L3 — future runs inactive by default; no user data, no production change, no threshold/baseline, no S6.5 authorization, no phase closure. M6 harness implemented, inactive by default, not yet executed — analysis-only, no schema/migration/sync design (4 schema-absent + 5 open-design categories, 5 required tables + 1 excluded file_references, 11 excluded sync domains). Focused renderer regression coverage evidenced via `pnpm build:check` pass on exact code state; no production/IPC/schema change, no threshold/baseline, no phase closure. Real-corpus/physical-size and production S6.5 remain ADR-gated.
...
- `B-01..B-05` (inactive resident cap 8, logical payload 32 MiB, idle TTL 30 min, LRU, single-oversized) are **approved initial renderer-local retention policy defaults, implementation pending, not empirically optimal, not baselines/SLA, calibration optional/non-blocking**. No production retention/eviction/TTL/LRU enforcement active. Adoption requires an explicit decision; directional artifacts alone do not authorize.
- `S6.4` (index/query) and `S6.5` (file dual-state / FTS dedup) are **Candidate — Not Authorized**. M5 synthetic evidence does not authorize production work; M6 is analysis-only and does not authorize sync design; each requires per-slice evidence and governance/ADR before any schema/query/storage/authority change.
...
- No additional cohesive ready-now batch is currently identified; the queue remains empty until the next reconnaissance pass. (Previous renderer-only regression-coverage batch evidenced and audited with `pnpm build:check` passing — no new behavior or phase closure.)
...
- Calibration activation and any post-calibration adoption decision for `B-01..B-05` (approved defaults pending implementation; calibration optional/non-blocking; directional calibrations are measurement-only and do not authorize policy).
- `B-01..B-05` production retention/eviction/TTL/LRU enforcement — requires explicit decision, not yet implemented.
- `S6.4` index/query and `S6.5` file dual-state / FTS dedup — each needs slice evidence plus ADR/governance; M4/M5 synthetic directional evidence does not authorize production inference. Real-corpus/physical-size remain unresolved; any S6.5 work still requires explicit Main/user activation, decision, privacy review where applicable, and ADR; S6.5 remains Candidate — Not Authorized.
