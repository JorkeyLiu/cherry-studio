...
> **Last updated**: 2026-09-02 — S7.13 Startup Stage Instrumentation Implemented & Closed (attribution-only instrumentation, bounded privacy-safe closed records; no production optimization, baseline, SLA, or threshold; S7.14+ deferred, later production optimization not authorized; residual runtime-evidence risk closed 2026-09-02 via fresh enabled Electron runtime E2E — `STARTUP_STAGE_ATTR=1 pnpm build` exit 0 then `STARTUP_STAGE_ATTR=1 STARTUP_STAGE_SYNTHETIC=1 pnpm test:e2e -- tests/e2e/specs/startup/startup-stage-instrumentation.spec.ts` exit 0, 1 passed); S7.12 Startup Critical-Path Attribution retained (attribution-only, no production optimization, timing-dominant unknown); M4/M5 synthetic batch accepted 2026-09-01 (M4 `chatdb-m4-fts-duplication-50k` 50k + M5 `chatdb-m5-file-dual-state` medium, derived; S6.5 Candidate — Not Authorized) + 2026-09-02 extensions — S7.14-E1 first-data attribution (renderer.firstData, renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional; no S7.14 closure, no winner/threshold) and S6.5-E1 M4 synthetic physical-page/file proxy (four raw metrics, sanitized failure paths, synthetic isolated; no S6.5 authorization); S7.1–S7.13 retained. Git owns history; no chained provenance.
...
| **7** | Startup Architecture | **S7.1–S7.13 Implemented & Closed 2026-09-02 (outcome/residual-risk; S7.12 attribution-only, S7.13 attribution-only instrumentation, bounded privacy-safe, no production optimization/baseline/SLA/threshold); S7.14-E1 first-data attribution evidence integrated 2026-09-02 (renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional; S7.14 not implemented/closed, no winner/threshold); S7.14+ deferred — See §6.8.2–§6.8.15; S7.12 topology-only attribution closed with no production optimization; S7.13 instrumentation closed with no production optimization; Phase 7 remains partially Open (S7.14+ deferred, later production optimization not authorized); no ready-now batch without explicit activation** | Phase 2; Phase 3 for conversation-startup |
...
- **Status**: **S7.1–S7.13 Implemented & Closed 2026-09-02 (outcome/residual-risk; S7.12 attribution-only, S7.13 attribution-only instrumentation, bounded privacy-safe, no production optimization/baseline/SLA/threshold); S7.14-E1 first-data attribution evidence integrated 2026-09-02 (renderer-owned behavior plus shared diagnostic stage-union extension, no IPC/preload/shared application contract change, isolated, directional, no S7.14 closure/winner/threshold; see §6.8.15); S7.14+ deferred** — See §6.8.2–§6.8.15 for per-slice outcomes; S7.12 Startup Critical-Path Attribution closed with no production optimization, timing-dominant unknown (controlled non-adoption); S7.13 Startup Stage Instrumentation closed with no production optimization, baseline, SLA, threshold, or winner — attribution-only harness only; S7.14-E1 is attribution-only isolated evidence and does not close S7.14. Phase 7 remains partially Open (S7.14+ deferred, later production optimization not authorized). Remaining Phase 7 tracks beyond S7.13 (background-window policy, S6.5, Phase 8) remain deferred.
...
- **Dependencies**: Phase 2; Phase 3 stable host for conversation-startup (already satisfied). S7.1–S7.13 all attribution/topology, renderer-local, or attribution-only instrumentation with no Main/preload/shared IPC/SQLite/Dexie/StoreSync/identity/release/migration/sync/background/telemetry or Markdown/ReactMarkdown/editor KaTeX governance crossing — see §6.8.2–§6.8.15 for per-slice touch boundaries; S7.12 attribution-only, changes no production code/config; S7.13 instrumentation-only, bounded privacy-safe, no authority/contract/schema/migration/StoreSync/retention change; S7.14-E1 renderer-owned behavior plus shared diagnostic stage-union extension (no IPC/preload/shared application contract change), no authority, compatibility, SQLite schema/migration/index/trigger, StoreSync, identity, sync, release, or platform change. S7.14+ deferred.
- **Relationship**: See §8 — S7.1 bundle/activation, S7.2 readiness/hydration, S7.3 Antd locale on-demand, S7.4 translation JSON + Day.js locale, S7.5 XLSX export, S7.6 Home-message KaTeX, S7.7 Mermaid demand-activation, S7.8 code tooling, S7.9 auto-sync, S7.10 maintenance (0ms sweep + retention split), S7.11 critical bootstrap failure isolation, S7.12 attribution-only critical-path topology (see §6.8.2–§6.8.13), S7.13 startup stage instrumentation (see §6.8.14), S7.14-E1 first-data attribution (see §6.8.15); conversation lazy activation (S3.5) distinct; S7.14+ deferred (background-window policy, S6.5, Phase 8).
...
- **No winner/baseline/SLA/threshold/production optimization/contract change** — attribution-only isolated evidence to select one later optimization after synthesis; S7.14 remains deferred.
...
**Residual risks (accepted)**: Enabled path requires build + runtime gate plus S7.13 exact disposable validation; ordinary builds/profile stay inert (default-off). `renderer.firstData` is renderer-local attribution-only, one-shot bounded interval, not a timing-dominant winner or baseline/SLA; stale/moved/deletion/generation/superseded settlements are correctly discarded but consume eligibility only via pre-dispatch or cache-hit consumption, not via stale predicate discard — next eligible records correctly. First-data real-corpus/physical cost/incidence/threshold, Dexie open/upgrade timing, and handler/preload ordering remain unresolved — S7.14-E1 remains directional only and S7.14 is not implemented/closed; later production optimization not authorized. Renderer predicate is best-effort without cross-process active-topic contract — fail-closed rather than inferring. No threshold/baseline/SLA; Phase 7 partially Open.
...
Independent/deferred tracks not blocked by S7.1–S7.13 — S7.1–S7.13 Implemented & Closed 2026-09-02 (outcome/residual-risk; S7.12 attribution-only, S7.13 attribution-only instrumentation), S7.14-E1 first-data attribution evidence integrated 2026-09-02 (renderer-local, isolated, directional; S7.14 not closed), S7.14+ deferred (later production optimization not authorized):
