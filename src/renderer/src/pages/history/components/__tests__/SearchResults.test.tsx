...
describe('SearchResults (SQLite search)', () => {
  ...
  describe('matching options and order', () => {
    it('substring mode change issues a new first-page request with matchMode=substring', async () => {
      ...
      // Locale-independent match-mode testids wrap the Segmented option labels.
      expect(screen.getByTestId('history-search-match-whole-word')).toBeInTheDocument()
      const substringOption = screen.getByTestId('history-search-match-substring')
      expect(substringOption).toBeInTheDocument()
      fireEvent.click(substringOption)
  ...
  describe('stale response safety', () => {
    ...
    it('mode change resets to first page', async () => {
      ...
      fireEvent.click(screen.getByTestId('history-search-match-substring'))
      await waitFor(() => expect(searchMessagesMock).toHaveBeenCalledTimes(3))
      await waitFor(() => expect(screen.getByTestId('pagination')).toHaveAttribute('data-current', '1'))
  ...
  // Unified navigation: search hits navigate directly via the stable-ID entry.
  // No pseudo-Message, no preview stub, no window fetch/publish in SearchResults.
  describe('search-hit direct navigation (unified navigation)', () => {
    it('non-resident hit calls the stable locate target without fetch/dispatch/stub preview', async () => {
      ...
      // No preview path: legacy onMessageClick stays unused for hits.
      expect(onMessageClick).not.toHaveBeenCalled()
      // SearchResults never touches the window/projection surface.
    ...
    it('resident hit still uses the stable locate target (no stub preview, no window fetch)', async () => {
      const anchor = makeItem(1)
      const resident = { id: 'message-1', topicId: 'topic-1', assistantId: 'assistant-1', blocks: ['block-1'] }
      storeTopicsMap.set('topic-1', { id: 'topic-1', name: 'Topic topic-1', assistantId: 'assistant-1' } as any)
      storeGetStateMock.mockReturnValue({
        messages: {
          entities: { 'message-1': resident },
    ...
    it('deleted-topic hit fails closed with toast and no locate, fetch, or publish', async () => {
      const anchor = makeItem(1)
      searchMessagesMock.mockResolvedValue(makeResponse([anchor]))
      storeGetStateMock.mockReturnValue({
        messages: { entities: {}, messageIdsByTopic: { 'topic-1': [] }, currentTopicId: 'topic-1' },
      ...
      await waitFor(() => expect(screen.getAllByTestId('result-item')).toHaveLength(1))
      // Simulate hard deletion after results render but before the hit click.
      ...
      expect(fetchMessagesWindowMock).not.toHaveBeenCalled()
      // The test's own bumpDeletionGeneration emits a residentRegistry invalidation;
      // assert no other (projection) dispatches occurred.
