...
#### 6.7.1 S6.4 Index/Query Authorization Review — Need Specific Evidence (2026-08-29)

S6.4 reviewed 2026-08-29 — **outcome: Need Specific Evidence; S6.4 remains Candidate — Not Authorized** (`ARCH-001..ARCH-012` unchanged; no production code, tests, schema, index, migration, IPC, StoreSync, S6.5, sync, startup, retention setting, threshold/baseline/SLA, or `architecture.md` change). Existing M1/M2/M3 evidence proves observed structures, harness fidelity, and current query-plan attribution, but does not demonstrate a concrete alternative's structural benefit. M1 confirms O(N-index) sibling `sort_order` shifts under dense ordering and current composite index, but no concrete query/index alternative or current-plan undercoverage is proven; existing implementation already minimizes tail append and batch normalization. M2 confirms <3-codepoint `LIKE` full scan with exact semantic parity, but existing FTS trigram cannot serve <3 terms and no concrete alternative is evaluated. M3 diagnoses the existing search fetch query plan structure on a synthetic 50k corpus and is directional only; it evaluated no alternative and does not establish index benefit or alternative improvement. M1 dense-order shift and M2/M3 search-query concerns are separate semantic-risk candidates; evidence must not be pooled to imply authorization. Any schema/index change remains SQLite migration/ADR-gated; a query-only change still requires exact candidate + parity + structural improvement evidence. Specific next evidence is candidate-bound, not generic measurement: identify an exact finite alternative; prove semantic parity and structural plan/write-path improvement in an isolated production-shaped corpus (real user data not required or authorized); state migration/ADR, write-amplification/storage, rollback, and protected ordering/Unicode/cursor semantics; requires explicitly activated decision/outcome with all four evidence-task fields to count as progress.
...
| Specific index/query optimizations | Phase 6 | **Candidate S6.4 — Not Authorized (reviewed 2026-08-29 — Need Specific Evidence)**; M1 and M2/M3 are separate candidate semantic-risk boundaries, no evidence pooling; any schema/index remains ADR-gated, query-only still requires candidate + parity + improvement; see §6.7.1 for full rationale and candidate-bound evidence requirements (isolated production-shaped corpus, real user data not required/authorized, migration/ADR, write-amplification/storage, rollback, protected ordering/Unicode/cursor semantics; no production/IPC/StoreSync/S6.5/threshold/baseline/SLA/`architecture.md` change) |
