...
| 治理领域 | 权威文档 | 本程序的边界 |
|---|---|---|
| 应用身份 / 兼容域 / 发布冻结 / 平台范围 | [`docs/adr/cherry-chat-application-identity.md`](../adr/cherry-chat-application-identity.md) | 不改变；涉及即触发 §9 ADR |
| SQLite 聊天权威 / L1 运行时 / L2 兼容导入 / L3 备份恢复 | [`docs/archived/sqlite-migration.md`](sqlite-migration.md) | 不改变；迁移/原子性/兼容语义涉及即触发 §9 ADR |
| 架构参考（进程职责、IPC、目录、技术栈） | [`docs/architecture/architecture.md`](../architecture/architecture.md) | 作为理解与测量的上下文；不修改 |
...
- **治理**：[`sqlite-migration.md`](sqlite-migration.md)、[`cherry-chat-application-identity.md`](../adr/cherry-chat-application-identity.md)、[`architecture.md`](../architecture/architecture.md)。
