...
> **文档状态**：In progress（Phase 0–3 完成；Phase 4.0 Done on macOS arm64；Phase 4.1 Done；Phase 4.2 Done；Phase 4.3 Done（已提交/已推送至迁移分支 `85603d0fd5`）；集成同步门 Baseline Sync Gate Done（integration `05a401b711` 已集成同步，已验证）；Phase 4.4.0（Promotion 协议基础，纯协议层）Done；Phase 4.4.1（Durable Preparation Gate，快照就绪准备门）Done；Phase 4.4.2（Destructive Promotion Executor，破坏性替换执行，止于 durable replacement-verified）Done（实现 + 聚焦验证 + 独立审计 pass-with-findings、两项接受的 ownership 硬化修正已落地复验、最终全量验证通过，未提交/未推送）；Phase 4.4.3+ 未开始）
>
...
| 属性 | 值 |
|---|---|
| **状态** | In progress（Phase 4.4.0 Done；Phase 4.4.1 Done；Phase 4.4.2 Done（实现 + 聚焦验证 + 独立审计修正落地 + 最终全量验证通过，未提交/未推送）；Phase 4.4.3 Not started） |
...
**非目标（明确排除，属 Phase 4.4.2+；其中 live close/install/reopen/verify 已由 Phase 4.4.2 落地，rollback restore/relaunch/启动恢复执行仍属 Phase 4.4.3）**：
...
**非目标（明确排除，属 Phase 4.4.3+，Not started）**：
- 不消费 rollback 快照执行 restore（快照仅保留）
- 不执行 relaunch
- 不清理 promotion journal / snapshot / candidate 残留
- 不实现启动恢复**动作执行器**——Phase 4.4.0 恢复矩阵仍为纯决策、Phase 4.4.1 启动候选保护仍为读取/保护/分类，均不执行 restore/accept/repair 动作
- 不新增 promotion IPC / preload / Renderer 触发路径
...
| 原子 promotion | 成功 → reopen + relaunch；失败 → 回滚到快照 | In progress（Phase 4.4.1 完成 rollback 快照创建 + journal `snapshot-ready` 准备门；Phase 4.4.2 完成 exact-once 执行转移 + 授权 close → 原子 rename-only install → durable journal 递进 → 授权 reopen → identity-bound 验证，止于 durable `replacement-verified`；rollback restore 执行 / relaunch / 启动恢复动作执行仍属 Phase 4.4.3，Not started） |
...
| Promotion protocol foundations | `src/main/services/chatDbImport/promotion/`（protocol/journal/recovery，Phase 4.4.0 已创建；Phase 4.4.1 新增 journalStore 落盘；Phase 4.4.2 加严 transition） | 纯协议层：promotion 状态/exact-once claim、journal v1 严格 codec、确定性恢复矩阵；Phase 4.4.1 新增独立 `journalStore.ts` crash-safe 落盘 writer（原子 rename 写入 `chat-import-promotion.journal.json`，内容恰好 version/sessionId/candidateId/phase；snapshot-ready 已落盘，LOCK-4413；`journal.ts` 保持纯 codec）；Phase 4.4.2 在 `journalStore.ts` 新增受门控 phase transition API（见 Phase 4.4.2） |
| Maintenance coordination contract | `src/main/services/chatDb/maintenanceCoordination.ts`（Phase 4.4.0 已创建；Phase 4.4.1 接线；Phase 4.4.2 扩展验证缝） | backup/restore/promotion/init/close 统一互斥 lease 契约（LOCK-4402）；Phase 4.4.1 将 promotion 实际接入既有 backup/restore/init/close 互斥协调、成为唯一 lease 持有者、无第三独立运行时锁（LOCK-4411）；Phase 4.4.2 新增 `validatePromotionAuthorization` 验证缝（模块私有 grant 注册表 + holder peek，仅 promotion 授权 verdict，不授予/不释放，LOCK-4422）+ `ChatDbService.closeForPromotion/reopenForPromotion` promotion-owned live 生命周期（`src/main/services/chatDb/index.ts`） |
| Atomic promotion executor | `src/main/services/chatDbImport/promotion/`（Phase 4.4.1 交付 Durable Preparation Gate；Phase 4.4.2 交付破坏性执行 execution/install/replacementVerifier/readonlyDbValidation；restore/relaunch/启动恢复动作执行仍属 Phase 4.4.3） | Phase 4.4.1：rollback 快照创建（`chat.db.pre-import-backup`，live→staging→验证→原子 rename，不复制 WAL/SHM，仅发布不 restore，LOCK-4412）+ journal `snapshot-ready` 落盘 + exact-once prepared handle（LOCK-4415）+ 非破坏性边界（LOCK-4416）。Phase 4.4.2：`execution.ts`（不可重排执行序列 closing-live→minting-proof→installing→journal-candidate-installed→reopening-live→verifying-replacement→journal-replacement-verified→settled，pre/post-install 失败分类，LOCK-4421..4428）、`install.ts`（closed-live proof + sidecar 处理 + 原子 rename-only + fsync + identity receipt，EXDEV 无 copy fallback，LOCK-4423/4426/4427）、`replacementVerifier.ts`（identity-bound 前后校验 + 完整只读门，LOCK-4424）、`readonlyDbValidation.ts`（快照/替换共享只读验证门）。rollback restore 执行 / relaunch / 启动恢复动作执行仍属 Phase 4.4.3 |
